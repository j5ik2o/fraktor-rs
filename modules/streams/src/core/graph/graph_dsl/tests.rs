...
fn graph_dsl_builds_reusable_flow_fragment() {
  let dsl = GraphDsl::from_flow(crate::core::stage::flow::Flow::new().map(|value: u32| value + 1));
  let flow = dsl.build();
...
fn graph_dsl_via_and_to_compose_fragment() {
  let graph = Source::single(2_u32)
    .via(GraphDsl::from_flow(crate::core::stage::flow::Flow::new().map(|value: u32| value + 1)).build())
...
fn graph_dsl_supports_fan_out_and_fan_in() {
  let dsl = GraphDsl::from_flow(crate::core::stage::flow::Flow::new().map(|value: u32| value + 1_u32))
    .broadcast(2)
    .expect("broadcast")
    .merge(2)
    .expect("merge");
...
fn graph_dsl_rejects_zero_fan_parameters() {
  assert!(GraphDsl::from_flow(crate::core::stage::flow::Flow::new().map(|value: u32| value + 1)).broadcast(0).is_err());
  assert!(GraphDsl::from_flow(crate::core::stage::flow::Flow::new().map(|value: u32| value + 1)).balance(0).is_err());
  assert!(GraphDsl::from_flow(crate::core::stage::flow::Flow::new().map(|value: u32| value + 1)).merge(0).is_err());
  assert!(GraphDsl::from_flow(crate::core::stage::flow::Flow::new().map(|value: u32| value + 1)).concat(0).is_err());
}
