...
pub(in crate::core::stage::flow) struct RecoverWithRetriesLogic<Out, F> {
  ...
  pub(in crate::core::stage::flow) recovery_source: Option<SecondarySourceRuntime<Out>>,
}
  ...
  F: FnMut(StreamError) -> Option<Source<Out, StreamNotUsed>> + Send + Sync + 'static,
{
  fn begin_recovery(&mut self, error: StreamError) -> Result<FailureAction, StreamError> {
    ...
    self.recovery_source = Some(SecondarySourceRuntime::new(source)?);
  ...
  F: FnMut(StreamError) -> Option<Source<Out, StreamNotUsed>> + Send + Sync + 'static,
{
  ...
  fn has_pending_output(&self) -> bool {
    self.recovery_source.as_ref().is_some_and(SecondarySourceRuntime::has_pending_output)
  }
