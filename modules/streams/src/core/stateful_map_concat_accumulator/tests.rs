...
impl StatefulMapConcatAccumulator<i32, i32> for Doubler {
  fn apply(&mut self, input: i32) -> alloc::vec::Vec<i32> {
    alloc::vec![input, input * 2]
  }
}
...
impl StatefulMapConcatAccumulator<i32, i32> for CountingAccumulator {
  ...
  fn on_complete(&mut self) -> alloc::vec::Vec<i32> {
    alloc::vec![self.count as i32]
  }
}
...
fn apply_should_transform_elements() {
  let mut acc = Doubler;
  assert_eq!(acc.apply(5), alloc::vec![5, 10]);
  assert_eq!(acc.apply(3), alloc::vec![3, 6]);
}
...
fn on_complete_should_emit_trailing_elements() {
  ...
  assert_eq!(trailing, alloc::vec![3]);
}
