...
use crate::dsl::Compression;
use crate::r#impl::fusing::*;
...
where
  T: Send + Sync + 'static,
{
  pub(crate) fn from_kill_switch_state(kill_switch_state: KillSwitchStateHandle) -> Self {
  ...
  pub(crate) fn from_coupled_termination_state(kill_switch_state: KillSwitchStateHandle) -> Self {
    let mut graph = StreamGraph::new();
  ...
  Out: Send + Sync + 'static,
{
  ...
  #[must_use]
  pub fn into_mat<Mat2, C>(self, sink: Sink<Out, Mat2>, _combine: C) -> Sink<In, C::Out>
  where
    C: MatCombineRule<Mat, Mat2>, {
    let (mut graph, left_mat) = self.into_parts();
  ...
  ///
  /// ```compile_fail
  /// use fraktor_stream_core_kernel_rs::{StreamNotUsed, SubstreamCancelStrategy, dsl::Flow};
  ///
  ...
  pub(crate) fn into_logics(self) -> (Vec<Box<dyn FlowLogic>>, Mat) {
    let (graph, mat) = self.into_parts();
  ...
  Out: Send + Sync + 'static,
{
  ...
  where
    ...
    let logic = MaterializeIntoSourceLogic::<Mat2, _> {
      factory:    Some(move || {
        let graph = source.via(self).into_mat(sink, KeepRight);
  ...
  pub fn flat_map_prefix<T, Mat2, F>(mut self, prefix: usize, factory: F) -> Flow<In, T, Mat>
  where
    Out: Send + Sync + 'static,
    T: Send + Sync + 'static,
    Mat2: Send + Sync + 'static,
  ...
  Out: Send + Sync + 'static,
{
  ...
  ///
  /// [`SharedKillSwitch`]: crate::SharedKillSwitch
  #[must_use]
...
pub(crate) fn map_async_definition<In, Out, F, Fut>(parallelism: usize, func: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  Out: Send + Sync + 'static,
...
pub(crate) fn map_async_partitioned_definition<In, Out, P, Partitioner, F, Fut>(
  parallelism: usize,
  ordered: bool,
  partitioner: Partitioner,
  func: F,
...
pub(crate) fn log_definition<In>() -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
...
pub(crate) fn stateful_map_definition<In, Out, Factory, Mapper>(factory: Factory) -> FlowDefinition
where
  In: Send + Sync + 'static,
  Out: Send + Sync + 'static,
  Factory: FnMut() -> Mapper + Send + Sync + 'static,
...
pub(crate) fn stateful_map_concat_definition<In, Out, Factory, Mapper, I>(factory: Factory) -> FlowDefinition
where
  In: Send + Sync + 'static,
  Out: Send + Sync + 'static,
  Factory: FnMut() -> Mapper + Send + Sync + 'static,
...
pub(crate) fn stateful_map_with_on_complete_definition<In, Out, S, Factory, Mapper, OnComplete>(
  mut factory: Factory,
  mapper: Mapper,
  on_complete: OnComplete,
) -> FlowDefinition
...
pub(crate) fn stateful_map_concat_accumulator_definition<In, Out, Factory, Acc>(
  mut factory: Factory,
) -> FlowDefinition
where
  In: Send + Sync + 'static,
...
pub(crate) fn map_concat_definition<In, Out, F, I>(func: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  Out: Send + Sync + 'static,
...
pub(crate) fn map_option_definition<In, Out, F>(func: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  Out: Send + Sync + 'static,
...
#[cfg(feature = "compression")]
pub(crate) fn try_map_concat_definition<In, Out, F>(func: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  Out: Send + Sync + 'static,
...
pub(crate) fn filter_definition<In, F>(predicate: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
...
pub(crate) fn drop_definition<In>(count: usize) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn take_definition<In>(count: usize) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn limit_weighted_definition<In, FW>(max_weight: usize, weight_fn: FW) -> FlowDefinition
where
  In: Send + Sync + 'static,
...
pub(crate) fn drop_while_definition<In, F>(predicate: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  F: FnMut(&In) -> bool + Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
...
pub(crate) fn take_while_definition<In, F>(predicate: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  F: FnMut(&In) -> bool + Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
...
pub(crate) fn take_until_definition<In, F>(predicate: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  F: FnMut(&In) -> bool + Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
...
pub(crate) fn grouped_definition<In>(size: usize) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
...
pub(crate) fn grouped_within_definition<In>(size: usize, duration_ticks: u64) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<Vec<In>> = Outlet::new();
...
pub(crate) fn grouped_weighted_definition<In, FW>(max_weight: usize, weight_fn: FW) -> FlowDefinition
where
  In: Send + Sync + 'static,
  FW: FnMut(&In) -> usize + Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
...
pub(crate) fn grouped_weighted_within_definition<In, FW>(
  max_weight: usize,
  duration_ticks: u64,
  weight_fn: FW,
) -> FlowDefinition
...
pub(crate) fn conflate_with_seed_definition<In, T, FS, FA>(seed: FS, aggregate: FA) -> FlowDefinition
where
  In: Send + Sync + 'static,
  T: Send + Sync + 'static,
  ...
  let logic =
    ConflateWithSeedLogic::<In, T, FS, FA> { seed, aggregate, pending: None, just_updated: false, _pd: PhantomData };
  FlowDefinition {
    kind:        StageKind::Custom,
    inlet:       inlet.id(),
    outlet:      outlet.id(),
...
pub(crate) fn expand_definition<In, F, I>(expander: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  F: FnMut(&In) -> I + Send + Sync + 'static,
...
pub(crate) fn sliding_definition<In>(size: usize) -> FlowDefinition
where
...
pub(crate) fn scan_definition<In, Acc, F>(initial: Acc, func: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  Acc: Clone + Send + Sync + 'static,
...
pub(crate) fn intersperse_definition<In>(start: In, inject: In, end: In) -> FlowDefinition
where
  In: Clone + Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn flat_map_concat_definition<In, Out, Mat2, F>(func: F) -> FlowDefinition
where
  In: Send + 'static,
  Out: Send + Sync + 'static,
  Mat2: Send + Sync + 'static,
...
pub(crate) fn flat_map_merge_definition<In, Out, Mat2, F>(breadth: usize, func: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  Out: Send + Sync + 'static,
  Mat2: Send + Sync + 'static,
...
pub(crate) fn flat_map_prefix_definition<In, Out, Mat2, F>(prefix_len: usize, factory: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  Out: Send + Sync + 'static,
  Mat2: Send + Sync + 'static,
...
pub(crate) fn prefix_and_tail_definition<In>(prefix_len: usize) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
...
pub(crate) fn buffer_definition<In>(capacity: usize, overflow_strategy: OverflowStrategy) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn throttle_definition<In>(capacity: usize, mode: ThrottleMode) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn delay_definition<In>(delay_ticks: u64) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn initial_delay_definition<In>(initial_delay_ticks: u64) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn take_within_definition<In>(duration_ticks: u64) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn debounce_definition<In>(silence_ticks: u64) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn sample_definition<In>(interval_ticks: u64) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn backpressure_timeout_definition<In>(duration_ticks: u64) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn completion_timeout_definition<In>(duration_ticks: u64) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn idle_timeout_definition<In>(duration_ticks: u64) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn initial_timeout_definition<In>(duration_ticks: u64) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn batch_definition<In>(size: usize) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<Vec<In>> = Outlet::new();
...
pub(crate) fn group_by_definition<In, Key, F>(
  max_substreams: usize,
  key_fn: F,
  substream_cancel_strategy: SubstreamCancelStrategy,
) -> FlowDefinition
...
pub(crate) fn map_error_definition<In, F>(mapper: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  F: FnMut(StreamError) -> StreamError + Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
...
pub(crate) fn on_error_continue_definition<In, P, C>(predicate: P, error_consumer: C) -> FlowDefinition
where
  In: Send + Sync + 'static,
  P: FnMut(&StreamError) -> bool + Send + Sync + 'static,
  C: FnMut(&StreamError) + Send + Sync + 'static, {
...
pub(crate) fn on_error_complete_definition<In, F>(predicate: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
...
pub(crate) fn recover_definition<In, F>(recover: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
...
pub(crate) fn recover_with_retries_definition<In, F>(max_retries: isize, recover: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
...
pub(crate) fn split_when_definition<In, F>(predicate: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  F: FnMut(&In) -> bool + Send + Sync + 'static, {
...
pub(crate) fn split_when_definition_with_cancel_strategy<In, F>(
  predicate: F,
  substream_cancel_strategy: SubstreamCancelStrategy,
) -> FlowDefinition
where
...
pub(crate) fn split_after_definition<In, F>(predicate: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  F: FnMut(&In) -> bool + Send + Sync + 'static, {
...
pub(crate) fn split_after_definition_with_cancel_strategy<In, F>(
  predicate: F,
  substream_cancel_strategy: SubstreamCancelStrategy,
) -> FlowDefinition
where
...
pub(crate) fn merge_substreams_definition<In>() -> FlowDefinition
where
  In: Send + Sync + 'static, {
  flatten_substreams_definition::<In>(StageKind::FlowMergeSubstreams)
}

pub(crate) fn merge_substreams_with_parallelism_definition<In>(parallelism: usize) -> FlowDefinition
where
  In: Send + Sync + 'static, {
...
pub(crate) fn concat_substreams_definition<In>() -> FlowDefinition
where
  In: Send + Sync + 'static, {
...
pub(crate) fn broadcast_definition<In>(fan_out: usize) -> FlowDefinition
where
  In: Clone + Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn wire_tap_definition<In>() -> FlowDefinition
where
  In: Clone + Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn partition_definition<In, F>(predicate: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  F: FnMut(&In) -> bool + Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
...
pub(crate) fn balance_definition<In>(fan_out: usize) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn merge_definition<In>(fan_in: usize) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn merge_preferred_definition<In>(fan_in: usize) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn merge_prioritized_definition<In>(fan_in: usize, priorities: &[usize]) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn merge_sorted_definition<In>(fan_in: usize) -> FlowDefinition
where
...
pub(crate) fn interleave_definition<In>(fan_in: usize) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn prepend_definition<In>(fan_in: usize) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn prepend_lazy_definition<In, Mat>(source: Source<In, Mat>) -> FlowDefinition
where
  In: Send + Sync + 'static,
  Mat: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
...
pub(crate) fn zip_definition<In>(fan_in: usize) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<Vec<In>> = Outlet::new();
...
pub(crate) fn zip_all_definition<In>(fan_in: usize, fill_value: In) -> FlowDefinition
where
  In: Clone + Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<Vec<In>> = Outlet::new();
...
pub(crate) fn merge_latest_definition<In>(fan_in: usize) -> FlowDefinition
where
  In: Clone + Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<Vec<In>> = Outlet::new();
...
pub(crate) fn watch_termination_definition<In>(completion: StreamFuture<()>) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn kill_switch_definition<In>(kill_switch_state: KillSwitchStateHandle) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn coupled_termination_definition<In>(kill_switch_state: KillSwitchStateHandle) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn unzip_definition<In>() -> FlowDefinition
where
  In: Send + Sync + 'static, {
...
pub(crate) fn unzip_with_definition<In, Out, F>(func: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  Out: Send + Sync + 'static,
...
pub(crate) fn zip_with_index_definition<In>() -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
...
pub(crate) fn concat_lazy_definition<In, Mat>(source: Source<In, Mat>) -> FlowDefinition
where
  In: Send + Sync + 'static,
  Mat: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
...
pub(crate) fn concat_definition<In>(fan_in: usize) -> FlowDefinition
where
  In: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn or_else_definition<In, Mat>(secondary: Source<In, Mat>) -> FlowDefinition
where
  In: Send + Sync + 'static,
  Mat: Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
...
pub(crate) fn combine_mat<Left, Right, C>(left: Left, right: Right) -> C::Out
where
  C: MatCombineRule<Left, Right>, {
  C::combine(left, right)
}

pub(crate) fn keep_alive_definition<In>(max_idle_ticks: u64, injected_elem: In) -> FlowDefinition
where
  In: Clone + Send + Sync + 'static, {
  let inlet: Inlet<In> = Inlet::new();
  let outlet: Outlet<In> = Outlet::new();
...
pub(crate) fn switch_map_definition<In, Out, Mat2, F>(func: F) -> FlowDefinition
where
  In: Send + Sync + 'static,
  Out: Send + Sync + 'static,
  Mat2: Send + Sync + 'static,
...
/// exponential backoff.
pub(crate) fn retry_flow_definition<In, Out, R>(
