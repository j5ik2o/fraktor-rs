...
use crate::{DynValue, StreamError, shape::PortId};

/// Runtime connection table for graph interpreter edges.
pub(crate) struct GraphConnections {
...
impl GraphConnections {
  ...
  #[must_use]
  pub(crate) const fn new(edges: Vec<BufferedEdge>, dispatch: Vec<OutletDispatchState>) -> Self {
    Self { edges, dispatch }
  }

  /// Returns all buffered edges.
  #[must_use]
  pub(crate) fn edges(&self) -> &[BufferedEdge] {
    &self.edges
  }

  /// Returns whether an outlet has buffered outgoing data.
  #[must_use]
  pub(crate) fn has_buffered_outgoing(&self, from: PortId) -> bool {
  ...
  pub(crate) fn poll_incoming_with_preferred(
    &mut self,
    to: PortId,
  ...
  pub(crate) fn offer_next(&mut self, from: PortId, value: DynValue) -> Result<(), StreamError> {
  ...
  pub(crate) fn offer_at(&mut self, edge_index: usize, value: DynValue) -> Result<(), StreamError> {
  ...
  pub(crate) fn outgoing_edge_indices(&self, from: PortId) -> Result<Vec<usize>, StreamError> {
  ...
  #[must_use]
  pub(crate) fn incoming_edge_indices(&self, to: PortId) -> Vec<usize> {
  ...
  pub(crate) fn close_and_clear_incoming(&mut self, inlet: PortId) -> Result<(), StreamError> {
