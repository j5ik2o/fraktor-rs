...
use crate::{DynValue, FlowLogic, StreamError, downcast_value, dsl::DelayStrategy};
...
pub(crate) struct StrategyDelayLogic<In, S> {
  strategy:   S,
...
where
  S: DelayStrategy<In>,
{
  pub(crate) const fn new(strategy: S) -> Self {
