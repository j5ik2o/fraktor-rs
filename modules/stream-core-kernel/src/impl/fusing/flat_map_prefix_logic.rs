...
use crate::{DynValue, FlowLogic, StageDefinition, StreamError, downcast_value, dsl::Flow};

pub(crate) struct FlatMapPrefixLogic<In, Out, Mat, F> {
  pub(crate) prefix_len:    usize,
  pub(crate) factory:       F,
  pub(crate) prefix_values: Vec<In>,
  pub(crate) inner_logics:  Vec<Box<dyn FlowLogic>>,
  pub(crate) factory_built: bool,
  pub(crate) source_done:   bool,
  pub(crate) _pd:           PhantomData<fn(In) -> (Out, Mat)>,
}
