...
pub(crate) struct SecondarySourceBridge<Out> {
  stream:     Stream,
  ...
  Out: Send + Sync + 'static,
{
  ...
  pub(crate) fn new<Mat>(source: Source<Out, Mat>) -> Result<Self, StreamError>
  where
    Mat: Send + 'static, {
  ...
  pub(crate) fn poll_next(&mut self) -> Result<Option<Out>, StreamError> {
...
pub(crate) struct ConcatSourceLogic<Out, Mat> {
  pub(crate) secondary:        Option<Source<Out, Mat>>,
  pub(crate) secondary_bridge: Option<SecondarySourceBridge<Out>>,
  pub(crate) pending:          VecDeque<Out>,
  ...
  Mat: Send + Sync + 'static,
{
  ...
  fn drain_pending(&mut self) -> Result<Vec<DynValue>, StreamError> {
    ...
    if self.secondary_bridge.is_none()
      && let Some(source) = self.secondary.take()
    {
      self.secondary_bridge = Some(SecondarySourceBridge::new(source)?);
    }
    if let Some(bridge) = self.secondary_bridge.as_mut()
      && let Some(value) = bridge.poll_next()?
    {
  ...
  fn has_pending_output(&self) -> bool {
    self.source_done
      && (!self.pending.is_empty()
        ...
        || self.secondary_bridge.as_ref().is_some_and(SecondarySourceBridge::has_pending_output))
  }
...
pub(crate) struct PrependSourceLogic<Out, Mat> {
  pub(crate) secondary:         Option<Source<Out, Mat>>,
  pub(crate) secondary_bridge:  Option<SecondarySourceBridge<Out>>,
  pub(crate) pending_secondary: VecDeque<Out>,
  pub(crate) pending_primary:   VecDeque<Out>,
}
...
pub(crate) struct OrElseSourceLogic<Out, Mat> {
  pub(crate) secondary:         Option<Source<Out, Mat>>,
  pub(crate) secondary_bridge:  Option<SecondarySourceBridge<Out>>,
  pub(crate) pending_secondary: VecDeque<Out>,
  pub(crate) emitted_primary:   bool,
  pub(crate) source_done:       bool,
}
