...
#[cfg(test)]
#[path = "fan_out_shape21_test.rs"]
mod tests;

/// Shape with one input port and twenty-one output ports.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
pub struct FanOutShape21<
  In,
  Out0,
  Out1,
  Out2,
...
>
  ...
  >
{
  /// Creates a new fan-out shape with one inlet and twenty-one outlets.
  #[must_use]
  pub const fn new(
    inlet: Inlet<In>,
    ...
    group4: (Outlet<Out16>, Outlet<Out17>, Outlet<Out18>),
    group5: (Outlet<Out19>, Outlet<Out20>),
  ) -> Self {
    ...
    let (out16, out17, out18) = group4;
    let (out19, out20) = group5;
    Self {
      inlet,
      out0,
      out1,
