...
impl ActorMaterializer {
  ...
  fn drive_streams_until_terminal(streams: &[StreamShared]) -> Result<(), StreamError> {
    ...
    for _ in 0..DIRECT_DRAIN_ROUND_LIMIT {
      ...
      for stream in streams {
        ...
        if matches!(stream.drive(), crate::materialization::DriveOutcome::Progressed) {
          progressed = true;
        }
      }
  ...
  fn prepare_resource_teardown(
    ...
    let mut result = Ok(());
    resources.downstream_cancellation_control_plane.with_locked(|plane| plane.replace_routes(Vec::new()));
  ...
  ///
  /// Used by [`crate::snapshot::MaterializerState::stream_snapshots`] to
  ...
  #[must_use]
  pub(crate) fn streams(&self) -> &[StreamShared] {
    &self.streams
  }
}
