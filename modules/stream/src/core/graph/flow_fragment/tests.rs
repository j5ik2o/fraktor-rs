...
fn flow_fragment_builds_reusable_flow_fragment() {
  let fragment = FlowFragment::from_flow(crate::core::stage::flow::Flow::new().map(|value: u32| value + 1));
...
fn flow_fragment_supports_fan_out_and_fan_in() {
  let fragment = FlowFragment::from_flow(crate::core::stage::flow::Flow::new().map(|value: u32| value + 1_u32))
    .broadcast(2)
    .expect("broadcast")
    .merge(2)
    .expect("merge");
...
fn flow_fragment_rejects_zero_fan_parameters() {
  assert!(
    FlowFragment::from_flow(crate::core::stage::flow::Flow::new().map(|value: u32| value + 1)).broadcast(0).is_err()
  );
  assert!(
    FlowFragment::from_flow(crate::core::stage::flow::Flow::new().map(|value: u32| value + 1)).balance(0).is_err()
  );
  assert!(FlowFragment::from_flow(crate::core::stage::flow::Flow::new().map(|value: u32| value + 1)).merge(0).is_err());
  assert!(
    FlowFragment::from_flow(crate::core::stage::flow::Flow::new().map(|value: u32| value + 1)).concat(0).is_err()
  );
}
