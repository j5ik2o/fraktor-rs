...
#[test]
fn reverse_port_ops_from_source_connects_source_to_inlet() {
...
#[test]
fn reverse_port_ops_from_source_with_flow_inlet() {
  ...
  assert!(result.is_ok());
  // 検証: build が成功してグラフが構築される（build() は Flow を返すため、構築自体が成功の証明）
...
#[test]
fn reverse_port_ops_connect_from_connects_outlet_to_inlet() {
