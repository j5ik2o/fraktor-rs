...
use crate::core::{
  DynValue, SinkDecision, SinkLogic, SourceLogic, StreamError, downcast_value,
  dsl::{Sink, Source},
  ...
  T: Send + Sync + 'static,
{
  ...
  /// Panics when `partition` is out of range.
  #[must_use]
  pub fn source_for(&self, partition: usize) -> Source<T, StreamNotUsed> {
    {
      let mut guard = self.state.lock();
      assert!(partition < guard.active_consumers.len(), "partition index out of range");
      guard.active_consumers[partition] = true;
  ...
  #[must_use]
  pub fn sink_for(&self, partition: usize) -> Sink<T, StreamNotUsed> {
