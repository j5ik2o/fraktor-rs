...
use crate::{
  ...
  behavior::Behavior,
  delivery::{
    ConsumerControllerCommand, DurableProducerQueueCommand, DurableProducerQueueState, MessageSent, ProducerController,
...
pub(crate) enum WppcDeferredAction<A>
where
...
#[derive(Clone, Copy)]
pub(crate) enum WppcDurableQueueTimeout {
  Load { attempt: u32 },
  Store { seq_nr: u64, attempt: u32 },
}

#[derive(Clone)]
pub(crate) enum BufferedWork<A>
where
  A: Clone + Send + Sync + 'static, {
...
pub(crate) struct PendingDurableStore<A>
where
  A: Clone + Send + Sync + 'static, {
  pub(crate) message:                A,
  pub(crate) worker_key:             u64,
  pub(crate) worker_local_seq_nr:    u64,
  pub(crate) confirmation_qualifier: String,
  pub(crate) replay_confirmation_of: Option<(String, u64)>,
}
...
where
  ...
  pub(crate) pending_stores:    BTreeMap<u64, PendingDurableStore<A>>,
}
...
where
  A: Clone + Send + Sync + 'static,
{
  pub(crate) const fn new(producer_id: String, buffer_size: u32) -> Self {
    Self {
      producer_id,
      current_seq_nr: 1,
      producer: None,
...
/// [`ServiceKey`](crate::receptionist::ServiceKey) and the
...
pub(crate) fn collect_on_worker_listing<A>(
...
pub(crate) fn collect_on_internal_demand<A>(
  state: &mut WorkPullingState<A>,
