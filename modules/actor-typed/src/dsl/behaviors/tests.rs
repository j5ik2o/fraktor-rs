...
fn receive_message_partial_returns_behavior_on_some() {
  ...
  assert!(matches!(result.directive(), crate::behavior::BehaviorDirective::Same));
}
...
fn receive_message_partial_returns_unhandled_on_none() {
  ...
  assert!(matches!(result.directive(), crate::behavior::BehaviorDirective::Unhandled));
}
...
fn receive_partial_handles_message() {
  ...
  assert!(matches!(unhandled.directive(), crate::behavior::BehaviorDirective::Unhandled));
}
...
fn monitor_sends_clone_to_monitor_ref() {
  ...
  let monitor_actor_ref = crate::test_support::actor_ref_with_sender(Pid::new(800, 0), monitor_sender);
...
fn monitor_passes_message_to_inner_behavior() {
  ...
  let monitor_inbox = ArcShared::new(SpinSyncMutex::new(Vec::new()));
  let monitor_sender = RecordingSender::new(monitor_inbox.clone());
  let monitor_actor_ref = crate::test_support::actor_ref_with_sender(Pid::new(801, 0), monitor_sender);
  let monitor_typed_ref = TypedActorRef::<u32>::from_untyped(monitor_actor_ref);
...
fn receive_message_with_same_invokes_handler_and_returns_same() {
  ...
  assert!(matches!(result.directive(), crate::behavior::BehaviorDirective::Same), "should return Same directive");
...
fn stopped_with_post_stop_has_stopped_directive() {
  ...
  assert!(
    matches!(behavior.directive(), crate::behavior::BehaviorDirective::Stopped),
...
fn stopped_with_post_stop_returns_stopped_directive() {
  ...
  assert!(matches!(result.directive(), crate::behavior::BehaviorDirective::Stopped), "should return Stopped directive");
}
