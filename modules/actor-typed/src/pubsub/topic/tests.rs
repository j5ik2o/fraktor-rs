...
fn topic_should_publish_to_subscribers_and_report_stats() {
  ...
  let topic = system.as_untyped().actor_of(topic_props.to_untyped()).expect("spawn topic");
  ...
  let subscriber = system.as_untyped().actor_of(subscriber_props.to_untyped()).expect("spawn subscriber");
