...
fn fatal_failure_stops_child() {
  ...
  let subscriber = subscriber_handle_with_shared_factory(RecordingSubscriber { events: events.clone() },
  );
...
impl Actor for RootGuardian {
  fn receive(&mut self, ctx: &mut ActorContext<'_>, message: AnyMessageView<'_>) -> Result<(), ActorError> {
    if message.downcast_ref::<Start>().is_some() && self.supervisor_slot.lock().is_none() {
      ...
      supervisor.try_tell(AnyMessage::new(Start)).map_err(|error| ActorError::from_send_error(&error))?;
    }
    Ok(())
  }
}
