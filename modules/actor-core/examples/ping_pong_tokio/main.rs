...
use cellactor_actor_core_rs::{
  Actor, ActorContext, ActorError, ActorRef, ActorSystem, AnyMessage, AnyMessageView, DispatcherConfig, NoStdToolbox,
  Props,
};
...
struct GuardianActor {
  dispatcher: DispatcherConfig<NoStdToolbox>,
}

impl GuardianActor {
  fn new(dispatcher: DispatcherConfig<NoStdToolbox>) -> Self {
    Self { dispatcher }
  }

  fn child_props<F, A>(&self, factory: F) -> Props<NoStdToolbox>
  where
    F: Fn() -> A + Send + Sync + 'static,
    A: Actor<NoStdToolbox> + Sync + 'static, {
...
async fn main() {
  ...
  let dispatcher: DispatcherConfig<NoStdToolbox> =
