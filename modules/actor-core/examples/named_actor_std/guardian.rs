use cellactor_actor_core_rs::{Actor, ActorContext, ActorError, AnyMessage, AnyMessageView, Props};
...
impl GuardianActor {
  fn spawn_named_child(&self, ctx: &ActorContext<'_>, name: &str) {
    ...
    match ctx.spawn_child(&props) {
      | Ok(child) => {
        println!("[guardian] 名前付きの子アクターを生成しました name={} pid={}", name, child.pid());
  ...
  fn spawn_anonymous_child(&self, ctx: &ActorContext<'_>) {
    ...
    match ctx.spawn_child(&props) {
      | Ok(child) => {
        println!("[guardian] 匿名の子アクターを生成しました pid={}", child.pid());
