...
fn adapter_entry_type_id_round_trip() {
  let entry = AdapterEntry::<i32>::new::<u32, _>(TypeId::of::<u32>(), |value| Ok(value as i32));
  assert_eq!(entry.type_id(), TypeId::of::<u32>());
}

#[test]
fn adapter_entry_executes_handler() {
  let entry = AdapterEntry::<i32>::new::<String, _>(TypeId::of::<String>(), |value| {
    value.parse::<i32>().map_err(|_| AdapterError::Custom("parse".into()))
  });
  let payload = AdapterPayload::new(String::from("12"));
