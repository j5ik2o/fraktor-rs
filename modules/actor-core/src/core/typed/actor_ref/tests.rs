...
fn path_returns_none_without_system() {
  let actor_ref = ActorRef::new_with_builtin_lock(Pid::new(1, 0), NoOpSender);
...
fn narrow_converts_message_type() {
  let actor_ref = ActorRef::new_with_builtin_lock(Pid::new(2, 0), NoOpSender);
  let typed_ref = TypedActorRef::<u32>::from_untyped(actor_ref);
...
fn unsafe_upcast_converts_message_type() {
  let actor_ref = ActorRef::new_with_builtin_lock(Pid::new(3, 0), NoOpSender);
  let typed_ref = TypedActorRef::<u32>::from_untyped(actor_ref);
  let original_pid = typed_ref.pid();
...
fn narrow_and_unsafe_upcast_round_trip() {
  let actor_ref = ActorRef::new_with_builtin_lock(Pid::new(4, 0), NoOpSender);
  let typed_ref = TypedActorRef::<u32>::from_untyped(actor_ref);
  let original_pid = typed_ref.pid();
...
fn narrow_is_consistent_with_map() {
  let actor_ref = ActorRef::new_with_builtin_lock(Pid::new(5, 0), NoOpSender);
...
fn typed_actor_ref_equality_and_order_are_consistent_by_pid() {
  let left = TypedActorRef::<u32>::from_untyped(ActorRef::new_with_builtin_lock(Pid::new(77, 1), NoOpSender));
  let right = TypedActorRef::<u32>::from_untyped(ActorRef::new_with_builtin_lock(Pid::new(77, 1), AlternateNoOpSender));
