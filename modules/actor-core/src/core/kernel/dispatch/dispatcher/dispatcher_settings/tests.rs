...
fn new_stores_all_fields() {
  let settings = DispatcherSettings::new("test-id", nz(5), Some(Duration::from_millis(50)), Duration::from_secs(2));
...
fn with_throughput_replaces_value() {
  let settings = DispatcherSettings::new("id", nz(1), None, Duration::from_secs(1)).with_throughput(nz(10));
  assert_eq!(settings.throughput(), nz(10));
}

#[test]
fn with_throughput_deadline_replaces_value() {
  let settings = DispatcherSettings::new("id", nz(1), None, Duration::from_secs(1))
...
fn with_shutdown_timeout_replaces_value() {
  let settings =
    DispatcherSettings::new("id", nz(1), None, Duration::from_secs(1)).with_shutdown_timeout(Duration::from_secs(5));
  assert_eq!(settings.shutdown_timeout(), Duration::from_secs(5));
}

#[test]
fn with_defaults_uses_documented_constants() {
  let settings = DispatcherSettings::with_defaults("default-id");
...
fn clone_preserves_all_fields() {
  let settings = DispatcherSettings::new("clone-me", nz(7), Some(Duration::from_millis(100)), Duration::from_secs(4));
