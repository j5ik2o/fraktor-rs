...
fn should_maintain_fifo_among_control_messages() {
  ...
  queue.enqueue(AnyMessage::control(10_u32)).unwrap();
  queue.enqueue(AnyMessage::control(20_u32)).unwrap();
  queue.enqueue(AnyMessage::control(30_u32)).unwrap();
...
fn should_interleave_control_and_normal_correctly() {
  ...
  queue.enqueue(AnyMessage::new(1_u32)).unwrap();
  queue.enqueue(AnyMessage::control(100_u32)).unwrap();
  queue.enqueue(AnyMessage::new(2_u32)).unwrap();
  queue.enqueue(AnyMessage::control(200_u32)).unwrap();
...
fn should_clean_up_both_queues() {
  let queue = UnboundedControlAwareMessageQueue::new();
  queue.enqueue(AnyMessage::new(1_u32)).unwrap();
  queue.enqueue(AnyMessage::control(2_u32)).unwrap();
  queue.enqueue(AnyMessage::new(3_u32)).unwrap();
...
fn should_drain_control_before_normal() {
  ...
  queue.enqueue(AnyMessage::control(1_u32)).unwrap();
  queue.enqueue(AnyMessage::control(2_u32)).unwrap();
  ...
  queue.enqueue(AnyMessage::new(99_u32)).unwrap();
