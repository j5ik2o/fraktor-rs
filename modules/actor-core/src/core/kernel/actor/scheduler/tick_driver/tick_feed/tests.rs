...
use crate::core::kernel::actor::scheduler::tick_driver::{TickDriverKind, TickExecutorSignal, TickFeed};

#[test]
fn enqueue_wakes_signal_and_preserves_order() {
  let signal = TickExecutorSignal::new();
  let feed = TickFeed::new(Duration::from_millis(1), 4, signal.clone());
...
fn snapshot_reports_dropped_ticks() {
  let signal = TickExecutorSignal::new();
  let feed = TickFeed::new(Duration::from_millis(1), 1, signal);
  feed.enqueue(1);
  feed.enqueue(1);
