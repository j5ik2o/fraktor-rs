...
use super::super::{pool::Pool, round_robin_pool::RoundRobinPool, routee::Routee, router_config::RouterConfig};
...
fn new_panics_on_zero_instances() {
  let _pool = RoundRobinPool::new(0);
}

#[test]
fn create_router_returns_functional_router() {
  let pool = RoundRobinPool::new(3);
...
fn with_dispatcher_overrides_default() {
  let pool = RoundRobinPool::new(2).with_dispatcher(String::from("custom-dispatcher"));
