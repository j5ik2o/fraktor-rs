...
impl ActorSystem {
  ...
  pub fn new(guardian_props: Props) -> Result<Self, ActorError> {
    ...
    let guardian_pid;
    {
      ...
      guardian_pid = guard.spawn_actor(&state, None, None, guardian_props)?;
    }
...
pub(crate) fn enqueue_user(handle: &SystemShared, pid: Pid, message: AnyOwnedMessage) -> Result<(), SendError> {
  ...
  let mut guard = handle.lock();
  if matches!(result, Err(SendError::ActorFailure(_))) && !entry.started {
    guard.remove_entry_recursive(entry);
  } else {
...
struct ActorEntry {
  ...
  restart:    crate::RestartStatistics,
  props:      Props,
  started:    bool,
}
...
impl ActorEntry {
  ...
  fn run_until_idle(&mut self) -> Result<(), SendError> {
  ...
  fn handle_failure(&mut self, error: ActorError) -> Result<(), SendError> {
    let directive = self.supervisor.strategy().decide(&error);
    match directive {
      | SupervisorDirective::Resume => Ok(()),
      | SupervisorDirective::Restart => {
        if self.restart.allow_restart() {
          self.restart_actor()?;
          Ok(())
        } else {
          self.stop_actor()?;
