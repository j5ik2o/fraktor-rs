...
impl SerializedMessage {
  ...
  #[must_use]
  pub fn new(serializer_id: SerializerId, manifest: Option<String>, bytes: Vec<u8>) -> Self {
  ...
  pub fn decode(bytes: &[u8]) -> Result<Self, SerializationError> {
    ...
    let serializer_raw = u32::from_le_bytes(bytes[cursor..cursor + 4].try_into().unwrap());
    cursor += 4;
    ...
    let manifest = if has_manifest == 1 {
      ...
      let manifest_len = u32::from_le_bytes(bytes[cursor..cursor + 4].try_into().unwrap()) as usize;
      cursor += 4;
    ...
    let payload_len = u32::from_le_bytes(bytes[cursor..cursor + 4].try_into().unwrap()) as usize;
    cursor += 4;
