...
use crate::core::{
  ClusterIdentity, DeliveryPolicy, PartitionBehavior, PubSubBroker, PubSubEvent, PubSubSubscriber, PubSubTopic,
  PubSubTopicOptions, PublishRejectReason,
};
...
fn rejects_subscription_when_topic_missing() {
  ...
  assert!(matches!(result, Err(crate::core::PubSubError::DuplicateSubscriber { .. })));
