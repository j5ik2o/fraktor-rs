...
use fraktor_actor_rs::core::{
  ...
  scheduler::{SchedulerConfig, SchedulerContextSharedGeneric},
};
...
use crate::core::{
  BatchingProducerConfig, ClusterPubSub, ClusterPubSubShared, ClusterTopology, PubSubBatch, PubSubError,
  PubSubPublisherGeneric, PubSubSubscriber, PubSubTopic, PublishAck, PublishRejectReason, PublishRequest,
};
...
fn flushes_when_batch_size_reached() {
  ...
  let scheduler_context =
    SchedulerContextSharedGeneric::from_config(NoStdToolbox::default(), SchedulerConfig::default());
  system.state().install_scheduler_context(scheduler_context.clone());
  let scheduler = scheduler_context.scheduler();
