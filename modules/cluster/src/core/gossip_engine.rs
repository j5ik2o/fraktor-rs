...
use crate::core::{
  ...
  gossip_outbound::GossipOutbound,
...
impl GossipEngine {
  ...
  pub fn disseminate(&mut self, delta: MembershipDelta) -> Vec<GossipOutbound> {
    let out = self
      ...
      .iter()
      .cloned()
      .map(|peer| GossipOutbound::new(peer, delta.clone()))
      .collect::<Vec<_>>();
  ...
  pub fn request_reconcile(&mut self, peer: &str, _from: MembershipVersion, _to: MembershipVersion) {
    ...
    self.events.push(GossipEvent::ReconcilingRequested { peer: peer.to_string(), local_version: self.table.version() });
  }
  ...
  pub fn apply_incoming(&mut self, delta: MembershipDelta, peer: &str) {
    ...
    if self.peers.iter().all(|p| self.peer_versions.get(p).copied().unwrap_or(MembershipVersion::zero()) == self.inflight_version) {
      self.state = GossipState::Confirmed;
      self.events.push(GossipEvent::Confirmed { version: self.inflight_version });
    } else {
      self.state = GossipState::Diffusing;
