...
fn diffusing_reaches_confirmed_after_all_peers_ack() {
  ...
  let delta = table
    .try_join("node-1".to_string(), "n1:4050".to_string())
    .expect("join succeeds");
  table.drain_events();

  let mut engine = GossipEngine::new(table, vec!["node-2".to_string(), "node-3".to_string()]);

  let outbound = engine.disseminate(delta.clone());
...
fn conflict_moves_engine_to_reconciling_and_emits_event() {
  ...
  let conflict_delta = MembershipDelta::new(
  ...
  assert_eq!(
    events,
    vec![GossipEvent::ConflictDetected {
...
fn missing_range_request_enters_reconciling() {
  ...
  assert_eq!(
    events,
    vec![GossipEvent::ReconcilingRequested { peer: "node-2".to_string(), local_version: MembershipVersion::new(1) }],
  );
}
