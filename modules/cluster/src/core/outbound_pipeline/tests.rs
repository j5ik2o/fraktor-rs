...
fn connected_dispatch_keeps_order_per_pid() {
  ...
  assert_eq!(
    events,
    vec![
      OutboundEvent::Dispatched { pid: "pid-1".to_string() },
      OutboundEvent::Dispatched { pid: "pid-1".to_string() },
    ],
  );
}
...
fn quarantine_blocks_send_and_lifts_after_deadline() {
  ...
  assert_eq!(
    pipeline.state(),
    &OutboundState::Quarantine { reason: "invalid association".to_string(), deadline: Some(30) },
  );
...
fn serialization_failure_is_recorded() {
  ...
  assert_eq!(
    events,
    vec![OutboundEvent::SerializationFailed { pid: "pid-1".to_string(), reason: "encode fail".to_string() }],
  );
}
