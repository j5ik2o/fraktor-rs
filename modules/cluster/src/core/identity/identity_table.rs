...
use super::{identity_event::IdentityEvent, resolve_error::ResolveError, resolve_result::ResolveResult};
use crate::core::membership::{MembershipDelta, MembershipTable, NodeStatus};
...
fn format_actor_path_error(err: &ActorPathError) -> String {
  match err {
    ...
    | ActorPathError::NotRootPath => "not root path".to_string(),
    | ActorPathError::NotChildPath => "not child path".to_string(),
  }
}
