...
fn start_member_publishes_static_topology_to_event_stream() {
  ...
  let provider = InprocSampleProvider::new(event_stream, block_list, "node-a").with_static_topology(static_topology);
...
fn start_client_also_publishes_static_topology() {
  ...
  let provider = InprocSampleProvider::new(event_stream, block_list, "client-a").with_static_topology(static_topology);
...
fn topology_includes_blocked_members_from_block_list_provider() {
  ...
  let provider = InprocSampleProvider::new(event_stream, block_list, "node-main").with_static_topology(static_topology);
...
fn no_topology_published_when_static_topology_not_set() {
  ...
  // 静的トポロジを設定しない
  let provider = InprocSampleProvider::<NoStdToolbox>::new(event_stream, block_list, "node-empty");
...
fn shutdown_succeeds_without_side_effects() {
  ...
  let provider = InprocSampleProvider::<NoStdToolbox>::new(event_stream, block_list, "node-shutdown");
...
fn advertised_address_is_stored_correctly() {
  ...
  let provider = InprocSampleProvider::<NoStdToolbox>::new(event_stream, block_list, "127.0.0.1:8080");
