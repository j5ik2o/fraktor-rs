...
use crate::core::{DownstreamCancelAction, DynValue, FlowLogic, StreamError, downcast_value, dsl::Source};
...
pub(in crate::core) struct SwitchMapLogic<In, Out, Mat2, F> {
  pub(in crate::core) func:         F,
  // 現在実行中のサブストリーム（新要素到着時に上書き＝キャンセル）
  pub(in crate::core) active_inner: Option<SecondarySourceBridge<Out>>,
  pub(in crate::core) _pd:          PhantomData<fn(In) -> (Out, Mat2)>,
}
