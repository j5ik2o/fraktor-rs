...
pub(in crate::core) struct InterpreterSnapshotBuilder<'a> {
...
impl<'a> InterpreterSnapshotBuilder<'a> {
  ...
  #[must_use]
  pub(in crate::core) const fn new(stages: &'a [StageDefinition]) -> Self {
    Self { stages }
  }

  /// Builds a running interpreter snapshot from the current edges.
  #[must_use]
  pub(in crate::core) fn build(&self, edges: &[BufferedEdge]) -> RunningInterpreter {
