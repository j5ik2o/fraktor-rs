use crate::core::{
  ...
  r#impl::{graph_dsl_builder::GraphDslBuilder, reverse_port_ops::ReversePortOps},
...
// --- ReversePortOps::wire_from_source の検証 ---

#[test]
fn reverse_port_ops_wire_from_source_connects_source_to_inlet() -> Result<(), StreamError> {
  ...
  let result = ReversePortOps::new(&sink_in).wire_from_source(Source::single(7_u32), &mut builder);
...
#[test]
fn reverse_port_ops_wire_from_source_with_flow_inlet() -> Result<(), StreamError> {
  ...
  let result = ReversePortOps::new(&flow_in).wire_from_source(Source::single(10_u32), &mut builder);
