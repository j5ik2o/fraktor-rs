...
use crate::core::{DynValue, StreamError, downcast_value, stream_ref::StreamRefSettings};

#[cfg(test)]
mod tests;

pub(in crate::core) const STREAM_REF_SUBSCRIPTION_TIMEOUT_MESSAGE: &str =
...
pub(in crate::core) struct StreamRefHandoff<T> {
...
impl<T> StreamRefHandoff<T> {
  ...
  pub(in crate::core) fn configure_buffer_capacity(&self, buffer_capacity: usize) {
  ...
  pub(in crate::core) fn subscribe(&self) {
  ...
  pub(in crate::core) fn is_subscribed(&self) -> bool {
    let guard = self.inner.lock();
    guard.subscribed
  }

  pub(in crate::core) fn offer(&self, value: T) -> Result<u64, StreamError>
  where
    T: Send + 'static, {
    let mut guard = self.inner.lock();
  ...
  pub(in crate::core) fn complete(&self) -> u64 {
    let mut guard = self.inner.lock();
    let seq_nr = guard.next_out_seq_nr;
  ...
  pub(in crate::core) fn fail(&self, error: StreamError) {
    let mut guard = self.inner.lock();
  ...
  pub(in crate::core) fn close_for_cancel(&self) {
    let mut guard = self.inner.lock();
  ...
  pub(in crate::core) fn poll_or_drain(&self) -> Result<Option<T>, StreamError>
  where
    T: Send + 'static, {
    let mut guard = self.inner.lock();
    if let Some(error) = &guard.failure {
  ...
  pub(in crate::core) fn record_cumulative_demand(&self) -> Result<(), StreamError> {
  ...
  pub(in crate::core) const fn subscription_timeout_error() -> StreamError {
