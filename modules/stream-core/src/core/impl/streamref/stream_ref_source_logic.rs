...
use crate::core::{DynValue, SourceLogic, StreamError, stream_ref::StreamRefSettings};
...
pub(in crate::core) struct StreamRefSourceLogic<T> {
...
impl<T> StreamRefSourceLogic<T> {
  pub(in crate::core) fn awaiting_remote_subscription(handoff: StreamRefHandoff<T>) -> Self {
    Self::new(handoff, StreamRefSourceSubscription::AwaitingRemote)
  }

  pub(in crate::core) fn subscribed(handoff: StreamRefHandoff<T>) -> Self {
