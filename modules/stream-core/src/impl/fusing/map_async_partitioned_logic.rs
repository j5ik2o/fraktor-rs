...
use crate::{DynValue, FlowLogic, StreamError, downcast_value};
...
pub(crate) struct MapAsyncPartitionedLogic<In, Out, P, Partitioner, F, Fut>
where
  Fut: Future<Output = Out> + Send + 'static, {
...
where
  ...
  Fut: Future<Output = Out> + Send + 'static,
{
  pub(crate) fn new(partitioner: Partitioner, func: F, parallelism: usize, ordered: bool) -> Self {
    Self {
      partitioner,
      func,
      parallelism,
