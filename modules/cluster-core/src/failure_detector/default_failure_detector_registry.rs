...
use crate::failure_detector::{failure_detector::FailureDetector, failure_detector_registry::FailureDetectorRegistry};

/// Default registry that creates failure detectors on demand via a factory closure.
///
/// Each resource key maps to its own [`FailureDetector`] instance created by
/// the factory provided at construction time.
