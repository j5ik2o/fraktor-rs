//! `FailureDetector` trait — single-resource availability tracker.
...
/// of the monitored resource. Each implementation monitors exactly one
/// resource; use [`super::FailureDetectorRegistry`] for multi-resource
/// management.
pub trait FailureDetector {
  /// Returns `true` if the resource is considered up and healthy.
  ///
  /// For detectors that have not yet received any heartbeat, this returns
  /// `true` (unmanaged connections are treated as healthy).
