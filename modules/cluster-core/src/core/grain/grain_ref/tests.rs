...
fn request_retries_on_timeout_until_policy_exhausted() {
  ...
  assert_eq!(ask_error, fraktor_actor_core_rs::actor::messaging::AskError::Timeout);
...
fn request_with_sender_forward_failure_completes_error_and_emits_event() {
  ...
  assert!(matches!(ask_error, fraktor_actor_core_rs::actor::messaging::AskError::SendFailed(_)));
