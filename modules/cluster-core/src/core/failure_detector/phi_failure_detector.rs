//! Simple phi failure detector ported from the legacy `fraktor-remote-rs`.
//!
//! This is the **legacy** simple `elapsed/mean` heuristic, kept here as the
//! detector implementation cluster tests rely on. Production code that needs
//! the proper Pekko-style Phi Accrual algorithm should use
//! `fraktor_remote_core_rs::failure_detector::PhiAccrualFailureDetector`
//! instead.
...
/// Phi failure detector monitoring a single resource.
///
/// Implements the legacy simple heuristic from `fraktor-remote-rs`: the
/// suspicion level grows as the silence interval diverges from the observed
/// heartbeat mean.
