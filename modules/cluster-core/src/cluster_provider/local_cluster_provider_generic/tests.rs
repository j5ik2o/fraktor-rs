...
use super::*;
use crate::{BlockListProvider, ClusterEvent, ClusterTopology, cluster_provider::ClusterProvider};
...
fn start_member_with_static_topology_publishes_it() {
  ...
  assert!(events.iter().any(|e| matches!(
    ...
    if *mode == crate::StartupMode::Member
  )));
}
...
fn start_client_with_static_topology_publishes_it() {
  ...
  assert!(events.iter().any(|e| matches!(
    e,
    ClusterEvent::Startup { mode, .. }
    if *mode == crate::StartupMode::Client
  )));
}
...
fn start_member_publishes_startup_event_to_event_stream() {
  ...
  assert!(events.iter().any(|e| matches!(
    ...
    if address == "node-a:8080" && *mode == crate::StartupMode::Member
  )));
}
