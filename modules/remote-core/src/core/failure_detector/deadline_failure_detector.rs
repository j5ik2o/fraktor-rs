...
impl DeadlineFailureDetector {
  ...
  #[must_use]
  pub const fn new(heartbeat_interval_ms: NonZeroU64, acceptable_heartbeat_pause_ms: u64) -> Self {
    Self { heartbeat_interval_ms, acceptable_heartbeat_pause_ms, last_heartbeat_ms: None }
  }
  ...
  pub const fn is_available(&self, now_ms: u64) -> bool {
    ...
    let deadline_ms = last_heartbeat_ms.saturating_add(self.deadline_ms());
    deadline_ms > now_ms
  }
  ...
  const fn deadline_ms(&self) -> u64 {
    self.heartbeat_interval_ms.get().saturating_add(self.acceptable_heartbeat_pause_ms)
  }
}
