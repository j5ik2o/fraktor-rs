...
use crate::provider::{provider_error::ProviderError, remote_actor_ref::RemoteActorRef};
...
pub trait RemoteActorRefProvider {
  ...
  ///   [`crate::provider::resolve_remote_address`] and either forwards to the actor-core local
  ///   provider (for local paths) or to this method (for remote paths).
  ///
  /// # Errors
  ///
