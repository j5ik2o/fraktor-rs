use crate::activation::PartitionIdentityLookupConfig;
...
fn getters_return_correct_values() {
  ...
  let config = PartitionIdentityLookupConfig::new(100, 50);
...
fn zero_values_are_allowed() {
  ...
  let config = PartitionIdentityLookupConfig::new(0, 0);
...
fn max_values_are_allowed() {
  ...
  let config = PartitionIdentityLookupConfig::new(usize::MAX, u64::MAX);
