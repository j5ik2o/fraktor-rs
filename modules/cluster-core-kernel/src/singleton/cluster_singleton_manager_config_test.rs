...
use super::ClusterSingletonManagerConfig;
use crate::singleton::{ClusterSingletonConfigError, LeaseUsageConfig};
...
fn default_settings_have_pekko_compatible_defaults() {
  ...
  assert_eq!(s.lease_config(), None);
}

#[test]
fn default_trait_delegates_to_new() {
  let a = ClusterSingletonManagerConfig::new();
  let b = ClusterSingletonManagerConfig::default();
...
fn removal_margin_none_is_distinct_from_explicit_zero() {
  let no_margin = ClusterSingletonManagerConfig::new();
  let explicit_zero = ClusterSingletonManagerConfig::new().with_removal_margin(Duration::ZERO);
...
fn builder_setters_update_fields() {
  ...
  assert_eq!(s.lease_config(), Some(&lease));
}
...
fn validate_rejects_empty_singleton_name() {
  let result = ClusterSingletonManagerConfig::new().with_singleton_name("").validate();

  assert_eq!(result, Err(ClusterSingletonConfigError::EmptySingletonName));
}

#[test]
fn validate_rejects_zero_hand_over_retry_interval() {
  let result = ClusterSingletonManagerConfig::new().with_hand_over_retry_interval(Duration::ZERO).validate();

  assert_eq!(result, Err(ClusterSingletonConfigError::NonPositiveHandOverRetryInterval));
}

#[test]
fn validate_delegates_to_lease_config_when_present() {
  let bad_lease = LeaseUsageConfig::new("", Duration::from_secs(1));
  let result = ClusterSingletonManagerConfig::new().with_lease_config(bad_lease).validate();

  assert_eq!(result, Err(ClusterSingletonConfigError::EmptyLeaseImplementation));
}

#[test]
fn validate_passes_for_default_settings() {
  assert_eq!(ClusterSingletonManagerConfig::new().validate(), Ok(()));
}
...
fn difference_field_names_returns_single_changed_field() {
  let base = ClusterSingletonManagerConfig::new();
  let changed = ClusterSingletonManagerConfig::new().with_singleton_name("other");
...
fn difference_field_names_returns_all_changed_fields() {
  let lease = LeaseUsageConfig::new("impl", Duration::from_secs(1));
  let base = ClusterSingletonManagerConfig::new();
  let changed = ClusterSingletonManagerConfig::new()
