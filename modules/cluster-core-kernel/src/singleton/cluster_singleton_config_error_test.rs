...
fn display_empty_singleton_name_contains_cause() {
  let err = ClusterSingletonConfigError::EmptySingletonName;
...
fn display_buffer_size_out_of_range_contains_cause_and_value() {
  let err = ClusterSingletonConfigError::BufferSizeOutOfRange { value: 99999 };
  let msg = alloc::format!("{err}");
...
fn display_non_positive_hand_over_retry_interval_contains_cause() {
  let err = ClusterSingletonConfigError::NonPositiveHandOverRetryInterval;
  let msg = alloc::format!("{err}");
...
fn display_non_positive_identification_interval_contains_cause() {
  let err = ClusterSingletonConfigError::NonPositiveIdentificationInterval;
  let msg = alloc::format!("{err}");
...
fn display_empty_lease_implementation_contains_cause() {
  let err = ClusterSingletonConfigError::EmptyLeaseImplementation;
  let msg = alloc::format!("{err}");
...
fn display_non_positive_lease_retry_interval_contains_cause() {
  let err = ClusterSingletonConfigError::NonPositiveLeaseRetryInterval;
  let msg = alloc::format!("{err}");
