...
#[test]
fn new_keeps_value_timestamp_and_writer() {
  let node = self_address(0);
  let register = LWWRegister::new(&node, "alpha", 10);
...
fn with_value_replaces_value_writer_and_timestamp() {
  ...
  let register = LWWRegister::new(&first, "alpha", 10);
  let updated = register.with_value(&second, "beta", 11);
...
fn with_value_with_clock_uses_current_timestamp() {
  ...
  let second = self_address(1);
  let register = LWWRegister::new(&first, "alpha", 10);
  let updated = register.with_value_with_clock(&second, "beta", |current, value| {
...
fn merge_picks_larger_timestamp() {
  let first = LWWRegister::new(&self_address(0), "alpha", 10);
  let second = LWWRegister::new(&self_address(1), "beta", 11);
...
fn merge_picks_lowest_node_order_when_timestamps_match() {
  let lower_node = LWWRegister::new(&self_address(0), "alpha", 10);
  let higher_node = LWWRegister::new(&self_address(1), "beta", 10);
...
fn merge_can_model_first_write_wins_with_descending_timestamps() {
  let node = self_address(0);
  let first = LWWRegister::new(&node, "alpha", 100);
  let later_candidate = first.with_value_with_clock(&node, "beta", |current, _| current - 1);
