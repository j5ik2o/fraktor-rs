...
#[test]
fn split_brain_resolver_config_preserve_active_strategy_and_durations() {
...
fn downing_provider_compatibility_preserves_provider_key() {
  ...
  assert!(compatibility.split_brain_resolver_config().is_none());
  assert!(compatibility.sbr_config_identity().is_none());
}

#[test]
fn downing_provider_compatibility_preserves_split_brain_resolver_config() {
  let settings = SplitBrainResolverConfig::new(
    Duration::from_secs(20),
    SplitBrainResolverStrategy::KeepMajority,
    Duration::from_secs(15),
  ...
  let compatibility =
    DowningProviderCompatibility::new("split-brain-resolver").with_split_brain_resolver_config(settings);
...
#[test]
fn split_brain_resolver_config_identity_uses_strategy_identifier_and_durations() {
  let settings = SplitBrainResolverConfig::new(
