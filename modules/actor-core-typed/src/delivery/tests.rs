...
use fraktor_actor_core_rs::core::kernel::actor::{Pid, setup::ActorSystemConfig};
...
use crate::{
  ...
  delivery::{
    ...
    DurableProducerQueueCommand, DurableProducerQueueState, MessageSent, ProducerController, ProducerControllerCommand,
...
fn producer_start_and_register_consumer_connect() {
  ...
  let pc_cell = system.as_untyped().actor_of(pc_props.to_untyped()).expect("spawn producer controller");
  ...
  let cc_cell = system.as_untyped().actor_of(cc_props.to_untyped()).expect("spawn consumer controller");
  ...
  let producer_cell = system.as_untyped().actor_of(producer_props.to_untyped()).expect("spawn producer");
...
fn consumer_controller_delivers_to_consumer() {
  ...
  let consumer_cell = system.as_untyped().actor_of(consumer_props.to_untyped()).expect("spawn consumer");
...
fn consumer_controller_settings_accessors() {
  use crate::delivery::ConsumerControllerConfig;
...
fn work_pulling_start_and_get_worker_stats() {
  ...
  let wppc_cell =
    system.as_untyped().actor_of(wppc_props.to_untyped()).expect("spawn work-pulling producer controller");
  let mut wppc_ref =
  ...
  let stats_cell = system.as_untyped().actor_of(stats_props.to_untyped()).expect("spawn stats");
...
fn work_pulling_delivers_to_worker_via_receptionist() {
  ...
  let cc_props =
    TypedProps::<ConsumerControllerCommand<u32>>::from_behavior_factory(|| ConsumerController::behavior::<u32>());
  let cc_cell = system.as_untyped().actor_of(cc_props.to_untyped()).expect("spawn cc");
  let mut cc_ref = TypedActorRef::<ConsumerControllerCommand<u32>>::from_untyped(cc_cell.into_actor_ref());
...
fn work_pulling_with_durable_queue_replays_loaded_unconfirmed_messages() {
  ...
  let cc_props =
    TypedProps::<ConsumerControllerCommand<u32>>::from_behavior_factory(|| ConsumerController::behavior::<u32>());
  let cc_cell = system.as_untyped().actor_of(cc_props.to_untyped()).expect("spawn worker consumer controller");
  let mut cc_ref = TypedActorRef::<ConsumerControllerCommand<u32>>::from_untyped(cc_cell.into_actor_ref());
  ...
  let consumer_cell = system.as_untyped().actor_of(consumer_props.to_untyped()).expect("spawn worker consumer");
  let consumer_ref = TypedActorRef::<ConsumerControllerDelivery<u32>>::from_untyped(consumer_cell.into_actor_ref());
  cc_ref.tell(ConsumerController::start(consumer_ref));
