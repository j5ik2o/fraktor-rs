...
use super::ConsumerControllerConfig;

#[test]
fn default_settings() {
  let settings = ConsumerControllerConfig::new();
...
fn with_flow_control_window() {
  let settings = ConsumerControllerConfig::new().with_flow_control_window(100);
  assert_eq!(settings.flow_control_window(), 100);
}

#[test]
fn with_only_flow_control() {
  let settings = ConsumerControllerConfig::new().with_only_flow_control(true);
  assert!(settings.only_flow_control());
}
...
fn resend_interval_builders_preserve_other_fields() {
  ...
  let settings = ConsumerControllerConfig::new().with_flow_control_window(200).with_only_flow_control(true);
