...
// `fraktor_actor_adaptor_std_rs::std::system::new_empty_actor_system` (see base/tests.rs). External
  ...
  M: Send + Sync + 'static,
{
  ...
  pub(crate) fn new_empty() -> Self {
    Self::from_untyped(fraktor_actor_adaptor_std_rs::std::system::new_empty_actor_system())
  }
}
...
fn new_test_system() -> TypedActorSystem<u32> {
  ...
  let config = ActorSystemConfig::new(crate::test_support::test_tick_driver()).with_start_time(Duration::from_secs(1));
...
fn name_returns_configured_system_name() {
  ...
  let guardian_props = TypedProps::<u32>::from_behavior_factory(Behaviors::ignore);
  let config = ActorSystemConfig::new(crate::test_support::test_tick_driver()).with_system_name("my-actor-system");
...
fn start_time_returns_configured_value() {
  ...
  let guardian_props = TypedProps::<u32>::from_behavior_factory(Behaviors::ignore);
  let config = ActorSystemConfig::new(crate::test_support::test_tick_driver()).with_start_time(expected_start);
  let system = TypedActorSystem::<u32>::create_from_props(&guardian_props, config).expect("system");
...
fn uptime_returns_elapsed_since_start() {
  ...
  let guardian_props = TypedProps::<u32>::from_behavior_factory(Behaviors::ignore);
  let config = ActorSystemConfig::new(crate::test_support::test_tick_driver()).with_start_time(start);
  let system = TypedActorSystem::<u32>::create_from_props(&guardian_props, config).expect("system");
...
fn address_returns_local_address_with_custom_name() {
  // Given: a typed actor system with a custom name
  let guardian_props = TypedProps::<u32>::from_behavior_factory(Behaviors::ignore);
  let config = ActorSystemConfig::new(crate::test_support::test_tick_driver()).with_system_name("custom-system");
  let system = TypedActorSystem::<u32>::create_from_props(&guardian_props, config).expect("system");
...
fn settings_returns_snapshot_preserved_through_from_untyped() {
  ...
  let untyped = fraktor_actor_adaptor_std_rs::std::system::new_empty_actor_system_with(|config| {
...
fn receptionist_ref_returns_none_when_missing() {
  let system =
    TypedActorSystem::<u32>::from_untyped(fraktor_actor_adaptor_std_rs::std::system::new_empty_actor_system());
...
fn event_stream_returns_typed_actor_ref_that_publishes_events_to_registered_subscribers() {
  ...
  let subscriber = crate::test_support::subscriber_handle(RecordingSubscriber::new(recorded_events.clone()));
  let _subscription = system.subscribe_event_stream(&subscriber);
...
fn event_stream_supports_subscribe_and_unsubscribe_commands() {
  ...
  let recorded_events = ArcShared::new(SpinSyncMutex::new(Vec::new()));
  let collector =
    crate::test_support::actor_ref_with_sender(Pid::new(900, 0), CollectorSender::new(recorded_events.clone()));
...
fn event_stream_subscription_survives_ephemeral_facade_drop_and_shared_unsubscribe_state() {
  ...
  let recorded_events = ArcShared::new(SpinSyncMutex::new(Vec::new()));
  let collector =
    crate::test_support::actor_ref_with_sender(Pid::new(901, 0), CollectorSender::new(recorded_events.clone()));
  let first = EventStreamEvent::Log(LogEvent::new(
    LogLevel::Info,
...
fn event_stream_from_untyped_wrapper_reuses_shared_subscription_state() {
  ...
  let recorded_events = ArcShared::new(SpinSyncMutex::new(Vec::new()));
  let collector =
    crate::test_support::actor_ref_with_sender(Pid::new(902, 0), CollectorSender::new(recorded_events.clone()));
  let first = EventStreamEvent::Log(LogEvent::new(
    LogLevel::Info,
...
fn typed_actor_system_new_rejects_empty_guardian_props() {
  ...
  let result = TypedActorSystem::<u32>::create_from_props(
    &guardian_props,
    ActorSystemConfig::new(crate::test_support::test_tick_driver()),
  );

  // 検証: invalid props として明示的に失敗する
  assert!(matches!(result, Err(fraktor_actor_core_rs::core::kernel::actor::spawn::SpawnError::InvalidProps(_))));
}
