...
fn untyped_recipient_ref_ask_preserves_send_failure_semantics() {
  ...
  assert!(matches!(result, Err(fraktor_actor_core_rs::core::kernel::actor::messaging::AskError::SendFailed(_))));
}
...
fn typed_actor_ref_tell_on_failing_sender_does_not_panic() {
  let mut recipient =
    TypedActorRef::<u32>::from_untyped(crate::test_support::actor_ref_with_sender(Pid::new(77, 1), FailingSender));
