...
fn with_stash_mailbox_sets_stash_requirement() {
  ...
  assert_eq!(
    ...
    fraktor_actor_core_rs::core::kernel::actor::props::MailboxRequirement::for_stash()
  );
}
...
fn with_stash_mailbox_accepts_bounded_mailbox_config() {
  ...
  assert_eq!(props.to_untyped().mailbox_config().validate(), Ok(()));
}
