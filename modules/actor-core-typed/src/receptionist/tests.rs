...
use fraktor_actor_core_kernel_rs::{
  ...
  event::{
    logging::LogLevel,
    stream::{EventStreamEvent, EventStreamSubscriber, EventStreamSubscription},
  },
  system::ActorSystem,
};
use fraktor_utils_core_rs::sync::{ArcShared, DefaultMutex, SharedLock, SpinSyncMutex, shared::Shared};
...
fn typed_system_without_bootstrap_receptionist() -> TypedActorSystem<u32> {
  ...
  TypedActorSystem::<u32>::from_untyped(system)
}

fn new_test_system() -> TypedActorSystem<u32> {
  let guardian_props = TypedProps::<u32>::from_behavior_factory(Behaviors::ignore);
  TypedActorSystem::<u32>::create_from_props(
    &guardian_props,
...
fn unsubscribe_should_stop_listing_updates() {
  ...
  let subscriber = system.as_untyped().actor_of(subscriber_props.to_untyped()).expect("spawn listing subscriber");
  ...
  let routee = system.as_untyped().actor_of(routee_props.to_untyped()).expect("spawn routee");
...
fn register_with_ack_sends_registered_to_reply_to() {
  ...
  let ack_actor = system.as_untyped().actor_of(ack_props.to_untyped()).expect("spawn ack receiver");
...
fn subscribe_returns_error_and_does_not_store_subscriber_when_watch_fails() {
  ...
  let subscriber = TypedActorRef::<Listing>::from_untyped(crate::test_support::actor_ref_with_sender(
    Pid::new(702, 0),
...
fn register_logs_warn_and_preserves_registration_when_notifying_closed_subscriber_fails() {
  ...
  let routee = crate::test_support::actor_ref_with_sender(Pid::new(705, 0), NullSender);
...
fn register_with_ack_logs_warn_and_preserves_registration_when_reply_target_is_closed() {
  ...
  let routee = crate::test_support::actor_ref_with_sender(Pid::new(703, 0), NullSender);
...
fn deregister_with_ack_logs_warn_and_removes_registration_when_reply_target_is_closed() {
  ...
  let routee = crate::test_support::actor_ref_with_sender(Pid::new(704, 0), NullSender);
...
fn get_registers_receptionist_extension_even_when_bootstrap_actor_is_missing() {
  let system = typed_system_without_bootstrap_receptionist();
...
fn create_extension_supports_register_and_find_without_bootstrap_receptionist() {
  ...
  let service =
    system.as_untyped().extended().spawn_system_actor(service_props.to_untyped()).expect("spawn system service");
...
fn get_cleans_up_terminated_routee_without_bootstrap_receptionist() {
  ...
  let routee_props = TypedProps::<u32>::from_behavior_factory(Behaviors::ignore);
  let routee =
    system.as_untyped().extended().spawn_system_actor(routee_props.to_untyped()).expect("spawn system routee");
  let routee_ref = TypedActorRef::<u32>::from_untyped(routee.clone().into_actor_ref());
...
fn get_cleans_up_terminated_subscriber_without_bootstrap_receptionist() {
  ...
  let subscriber =
    system.as_untyped().extended().spawn_system_actor(subscriber_props.to_untyped()).expect("spawn system subscriber");
  let subscriber_ref = TypedActorRef::<Listing>::from_untyped(subscriber.clone().into_actor_ref());
