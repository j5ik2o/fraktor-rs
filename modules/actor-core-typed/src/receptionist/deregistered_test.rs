...
use crate::receptionist::{Deregistered, ServiceKey};
...
fn new_should_store_fields() {
  ...
  let dereg = Deregistered::new("svc", TypeId::of::<u32>(), actor_ref);

  assert_eq!(dereg.service_id(), "svc");
  assert_eq!(dereg.type_id(), TypeId::of::<u32>());
}
...
fn is_for_key_returns_true_for_matching_key() {
  ...
  let dereg = Deregistered::new("svc", TypeId::of::<u32>(), stub_actor_ref(1));

  assert!(dereg.is_for_key(&key));
}
...
fn service_instance_returns_typed_ref_for_matching_key() {
  ...
  let typed_ref = dereg.service_instance(&key).expect("should succeed for matching key");
...
fn service_instance_returns_error_for_mismatched_service_id() {
  ...
  let result = dereg.service_instance(&key);
