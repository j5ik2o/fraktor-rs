...
fn typed_props_empty_supports_immutable_builder_chain() {
  ...
  assert_eq!(
    ...
    fraktor_actor_core_rs::core::kernel::dispatch::mailbox::MailboxCapacity::Unbounded
  );

  assert!(configured.to_untyped().dispatcher_same_as_parent());
  assert_eq!(
    configured.to_untyped().mailbox_policy().capacity(),
    fraktor_actor_core_rs::core::kernel::dispatch::mailbox::MailboxCapacity::Bounded { capacity }
  );
