...
fn group_router_should_route_via_system_receptionist() {
  ...
  .expect("system");
  let router = system.as_untyped().actor_of(router_props.to_untyped()).expect("spawn group router");
...
fn consistent_hash_routing_is_stable_across_routee_order_changes() {
  let routees = vec![
    TypedActorRef::<u32>::from_untyped(crate::test_support::actor_ref_with_sender(
      fraktor_actor_core_rs::core::kernel::actor::Pid::new(11, 0),
      fraktor_actor_core_rs::core::kernel::actor::actor_ref::NullSender,
    )),
    TypedActorRef::<u32>::from_untyped(crate::test_support::actor_ref_with_sender(
      fraktor_actor_core_rs::core::kernel::actor::Pid::new(22, 0),
      fraktor_actor_core_rs::core::kernel::actor::actor_ref::NullSender,
    )),
    TypedActorRef::<u32>::from_untyped(crate::test_support::actor_ref_with_sender(
      fraktor_actor_core_rs::core::kernel::actor::Pid::new(33, 0),
      fraktor_actor_core_rs::core::kernel::actor::actor_ref::NullSender,
    )),
  ];
...
fn group_router_should_route_via_explicit_receptionist() {
  ...
  let receptionist =
    system.as_untyped().actor_of(receptionist_props.to_untyped()).expect("spawn explicit receptionist");
...
fn group_router_should_ignore_mismatched_listing_update() {
  ...
  let mismatched_routee =
    system.as_untyped().actor_of(mismatched_routee_props.to_untyped()).expect("spawn mismatched routee");
...
fn group_router_should_unsubscribe_when_stopped() {
  ...
  let receptionist =
    system.as_untyped().actor_of(receptionist_props.to_untyped()).expect("spawn tracking receptionist");
  let receptionist_ref = TypedActorRef::<ReceptionistCommand>::from_untyped(receptionist.into_actor_ref());
