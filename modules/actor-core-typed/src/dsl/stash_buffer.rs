...
use fraktor_actor_core_rs::core::kernel::actor::{error::ActorError, messaging::AnyMessage};
  ...
  M: Send + Sync + 'static,
{
  ...
  pub fn len(&self, ctx: &TypedActorContext<'_, M>) -> Result<usize, ActorError> {
    Ok(ctx.as_untyped().stashed_messages_snapshot()?.len())
  }
  ...
  pub fn clear(&self, ctx: &TypedActorContext<'_, M>) -> Result<(), ActorError> {
    ctx.as_untyped().clear_stashed_messages().map(|_cleared_count| ())
  }
  ...
  where
    M: Clone,
    F: FnMut(M) -> M, {
    ctx.as_untyped().unstash_with_limit(count, |message| {
      let payload = message
        .payload()
        .downcast_ref::<M>()
        .cloned()
  ...
  where
    ...
    Ok(
      ctx
        ...
        .stashed_messages_snapshot()?
        .iter()
        .filter_map(|message| message.payload().downcast_ref::<M>().cloned())
        .collect::<Vec<M>>(),
    )
  }
}
