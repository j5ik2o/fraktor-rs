...
fn stash_buffer_inspects_and_clears_stashed_messages() {
  ...
  context.with_current_message(AnyMessage::new(1_i32), |context, _message| context.stash().expect("stash first"));
  context.with_current_message(AnyMessage::new(2_i32), |context, _message| context.stash().expect("stash second"));
  ...
  assert_eq!(stash.len(&typed_ctx).expect("len"), 0);
}
...
fn stash_buffer_unstash_requeues_limited_messages_with_wrap() {
  ...
  context.with_current_message(AnyMessage::new(1_i32), |context, _message| context.stash().expect("stash first"));
  context.with_current_message(AnyMessage::new(2_i32), |context, _message| context.stash().expect("stash second"));
  context.with_current_message(AnyMessage::new(3_i32), |context, _message| context.stash().expect("stash third"));
  ...
  assert_eq!(stash.len(&typed_ctx).expect("len"), 1);
