...
fn delegate_returns_delegatee_when_behavior_reports_same() {
  ...
  let actor = system.as_untyped().actor_of(actor_props.to_untyped()).expect("spawn actor");
...
fn ask_sends_request_and_delivers_adapted_response() {
  ...
  let actor = system.as_untyped().actor_of(requester_props.to_untyped()).expect("spawn requester");
...
fn forward_preserves_sender_through_typed_context() {
  ...
  let target_untyped =
    crate::test_support::actor_ref_with_sender(Pid::new(900, 0), CapturingSender { inbox: inbox.clone() });
...
fn typed_props_with_tags_are_readable_via_typed_context() {
  ...
  use fraktor_actor_core_rs::core::kernel::actor::{ActorCell, ActorContext};
...
fn typed_context_system_reuses_shared_event_stream_endpoint() {
  ...
  let collector =
    crate::test_support::actor_ref_with_sender(Pid::new(903, 0), EventCollector::new(recorded_events.clone()));
...
fn spawn_anonymous_creates_child_actor_that_receives_messages() {
  ...
  let _actor = system.as_untyped().actor_of(parent_props.to_untyped()).expect("spawn parent");
  wait_until(|| *received.lock() == 42);
...
fn spawn_anonymous_child_restarts_under_supervision() {
  ...
  let parent = system.as_untyped().actor_of(parent_props.to_untyped()).expect("spawn parent");
...
fn typed_context_set_logger_name_delegates_to_untyped() {
  ...
  let actor = system.as_untyped().actor_of(actor_props.to_untyped()).expect("spawn");
...
fn typed_pipe_to_delivers_ok_result_to_external_target() {
  ...
  let receiver_child = system.as_untyped().actor_of(receiver_props.to_untyped()).expect("spawn receiver");
  ...
  let sender_child = system.as_untyped().actor_of(sender_props.to_untyped()).expect("spawn sender");
...
fn typed_pipe_to_drops_message_and_logs_when_map_ok_returns_adapter_error() {
  ...
  let subscriber = crate::test_support::subscriber_handle(RecordingLogSubscriber::new(logs.clone()));
