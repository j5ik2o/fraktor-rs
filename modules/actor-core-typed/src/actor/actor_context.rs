  ...
  M: Send + Sync + 'static,
{
  ...
  where
    C: Send + Sync + 'static, {
    self.inner_mut().system().stop(actor_ref.as_untyped())
  }
  ...
  /// typed [`TypedChildRef`](crate::actor::child_ref::TypedChildRef).
  #[must_use]
  ...
  ///
  /// When the delegated behavior returns [`crate::dsl::Behaviors::same`] or
  /// [`crate::dsl::Behaviors::unhandled`], the delegated behavior itself becomes
  ...
  pub(crate) const fn as_untyped(&self) -> &ActorContext<'a> {
    self.inner()
  }
