use fraktor_actor_core_rs::core::kernel::event::logging::LogLevel;
...
fn log_options_builder_overrides_fields() {
  let options = LogOptions::new().with_enabled(false).with_level(LogLevel::Info).with_logger_name("typed.behaviors");
  assert!(!options.enabled());
