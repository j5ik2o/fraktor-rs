...
fn down_rejects_self_authority() {
  ...
  assert!(
    matches!(result, Err(fraktor_cluster_core_rs::core::ClusterProviderError::DownFailed(reason)) if reason == "cannot down self authority")
  );
}
...
fn join_is_explicitly_unsupported() {
  ...
  assert!(
    matches!(result, Err(fraktor_cluster_core_rs::core::ClusterProviderError::JoinFailed(reason)) if reason == "join is not supported by aws ecs provider")
  );
}
...
fn leave_is_explicitly_unsupported() {
  ...
  assert!(
    matches!(result, Err(fraktor_cluster_core_rs::core::ClusterProviderError::LeaveFailed(reason)) if reason == "leave is not supported by aws ecs provider")
  );
}
