...
fn open_rejects_calls() {
  ...
  let mut cb = CircuitBreaker::new_with_clock(1, Duration::from_secs(10), clock_fn);
  cb.record_failure();
  assert_eq!(cb.state(), CircuitBreakerState::Open);
...
fn open_transitions_to_half_open_after_reset_timeout() {
  ...
  let mut cb = CircuitBreaker::new_with_clock(1, Duration::from_millis(10), clock_fn);
  cb.record_failure();
  assert_eq!(cb.state(), CircuitBreakerState::Open);
...
fn open_remains_open_before_reset_timeout() {
  ...
  let mut cb = CircuitBreaker::new_with_clock(1, Duration::from_millis(100), clock_fn);
  cb.record_failure();
  assert_eq!(cb.state(), CircuitBreakerState::Open);
...
fn recovery_cycle_closed_open_half_open_closed() {
  ...
  let mut cb = CircuitBreaker::new_with_clock(2, Duration::from_millis(10), clock_fn);
