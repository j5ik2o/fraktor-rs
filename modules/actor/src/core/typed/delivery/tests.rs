...
use crate::core::typed::{
  ...
  delivery::{
    ...
    DurableProducerQueueCommand, DurableProducerQueueState, ProducerController, ProducerControllerCommand,
...
) -> crate::core::typed::Behavior<DurableProducerQueueCommand<A>>
where
...
fn durable_queue_probe_behavior_with_pid<A>(
  events: ArcShared<NoStdMutex<Vec<&'static str>>>,
  durable_pid: ArcShared<NoStdMutex<Option<crate::core::kernel::actor::Pid>>>,
) -> crate::core::typed::Behavior<DurableProducerQueueCommand<A>>
where
  A: Clone + Send + Sync + 'static, {
...
fn work_pulling_with_durable_queue_replays_loaded_unconfirmed_messages() {
  ...
  let durable_events = ArcShared::new(NoStdMutex::new(Vec::<&'static str>::new()));
  let delivered = ArcShared::new(NoStdMutex::new(Vec::<u32>::new()));
  let loaded_state = DurableProducerQueueState::empty()
    .add_message_sent(crate::core::typed::delivery::MessageSent::new(1, 99_u32, false, "legacy-worker".to_string(), 0));
