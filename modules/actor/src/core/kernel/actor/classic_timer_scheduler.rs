...
impl ClassicTimerScheduler {
  ...
  fn with_cell<R>(&self, f: impl FnOnce(&crate::core::kernel::actor::ActorCell) -> R) -> R {
    let state = self.system.state();
    let Some(cell) = state.cell(&self.pid) else {
      panic!("classic timer facade requires a registered actor cell");
    };
    f(&cell)
  }
  ...
  pub fn is_timer_active(&self, key: &str) -> bool {
    self.with_cell(|cell| cell.is_timer_active(key))
  }
