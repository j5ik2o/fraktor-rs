...
fn test_e2e_authority_unresolved_deferred_connected_delivery() {
  ...
  // 未解決状態でメッセージを defer
  assert_eq!(registry.state(authority), AuthorityState::Unresolved);
  ...
  // 接続確立
  let deferred = registry.set_connected(authority).expect("should have deferred messages");
  assert_eq!(deferred.len(), 2);
  assert_eq!(registry.state(authority), AuthorityState::Connected);
...
fn test_e2e_authority_quarantine_invalid_association() {
  ...
  // 初期メッセージを defer
  registry.defer_send(authority, AnyMessage::new(1i32)).expect("defer");
  registry.set_connected(authority);

  // InvalidAssociation をトリガー
  registry.handle_invalid_association(authority, 0, Some(Duration::from_secs(300)));
  assert!(matches!(registry.state(authority), AuthorityState::Quarantine { .. }));

  // Quarantine 中は新規メッセージが拒否される
  let result = registry.defer_send(authority, AnyMessage::new(2i32));
...
fn test_e2e_actor_selection_remote_authority_state_sequence() {
  ...
  assert_eq!(registry.deferred_count("remote-node:2552"), 1);

  // 接続後は成功し、deferred キューは flush されない（resolve は状態確認のみ）
  registry.set_connected("remote-node:2552");
  let resolved = ActorSelectionResolver::resolve_relative_with_authority(&base, "worker", &mut registry, None)
  ...
  registry.set_quarantine("remote-node:2552", 0, Some(Duration::from_secs(60)));
  let result = ActorSelectionResolver::resolve_relative_with_authority(
    &base,
    "worker",
    &mut registry,
