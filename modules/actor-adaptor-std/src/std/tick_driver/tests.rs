...
use fraktor_actor_core_rs::core::kernel::{
  actor::scheduler::{
    ...
    tick_driver::{
      ...
      TickDriverBootstrap, TickDriverConfig as CoreTickDriverConfig, TickDriverControl,
      TickDriverControlShared, TickDriverError, TickDriverHandle, TickDriverId, TickDriverKind, TickDriverProvisioningContext, TickExecutorPump,
      TickFeedHandle, next_tick_driver_id,
    },
  },
  event::stream::{
    EventStreamEvent, EventStreamShared, EventStreamSubscriber,
...
async fn tokio_interval_driver_produces_ticks() {
  ...
  let (mut runtime, _) =
    TickDriverBootstrap::provision(&config, &ctx).expect("runtime");
