...
use crate::actor::{
  ...
  props::Props,
  supervision::{
    BackoffOnStopOptions, BackoffSupervisorStrategy, SupervisorDirective, SupervisorStrategy, SupervisorStrategyKind,
  },
};

struct DummyActor;

impl crate::actor::Actor for DummyActor {
...
fn new_creates_options_with_required_fields() {
  ...
  // When: constructing BackoffOnStopOptions
  let options = BackoffOnStopOptions::new(props, String::from("child-1"), strategy);

  // Then: accessors return the expected values
  assert_eq!(options.child_name(), "child-1");
  assert_eq!(options.strategy().min_backoff(), Duration::from_millis(100));
}
...
fn with_max_retries_sets_value() {
  ...
  let options = options.with_max_retries(5);
...
fn builder_methods_can_be_chained() {
  ...
  let options = BackoffOnStopOptions::new(child_props(), String::from("child"), default_strategy())
    .with_auto_reset(Duration::from_secs(60))
    .with_max_retries(10);

  // Then: all values are set correctly
  assert_eq!(options.auto_reset(), Some(Duration::from_secs(60)));
...
fn strategy_accessor_returns_inner_strategy() {
  ...
  let strategy = BackoffSupervisorStrategy::new(Duration::from_secs(1), Duration::from_secs(60), 0.5);
  let options = BackoffOnStopOptions::new(child_props(), String::from("child"), strategy);

  // Then: the strategy accessor returns the configured values
  assert_eq!(options.strategy().min_backoff(), Duration::from_secs(1));
  assert_eq!(options.strategy().max_backoff(), Duration::from_secs(60));
}

#[test]
fn child_name_accessor_returns_configured_name() {
  // Given: options with a specific child name
  let options = BackoffOnStopOptions::new(child_props(), String::from("my-worker"), default_strategy());

  // Then: child_name returns the configured name
  assert_eq!(options.child_name(), "my-worker");
}
