use crate::{
  ...
  dispatch::mailbox::{
    envelope::Envelope, message_queue::MessageQueue, unbounded_deque_message_queue::UnboundedDequeMessageQueue,
  },
};
...
fn should_prepend_via_enqueue_first() {
  ...
  queue.as_deque().unwrap().enqueue_first(Envelope::new(AnyMessage::new(0_u32))).unwrap();
  ...
  let fourth = queue.dequeue().unwrap().into_payload();
...
fn should_interleave_enqueue_and_enqueue_first() {
  ...
  queue.enqueue(Envelope::new(AnyMessage::new(10_u32))).unwrap();
  queue.enqueue(Envelope::new(AnyMessage::new(20_u32))).unwrap();
  queue.as_deque().unwrap().enqueue_first(Envelope::new(AnyMessage::new(5_u32))).unwrap();
  queue.enqueue(Envelope::new(AnyMessage::new(30_u32))).unwrap();
  queue.as_deque().unwrap().enqueue_first(Envelope::new(AnyMessage::new(1_u32))).unwrap();
