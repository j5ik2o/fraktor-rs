...
use fraktor_cluster_core_kernel_rs::singleton::{
  ClusterSingletonManagerSettings, ClusterSingletonProxySettings, LeaseUsageSettings,
};

use crate::ClusterSingletonSettings;
...
fn default_derives_match_kernel_manager_defaults() {
  ...
  let unified = ClusterSingletonSettings::new();
  let derived_manager = unified.to_manager_settings("singleton");
  let kernel_manager = ClusterSingletonManagerSettings::new();
  ...
  assert_eq!(derived_manager.lease_settings(), kernel_manager.lease_settings());
}
...
fn default_derives_match_kernel_proxy_defaults() {
  ...
  let unified = ClusterSingletonSettings::new();
  let derived_proxy = unified.to_proxy_settings("singleton");
  let kernel_proxy = ClusterSingletonProxySettings::new();
...
fn manager_derivation_is_lossless() {
  ...
  let manager = unified.to_manager_settings("my-singleton");
  ...
  assert_eq!(manager.lease_settings(), Some(&lease));
}
...
fn proxy_derivation_is_lossless() {
  ...
  let proxy = unified.to_proxy_settings("my-proxy");
...
fn proxy_only_fields_do_not_affect_manager_derivation() {
  ...
  let manager = unified.to_manager_settings("test");
  let default_manager = ClusterSingletonManagerSettings::new().with_singleton_name("test");
  ...
  assert_eq!(manager.lease_settings(), default_manager.lease_settings());
}

#[test]
fn manager_only_fields_do_not_affect_proxy_derivation() {
  // removal_margin / hand_over_retry_interval / min_hand_over_retries / lease_settings は proxy
  // に存在しない（要件 3.3）
  let lease = LeaseUsageSettings::new("lease-impl", Duration::from_secs(3));
  let unified = ClusterSingletonSettings::new()
  ...
  let proxy = unified.to_proxy_settings("test");
  let default_proxy = ClusterSingletonProxySettings::new().with_singleton_name("test");
...
fn default_equals_new() {
  let via_new = ClusterSingletonSettings::new();
  let via_default = ClusterSingletonSettings::default();
  // Default が new() を委譲していることを確認
  let m1 = via_new.to_manager_settings("x");
  let m2 = via_default.to_manager_settings("x");
  let p1 = via_new.to_proxy_settings("x");
  let p2 = via_default.to_proxy_settings("x");
  assert_eq!(m1, m2);
  assert_eq!(p1, p2);
}
