...
use fraktor_actor_rs::core::kernel::pattern::{CircuitBreakerState, Clock};

use crate::std::pattern::circuit_breaker;
...
fn open_rejects_calls() {
  ...
  let mut cb =
    fraktor_actor_rs::core::kernel::pattern::CircuitBreaker::new_with_clock(1, Duration::from_secs(10), clock);
  cb.record_failure();
  assert_eq!(cb.state(), CircuitBreakerState::Open);
...
fn open_transitions_to_half_open_after_reset_timeout() {
  let clock = FakeClock::new();
  let mut cb = fraktor_actor_rs::core::kernel::pattern::CircuitBreaker::new_with_clock(
    1,
