...
/// `SharedLock`. The wrapper itself only exposes a `with_write` accessor —
/// there is no read-only path because every meaningful `Association`
/// operation is a state transition.
pub struct AssociationShared {
  inner: SharedLock<Association>,
}
...
impl AssociationShared {
  ...
  pub fn new(association: Association) -> Self {
    Self { inner: SharedLock::new_with_driver::<DefaultMutex<_>>(association) }
  }
