...
use fraktor_remote_core_rs::association::{Association, AssociationEffect};
...
fn apply_one(assoc: &mut Association, effect: AssociationEffect) {
  match effect {
    ...
    | AssociationEffect::PublishLifecycle(event) => {
      // TODO(section-22): publish through the actor-system event stream once
      // StdRemoting wires the lifecycle subscriber.
...
fn log_recursive(effect: AssociationEffect) {
  match effect {
    | AssociationEffect::DiscardEnvelopes { reason, envelopes } => {
      tracing::warn!(
        discarded = envelopes.len(),
