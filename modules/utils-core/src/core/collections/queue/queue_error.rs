...
pub enum QueueError<T> {
  ...
  AllocError(T),
  /// The operation timed out while waiting for capacity. Contains the element that could not be
  /// enqueued.
  TimedOut(T),
}
...
impl<T> QueueError<T> {
  ...
  pub fn into_item(self) -> Option<T> {
    match self {
      | Self::Full(item)
      | Self::OfferError(item)
      | Self::Closed(item)
      | Self::AllocError(item)
      | Self::TimedOut(item) => Some(item),
