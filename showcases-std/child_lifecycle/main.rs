...
fn fussy_worker(crashes_remaining: u32) -> Behavior<ChildCommand> {
  Behaviors::receive_message(move |_ctx, message: &ChildCommand| match message {
    ...
    | ChildCommand::Crash => {
      if crashes_remaining > 0 {
        println!("  [child] クラッシュします (残り {} 回)", crashes_remaining - 1);
...
fn parent() -> Behavior<ParentCommand> {
  ...
  let inner = Behaviors::setup(move |ctx| {
    // 子アクターを監視付きで生成
    let child = ctx.spawn_child_watched(&TypedProps::from_behavior_factory(|| fussy_worker(2))).expect("spawn child");
