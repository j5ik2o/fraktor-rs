...
<!--
[目安 30秒]
本トークでは、Rustで宣言的なストリームDSLを設計するとき、APIの背後にどのような実行系が必要になるかを扱います。題材は開発中のfraktor-rsです。
...
OSSではAmadeus、TAKT、そして本トークの題材であるfraktorを開発しています。
-->
...
coreはホスト環境に依存しない契約と状態機械を持ち、Tokioやネットワークへの接続はstd adaptorへ分離しています。
-->
...
cart_ref.tell(CartCommand::AddItem { qty: 1 });
```
...
<div class="compare-visual">
  <div class="compare-lane">
    ...
    <div>
      ...
      <div class="lane-code">cart_ref.tell(CartCommand::AddItem { qty: 1 })</div>
    </div>
...
StreamはSource、Flow、Sinkとして処理グラフを宣言し、需要量と終端伝播を共通の実行モデルへ任せます。コード例のとおり、Actor側はtellの一行、Stream側はSourceからSinkへ至る一つの宣言として書けます。そして物理実行では、そのグラフをislandへ分け、一つのislandを一つのactorとして動かします。
...
上段が送信経路です。ActorRefへのtellを契機に、MessageDispatcherのdispatchがメッセージをmailboxへenqueueします。actorからdispatcherを呼ぶ流れではありません。
...
次にMaterializerが設計図を実行計画へ変換し、最後にActor Systemが複数のactorとして駆動します。
...
OutやInはステージ間を流れるデータです。一方のMatは、ストリームを実行した結果として外側へ返すハンドルや完了値です。
...
runは実行を開始し、Sink::headのmaterialized valueであるStreamFutureをMaterializedの中へ返します。処理結果の42は、そのfutureの完了を待って初めて得られます。
...
<p class="center muted small" style="margin-top: 22px"><code>into_mat(Sink::head(), KeepRight)</code> = 「Sink 側の <code>StreamFuture</code> を残す」という型レベルの指定</p>
...
KeepLeft、KeepRight、KeepBoth、KeepNoneがその実装で、先ほどのinto_matで渡したKeepRightは、Sink側のStreamFutureを残す指定です。
...
<!--
[目安 1分15秒]
RunnableGraphは、ステージ、接続、属性を並べたStreamPlanと、外へ返すmaterialized valueを保持します。StreamPlanはまだ実行器ではなく、実行前データです。
...
次にasyncの印で、同じactor上で動かすステージのまとまりへ分けます。このまとまりをislandと呼びます。islandをまたぐ接続には有限FIFOを置き、各islandのGraphInterpreterを作って、一つずつactorとして生成します。
...
<!--
[目安 1分20秒]
GraphInterpreterは専用スレッドを占有して回り続けるループではなく、Driveコマンドを受けるたびに少しだけ進む協調的ステートマシンです。
一回のdriveでは、保留中の非同期処理や境界pushを再試行し、初回だけSinkを開始します。demandがあればSourceからpullし、Flowを進め、Sinkへ一要素を渡します。最後に終端条件を確認します。
...
<!--
[目安 1分25秒]
ここでバックプレッシャーを定義します。上流が生成できるだけ送り続けるのではなく、下流が受け取れる要素数をdemandとして上流へ伝えます。
...
実際の分割はMaterializerが設計図を解釈するときまで遅延されます。
-->
...
<!--
[目安 1分30秒]
左は分割前の一本の論理グラフです。filterの後ろにasync boundaryの印があります。
Materializerはそこで辺を切り、上流側をIsland 1、下流側をIsland 2として扱います。各islandは一つのactorになり、境界にはBoundarySink、有限FIFO、BoundarySourceが挿入されます。
...
<!--
[目安 1分10秒]
分割アルゴリズムは、async属性を持つ上流ステージから出る辺を隣接関係へ追加しません。つまり、その辺をグラフの切断点として扱います。
残った辺は無向グラフとして接続し、BFSで連結成分を求めます。一つの連結成分が一つのislandです。
最後に元のトポロジカル順を保ってisland IDを割り当てるため、分割後もデータの向きは失われません。
-->
...
<!--
[目安 1分20秒]
データは上流から下流へ流れますが、キャンセルは逆向きに伝える必要があります。下流のBoundarySourceがキャンセルされると、制御プレーンが上流islandのactorへCancelコマンドを送ります。
配送できれば上流actorが処理を止めます。actorが既に消えているなど配送自体が失敗した場合は、処理を宙ぶらりんにせずkill switchで全islandを失敗させます。
...
<!--
[目安 1分20秒]
物理実行では、一つのislandが一つのStreamIslandActorになります。tickとは、schedulerが既定10ミリ秒間隔で発する前進の合図です。schedulerのtickはChildRefのtry_tellでDriveを送ります。dispatcherがmailboxへenqueueして実行登録し、Mailbox::runがDriveをdequeueしてactorのreceiveへ渡します。
...
<!--
[目安 1分30秒]
map_asyncは、要素ごとにFutureを返す非同期ステージです。そのFutureは、何もしないWakerを使って前ページのDriveのたびにpollします。完了したFutureだけをCompletedへ遷移させる設計です。
...
最初からすべてを共有可変状態にせず、共有範囲を設計上の例外として狭くします。
...
<!--
[目安 1分20秒]
FIFOが満杯ならtry_pushは拒否した値をErr valueとして返し、呼び出し元はpendingとして再試行します。
SharedAccessのwith_writeはFnOnceを受け、任意の戻り値Rを返せます。値をクロージャへmoveし、ResultのErrとして同じ所有権を外へ戻せます。
したがって直接ロックは所有権上の必然ではありません。現在のIslandBoundarySharedはSharedLockへ寄せられるリファクタリング候補で、重要なのは共有API自体をFnOnceとRで設計することです。
-->
...
ActorMaterializerはActor Systemを受け取るだけなので、std環境との差分をstream層へ持ち込まずに済みます。
実装にはGraphDslのfan-inとfan-out、Actor System間をつなぐStreamRef、hub系、restartやthrottleも存在します。
...
三つ目は、no_std自体を実行モデルと呼ぶのではなく、no_stdを境界としてcoreとstd adaptorの責務を分けることです。coreは外部reactorに依存せずtickで前進し、std adaptorがTokioなどの実行環境へ接続します。その選択にはポータビリティと引き換えのレイテンシがあります。
