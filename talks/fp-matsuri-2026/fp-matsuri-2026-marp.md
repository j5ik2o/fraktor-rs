...
# Iterator の合成に、レート差と並行性を足すと？

<div class="compare-visual">
  <div class="compare-lane">
    <h2>Iterator</h2>
    <div class="pipeline">
      <span class="step">next()</span><span class="arrow">→</span>
      <span class="step">map</span><span class="arrow">→</span>
      <span class="step">filter</span><span class="arrow">→</span>
      <span class="step">consumer</span>
    </div>
    <div class="lane-note">pull・同期・1本のループ</div>
  </div>
  <div class="compare-lane hot">
    ...
    <div class="pipeline">
      ...
      <span class="step boundary-step">FIFO</span><span class="arrow">→</span>
      <span class="step">Sink</span>
    </div>
    <div class="lane-note">レート差・fan-out・実行境界</div>
  </div>
</div>

<p class="lead center" style="margin-top: 28px">単一ループの外へ出た瞬間、実行モデルが必要になる。</p>

<!--
[目安 1分10秒]
Iteratorなら、消費側がnextを呼ぶ一つの同期ループでmapやfilterを合成できる。
しかし、生成側と消費側の速度が違う、途中で分岐する、別スレッドで動かす、といった条件が入ると、一つの呼び出しスタックでは表現できない。
そこで必要になるのが、流量、実行単位、完了や失敗の伝播まで含む実行モデルである。
-->
...
<div class="agenda">
  <div class="item">01<b>INTRO</b>問いを立てる</div>
  <div class="item current">02<b>DSL</b>記述する</div>
  <div class="item">03<b>MATERIALIZER</b>解釈する</div>
  <div class="item">04<b>ISLAND</b>並行実行する</div>
  <div class="item">05<b>RUST</b>制約と折り合う</div>
  <div class="item">06<b>TAKEAWAYS</b>持ち帰る</div>
</div>

<!--
[目安 20秒]
最初はDSLの層である。この節では、実行方法をいったん忘れ、利用者が何を記述するのかを見る。
ポイントは、要素の型だけでなく、実行によって得られる値も型として保持することである。
-->
...
<p class="muted"><code>KeepLeft</code> / <code>KeepRight</code> / <code>KeepBoth</code> / <code>KeepNone</code> が、この trait を実装する。</p>

<div class="flow" style="margin-top: 24px">
  <div class="node">Left</div><div class="arrow">＋</div><div class="node">Right</div><div class="arrow">→</div><div class="node hot">Rule::Out</div>
</div>

<!--
[目安 1分20秒]
ステージを合成すると、左右それぞれにmaterialized valueが存在する。どちらを外へ返すかも、暗黙の規則にせず型で選ぶ。
KeepLeft、KeepRight、KeepBoth、KeepNoneは同じtraitを実装し、関連型Outが合成後の結果型を決める。
関数を並べるだけでなく、実行結果の合成規則まで宣言へ含める点が、このDSLの関数型らしい部分である。
-->
...
<div class="two-col" style="margin-top: 28px">
  <div class="panel"><h2>組み立てる</h2><p><code>via</code> / <code>to</code> / <code>into_mat</code></p><p class="muted">まだ何も流れない</p></div>
...
<div class="agenda">
  <div class="item">01<b>INTRO</b>問いを立てる</div>
  <div class="item">02<b>DSL</b>記述する</div>
  <div class="item current">03<b>MATERIALIZER</b>解釈する</div>
  <div class="item">04<b>ISLAND</b>並行実行する</div>
  <div class="item">05<b>RUST</b>制約と折り合う</div>
  <div class="item">06<b>TAKEAWAYS</b>持ち帰る</div>
</div>
...
<!--
[目安 20秒]
ここからは記述された設計図をどう実行可能にするかへ進む。
Materializerは単なるrunメソッドの別名ではなく、論理グラフを物理的な実行単位へ変換する境界である。
-->
...
一要素をSinkへ渡す直前にconsumeで残量を減らす。図のように一件要求し、一件渡せば残量はゼロへ戻る。requestゼロは意味のない要求なのでエラーである。
内部では有限値の飽和加算と無制限要求も扱うが、このスライドで重要なのはrequestとconsumeが対になっている点である。
この小さな状態機械が、グラフ全体の流量制御の基礎になる。
-->
...
<div class="agenda">
  ...
  <div class="item">02<b>DSL</b>記述する</div>
  <div class="item">03<b>MATERIALIZER</b>解釈する</div>
  <div class="item current">04<b>ISLAND</b>並行実行する</div>
  <div class="item">05<b>RUST</b>制約と折り合う</div>
  <div class="item">06<b>TAKEAWAYS</b>持ち帰る</div>
</div>
...
<!--
[目安 20秒]
ここからが本トークの中心である。設計図に付けたasync boundaryが、どのように並行実行単位へ変わるかを見る。
先ほど説明したactor、dispatcher、demandがここで一つにつながる。
-->
...
<div class="state-choice">
  <div class="state open">Open<br><span class="tiny">push / pull 可能</span></div>
  <div class="branch-arrows">↙ ↓ ↘</div>
  <div class="terminal-options">
    <div class="state terminal">Completed</div>
    <div class="state terminal">Failed(error)</div>
    <div class="state terminal">DownstreamCancelled</div>
  </div>
</div>
...
<p class="muted">push / pull と、正常完了・失敗・キャンセルの伝播を同じ境界で扱う。</p>

<!--
[目安 1分10秒]
IslandBoundaryは値のFIFOだけでなく、ライフサイクル状態も保持する。Openの間だけpushを受け付け、Openから完了、失敗、下流キャンセルのいずれか一つへ遷移する。三つの終端状態を順番に通るわけではない。
BoundarySourceはFIFOが空のときにこの状態を確認し、単なる一時的な空とストリーム終端を区別する。
データ経路と終端状態を同じ境界で管理することで、island間の意味をそろえている。
-->
...
<p class="center muted">「データより先に『終わった』が届く」事故を防ぐための順序保証である。</p>
...
```
  ...
  <div>
    <table class="comparison">
      <tr><th>waker 駆動</th><th>tick ポーリング</th></tr>
      <tr><td>低レイテンシ</td><td class="warn">最悪 drive 間隔</td></tr>
      <tr><td>reactor と統合</td><td class="good"><code>no_std</code> を維持</td></tr>
      <tr><td>外部実行系に依存</td><td class="good">決定的な駆動</td></tr>
    </table>
    <p class="muted small">完了検知は最悪10ms遅れる（既定値）。</p>
  </div>
</div>
...
reactorと統合するwaker駆動より完了検知は遅く、既定設定では最悪10ミリ秒待つ。一方で、特定の非同期ランタイムへ依存せずno_stdを維持でき、駆動タイミングも決定的になる。
これは弱点を隠す話ではなく、レイテンシとポータビリティのどちらを選んだかという明示的なトレードオフである。
-->
...
<div class="agenda">
  ...
  <div class="item">03<b>MATERIALIZER</b>解釈する</div>
  <div class="item">04<b>ISLAND</b>並行実行する</div>
  <div class="item current">05<b>RUST</b>制約と折り合う</div>
  <div class="item">06<b>TAKEAWAYS</b>持ち帰る</div>
</div>

<!--
[目安 20秒]
最後に、ここまでの実行モデルがRustの所有権、Send、no_stdという制約からどのような形になったかを見る。
抽象的な原則ではなく、実装上の判断を四点に絞る。
-->
...
# 値を返す API が、共有ラッパーの形を決める

```rust
// IslandBoundary::try_push（満杯判定の抜粋）
...
<div class="flow" style="margin-top: 20px">
  <div class="node">try_push(value)</div><div class="arrow">→</div><div class="node boundary">FIFO full</div><div class="arrow">→</div><div class="node hot">Err(value)</div>
</div>

<p class="muted center small">拒否した値を返すため、<code>IslandBoundaryShared</code> は直接ロックする例外になった。</p>

<!--
[目安 1分20秒]
FIFOが満杯、または境界がOpenでなければ、try_pushは拒否した値をErr valueとして呼び出し元へ返す。値を捨てず、pendingとして再試行するためである。
クロージャ型のSharedAccessでは、ロック内へ渡した値の所有権をそのまま外へ戻すAPIを表現しにくい。
そのためIslandBoundarySharedだけは直接ロックする共有型を使う。所有権要件が同期APIの形を決めた例である。
-->
...
# 深掘りしない機能も、同じ実行基盤に載っている

<div class="metric-row scope-grid" style="grid-template-columns: repeat(4, 1fr); margin-top: 72px">
  <div class="panel center"><h2>GraphDsl</h2><p class="muted small">fan-in / fan-out</p></div>
  <div class="panel center"><h2>StreamRef</h2><p class="muted small">ActorSystem 間参照</p></div>
  <div class="panel center"><h2>hub 系</h2><p class="muted small">broadcast / merge</p></div>
  <div class="panel center"><h2>restart / throttle</h2><p class="muted small">運用上の制御</p></div>
</div>

<p class="lead center" style="margin-top: 60px">今回は、これらを支える「記述・解釈・island」に絞った。</p>

<!--
[目安 40秒]
実装にはGraphDslのfan-inとfan-out、Actor System間をつなぐStreamRef、hub系、restartやthrottleも存在する。
ただし本トークでは個別オペレータの機能紹介へ広げず、それらが共通して載る記述、解釈、islandの基盤に焦点を絞った。
このあと三つの設計判断としてまとめる。
-->
...
<div class="agenda">
  ...
  <div class="item">04<b>ISLAND</b>並行実行する</div>
  <div class="item">05<b>RUST</b>制約と折り合う</div>
  <div class="item current">06<b>TAKEAWAYS</b>持ち帰る</div>
</div>

<!--
[目安 20秒]
まとめである。個別の型名ではなく、別の宣言的DSLや実行系を設計するときにも再利用できる判断を三つ持ち帰ってほしい。
-->
