...
| モジュール | 役割 | 規模 |
|---|---|---|
| `stream-core-kernel` | DSL・実行系（Materializer / GraphInterpreter / island 分割）本体。`#![no_std]` | 実装 約45,300行 / テスト 約42,500行 |
| `stream-core-actor-typed` | typed actor 連携（PubSub の source/sink）。no_std | 約700行 |
| `stream-adaptor-std` | std 依存 I/O（FileIO / StreamConverters 等） | 約1,600行 |

### 実績数値（イントロおよびまとめで使用）

- 公開型 216 個、オペレータ実装（fusing logic）78 種、DSL ファイル 40 本
- テスト行数は実装行数とほぼ 1:1（約4.2万行）
- Pekko（stream + stream-typed、公開型689個）との**固定スコープ比較で 47/50 概念 = 94% カバー**
...
4. no_std を貫くなら、Tokio の waker/reactor に依存しない実行モデルを自作する覚悟が要る（tick 駆動ポーリングというトレードオフ）
5. 所有権が API 設計を規定する（例: 拒否された値の所有権を返す必要から、クロージャ型ロック API を使えなかった箇所がある）

---

## 3. アウトライン（セクション別詳細）

### セクション1: イントロ「表面は簡単、実行が難しい」（6分 / 5枚程度）
...
- **「fraktor-rs とは」を独立した2枚でしっかり説明する**:
  ...
  - 2枚目「全体像と本トークの位置」: `{utils, actor, persistence, remote, cluster, stream}` の6ドメイン × core / adaptor-std の2層構成（図解E）。本トークはこのうち **stream 層**の話。参照実装（`references/pekko`, `references/protoactor-go`）を読みながら Rust イディオムに変換して逆輸入する開発スタイル。実績数値（公開型216、Pekko 固定スコープ94%カバー、テスト約4.2万行）をここで見せる
- 問いの提示: `Iterator` の `map/filter` 合成と何が違うのか？（1枚）
  - Iterator = pull・同期・単一消費者。ここにレート差、fan-in/fan-out、スレッド境界が入った瞬間に単一ループでは済まなくなる
- 本トークの地図: 「DSL（記述）→ Materializer（解釈）→ island（物理実行）」の3層を順に降りていく（図解A）（1枚）

### セクション2: 表面 — 宣言的 DSL の設計（7分 / 6枚程度）
...
### セクション3: 解釈 — Materializer と GraphInterpreter（9分 / 7枚程度）
...
### セクション4: 核心 — async boundary と island 実行モデル（15分 / 13枚程度）※最重量セクション
...
- **正直なトレードオフの開示（本トークのハイライト）**: `map_async` の Future は `noop_waker`（何もしない Waker）で tick ごとにポーリングされる（コード抜粋 #9）。Tokio の waker/reactor と統合しない代わりに no_std を貫ける。完了検知のレイテンシは最悪 drive 間隔（既定10ms）
  - 「async/await の皮を被った同期・協調・固定間隔ポーリング」という設計判断。レイテンシ vs ポータビリティ/決定性

### セクション5: Rust との折り合い — 所有権・Send/Sync・no_std（6分 / 6枚程度）

- プロジェクト全体の内部可変性ポリシー: ロジックは `&mut self` で設計し、共有が必要な箇所だけ `SharedLock`（クロージャ型 API `with_read`/`with_write`、ガードを外に返さない）でラップ
- **所有権が API を規定する実例**: `IslandBoundaryShared` はクロージャ型 API では表現できない。理由は `try_push` が「拒否された値の所有権を呼び出し元に返す」必要があるから（コード抜粋 #6 の rustdoc に明記）。規約からの意図的逸脱をドキュメントで正当化
...
意味: 実行エンジンは Tokio の reactor/waker と統合されていない。Future の完了検知は次の drive tick（既定10ms、設定可能）まで遅れる。これは「no_std ポータビリティと決定性のために、真の waker 駆動を捨てた」意図的なトレードオフ。
