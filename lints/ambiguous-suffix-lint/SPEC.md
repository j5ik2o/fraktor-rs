...
- 曖昧なサフィックス（Manager, Util, Facade, Service, Runtime, Engine）を持つ公開型、モジュール名、ファイル名を検出し、より責務が明確な命名への変更を促す。
- 型名・モジュール名・ファイル名だけで責務・境界・依存方向が推測できる状態を維持する。
...
- 非公開型（`pub` なし）は対象外とする。
- 公開型のみ、サフィックスと完全一致する名前（例: `Service` 単体）は対象外とする。モジュール名・ファイル名は完全一致でも違反とする。
