...
## 概要
- `separate_tests` リンターは、プロダクションコードとテストコードを物理的に分離し、sibling `<module>_test.rs` に集約する運用を強制します。
...
- `tests.rs`、`*_test.rs`、`*_tests.rs`、`tests/ui` 配下、`tests/` ディレクトリ内のファイルは対象外です。
...
## 修正ガイド
1. 対象モジュールと同じディレクトリに `<module>_test.rs` を作成し、インラインの `mod tests` 内容を丸ごと移動する。
2. 親ファイルには `#[cfg(test)] #[path = "<module>_test.rs"] mod tests;` だけを残し、テストから本体へアクセスする際は `use super::*;` を利用する。
3. 個別のテスト関数や `#[cfg(test)]` でガードされたヘルパーもすべて `<module>_test.rs` へ移し、必要に応じてユーティリティモジュールを追加する。
