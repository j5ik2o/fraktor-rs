...
- System/User の 2 本キュー（`async_mpsc` バックエンド）と WaitNode ベースの Block 待機戦略を採用し、Busy wait を禁止する。外部 API は同期呼び出しのまま維持し、Dispatcher 内部で協調ポーリングする軽量 executor を用意して `async` 依存を回避する。  
