...
3. **前提** `std` フィーチャを有効化したホスト環境で Tokio ランタイム（マルチスレッド）が初期化済み、**操作** `modules/actor-core/examples/ping_pong_tokio/main.rs` から TokioExecutor を指定して Ping/Pong サンプルを起動、**結果** メッセージ処理が Tokio タスクとして完了し、標準出力に `reply_to` ベースの応答とスレッド ID が記録される。
...
- **FR-035**: ホスト環境向けの動作検証を目的に、examples 配下で Tokio 依存を許容した `TokioExecutor`（仮称）の実装を提供しなければならない。Core クレートは Tokio を依存に含めず、Executor トレイト境界を介して連携する。TokioExecutor は `std` フィーチャ有効時のみビルドされ、Tokio ランタイム上で ActorSystem を駆動する Ping/Pong サンプルが成功することを確認する。
