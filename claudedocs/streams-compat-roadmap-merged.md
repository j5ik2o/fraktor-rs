...
1. [x] C1 未対応総数を170台にしない（現状169）ことを確認し、各スプリント終了時に減算して更新する
2. [x] C2 全タスク完了時に最終的な互換性受け入れ基準を再チェックする
3. [x] C3 命名規則、lints、`./scripts/ci-check.sh all` を最終合格させる

#### 13.7.x クロージング結果（2026-02-16）

- C1: 5.2 の集計を更新し、`Pekko側アンカー総数: 210、実装済み推定: 210、未対応: 0` を確認
- C2: 「3. 達成条件」1〜4 と「8. テスト要件」1〜8 を再確認し、全項目が充足済みであることを確認
- C3: `./scripts/ci-check.sh all` を実行し最終合格（fmt / dylint / clippy / tests / examples を通過）
