...
`fraktor_remote_core_rs::core::transport::RemoteTransport` trait が定義され、Pekko `RemoteTransport` (Scala abstract class, 113行) のメソッドセットに対応する API を提供する SHALL。
...
`fraktor_remote_core_rs::core::transport::TransportError` enum が定義され、トランスポート操作の失敗カテゴリを網羅する SHALL。
