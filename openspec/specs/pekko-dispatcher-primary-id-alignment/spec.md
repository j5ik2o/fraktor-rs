# pekko-dispatcher-primary-id-alignment Specification

## Purpose

fraktor-rs の dispatcher / mailbox registry の **primary entry id** を Pekko 原典 (`Dispatchers.DefaultDispatcherId` (`Dispatchers.scala:160-164`) / `Mailboxes.DefaultMailboxId` (`Mailboxes.scala:58`)) と整合させる契約。`pekko-dispatcher-alias-chain` が残した DP-M1 (primary id divergence) を閉塞し、fraktor-rs 独自の legacy 短縮表記 `"default"` を alias 含めて完全退役させる。typed facade は kernel primary id 定数を直接参照することで、値の 2 重定義を廃止する。change `pekko-dispatcher-primary-id-alignment` (2026-04-23 archive, PR #1646) で確立。
