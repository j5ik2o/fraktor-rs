# actor-system-construction-boundary Specification

## Purpose
TBD - created by archiving change seal-actor-system-construction-boundary. Update Purpose after archive.
## Requirements
...
`fraktor_actor_adaptor_std_rs::system` は std test helper として `create_noop_actor_system` と
`create_noop_actor_system_with` を提供しなければならない(MUST)。これらの helper は `TestTickDriver` と std
...
- **THEN** `create_noop_actor_system` と `create_noop_actor_system_with` は利用できる
...
- **AND** untyped system handle が必要な tests は `create_noop_actor_system` または `ActorSystem::create_with_noop_guardian`
  を使う
