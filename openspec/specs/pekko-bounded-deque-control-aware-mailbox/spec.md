# pekko-bounded-deque-control-aware-mailbox Specification

## Purpose
Pekko `Mailbox.scala:844` `BoundedDequeBasedMailbox` および `Mailbox.scala:931` `BoundedControlAwareMailbox` と等価な bounded mailbox variant を fraktor-rs に提供する契約。Stash (`needs_deque()`) + Bounded、および ControlAware (`needs_control_aware()`) + Bounded の 2 組合せを valid 化し、それぞれ `BoundedDequeMessageQueue` / `BoundedControlAwareMessageQueue` による capacity + `MailboxOverflowStrategy` 強制と、deque semantics / control 優先 dequeue を保証する。併せて `MailboxConfig::validate()` から `BoundedWithDeque` / `ControlAwareRequiresUnboundedPolicy` の fail-fast 拒否分岐を撤去する (BREAKING: `MailboxConfigError` の 2 variant 削除)。change `pekko-bounded-deque-control-aware-mailbox` (2026-04-23 archive, PR #1642) で確立。
...
- `enqueue_first` (front 挿入) は stash rehydration 用途が中心で、Pekko に明示規定が無い。以下の設計を採用する:
  ...
  - `DropOldest`: **`len >= capacity` なら `Err(SendError::Full(..))` を返し、いずれの既存 entry も evict しない** (設計判断の Rationale は design.md Decision 2-c 参照)。
