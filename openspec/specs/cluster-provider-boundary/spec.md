# cluster-provider-boundary Specification

## Purpose

Cluster provider が membership / lifecycle / discovery input を cluster core へ渡す責務境界を定義する。
...
Local / static provider は provider boundary で membership behavior を定義する SHALL。Local provider は membership を変更する explicit join / leave operation と、core downing decision boundary を通過した down input に対して topology update を publish する MUST。Static provider は start 時に configured static topology だけを publish し、discovery を実行しない MUST。
...
#### Scenario: local explicit leave は left topology を publish する

- **WHEN** local provider が current member authority に対する explicit leave を受け取る
- **THEN** その authority を `left` に含めた topology update を publish する
- **AND** その authority は provider の current member set から削除される

#### Scenario: local down input は core decision 後に left topology を publish する

- **WHEN** local provider が core downing decision boundary を通過した current member authority の down input を受け取る
- **THEN** その authority を `left` に含めた topology update を publish する
- **AND** その authority は provider の current member set から削除される
...
Provider boundary は core downing decision を通過した explicit down または provider-observed departure が member departure input を生成できるようにする SHALL。この capability は failure observation policy、Split Brain Resolver behavior、reachability matrix semantics、rebalance、remembered entity recovery を定義しない MUST。

#### Scenario: core decision 済み explicit down は departure input へ変換される

- **WHEN** core downing decision boundary が current member authority に対する explicit down を許可する
- **THEN** provider-neutral な member departure topology input を生成する
...
- **AND** downing decision model は `failure-downing-minimum` capability で仕様化する
