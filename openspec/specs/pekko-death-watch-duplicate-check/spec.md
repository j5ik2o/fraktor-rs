# pekko-death-watch-duplicate-check Specification

## Purpose
Pekko `dungeon/DeathWatch.scala:36-66, 126-132` の `watch` / `watchWith` + `checkWatchingSame` 相当の duplicate registration 検出を fraktor-rs で保証する。同一 target への異種 watch 登録は `Err(WatchRegistrationError::Duplicate { target, conflict })` で拒否し、silent overwrite を廃止する。`AnyMessage` に `PartialEq` が無いため `watch_with → watch_with` は conservative に全拒否する (Pekko `Some(m1) == Some(m2)` 許容の真部分集合)。併せて rollback パスは `watching` User entry と `watch_with_messages` の両方を巻き戻すことで retry を可能に保つ。change `pekko-death-watch-duplicate-check` (2026-04-23 archive, PR #1638) で確立。
...
送信失敗時の rollback は以下を満たさなければならない (MUST):

- **`watch` の send が非 Closed な `SendError` で失敗した場合**: 直前に追加した `(target, User)` を `watching` から `unregister_watching` で除去してから `Err` を返す。これにより retry 時に `WatchRegistrationKind::Plain` による silent `Ok(())` を避ける。
- **`watch_with` の内部 `self.watch` が `Err` を返した場合**: `watch_with_messages` の entry を `remove_watch_with` で除去し、さらに `watching` 側も `unregister_watching` で巻き戻す。
...
- **AND** 送信失敗時は `watch_with_messages` と `watching` の両 entry を rollback + `WatchRegistrationError::Send(..)` を返す

#### Scenario: watch の send 失敗時は watching を rollback して retry を許容する

- **GIVEN** actor A が actor B に `watch(B)` を呼び、`register_watching` 後の `send_system_message(Watch)` が非 Closed な `SendError` (例: `Full` / `Suspended`) で失敗した
- **WHEN** rollback が実行される
- **THEN** `watching` から `(B.pid(), WatchKind::User)` の entry が除去される
- **AND** `WatchRegistrationError::Send(..)` が返る
- **AND** 後続の `watch(B)` 呼び出しは `WatchRegistrationKind::None` として扱われ、`Watch` system message が再送される (silent `Ok(())` で詰まらない)

#### Scenario: watch_with の rollback は watching と watch_with_messages の両方を除去する

- **GIVEN** actor A が actor B に `watch_with(B, msg)` を呼び、内部の `self.watch` が非 Closed な `SendError` で失敗した
- **WHEN** rollback が実行される
- **THEN** `watch_with_messages` から `(B.pid(), msg)` が除去される
- **AND** `watching` から `(B.pid(), WatchKind::User)` が除去される
- **AND** `WatchRegistrationError::Send(..)` が返る
- **AND** 後続の `watch_with(B, msg2)` 呼び出しは `WatchRegistrationKind::None` として新規登録できる (`WatchConflict::PlainThenWatchWith` で block されない)
