...
- [ ] 1.1 `ActorSystemConfig` と `ActorSystemSetup` に runtime lock provider の設定面を追加し、default config が builtin spin provider を seed する
- [ ] 1.2 actor runtime 用 provider 契約、opaque cell、`MailboxLockSet` を定義し、`RuntimeMutex<T, D>` のような public generic を導入しない方針をコード構造へ反映する
...
- [ ] 2.1 `DefaultDispatcherConfigurator` / `BalancingDispatcherConfigurator` / `PinnedDispatcherConfigurator` が runtime lock provider を束縛するように更新する
...
- [ ] 3.3 `Mailbox::new` / `Mailbox::new_sharing` が `MailboxLockSet` を受け取り、既存 run / enqueue / cleanup semantics を維持したまま hot path lock を provider ベースへ移行する
...
- [ ] 4.1 `utils-adaptor-std` に debug provider helper と std provider helper を追加する
