...
- [x] 1.1.1 `modules/actor-core/src/extension.rs` を作成
  - `Extension<TB>` トレイトの定義
  - `ExtensionId<TB>` トレイトの定義
  - RustDoc コメントの追加

- [x] 1.1.2 `modules/actor-core/src/lib.rs` に Extension モジュールを追加
  - `pub mod extension;` を追加
  - 公開APIとして `Extension` と `ExtensionId` を re-export

- [x] 1.1.3 `ActorSystemGeneric` に Extension 管理機能を追加
...
- [x] 1.1.4 `SystemStateGeneric` に Extension ストレージを追加
...
- [x] 1.2.1 `modules/actor-core/src/serialization/error.rs` を更新
  - 既存の `SerializationError` を Pekko 互換に拡張
  - `Display` トレイトの実装
  - `std::error::Error` トレイト実装（feature "std" のみ）

### 1.3 Serializer API の実装
- [x] 1.3.1 `modules/actor-core/src/serialization/serializer.rs` を作成
...
- [x] 1.3.2 `SerializerHandle` 構造体を実装
...
- [x] 1.4.1 `modules/actor-core/src/serialization/registry.rs` を作成
...
- [x] 2.1.1 `modules/actor-core/src/serialization/extension.rs` を作成
...
- [x] 2.1.2 `Serialization<TB>` に API メソッドを実装
...
- [x] 3.1.1 `modules/actor-core/src/serialization/bincode_serializer.rs` を作成
...
- [x] 3.1.2 `Cargo.toml` に bincode 依存を追加
...
- [x] 3.1.3 フレームワークの組み込みシリアライザとして登録
...
- [x] 4.1.1 `modules/actor-core/src/serialization/mod.rs` を作成
  - 各サブモジュールを宣言
  - 公開APIを re-export

### 4.2 lib.rs への追加
- [x] 4.2.1 `modules/actor-core/src/lib.rs` を編集
  - `pub mod serialization;` を追加（既に存在する場合はスキップ）

## 5. テストの実装

### 5.1 Extension システムのテスト
- [x] 5.1.1 `modules/actor-core/src/extension/tests.rs` を作成
  - Extension の登録・取得のテスト
  - 同じ ExtensionId で複数回登録した場合の動作テスト（同じインスタンスを返す）
  - 異なる ExtensionId の独立性テスト

### 5.2 Serialization Extension のテスト
- [x] 5.2.1 `modules/actor-core/src/serialization/extension/tests.rs` を作成
...
- [x] 5.3.1 `modules/actor-core/src/serialization/bincode_serializer/tests.rs` を作成
...
- [x] 5.4.1 `modules/actor-core/src/serialization/registry/tests.rs` を作成
  - シリアライザの登録・取得テスト
  - 型バインディングのテスト
...
### 6.1 RustDoc の充実
- [x] 6.1.1 すべての公開API に `///` コメントを追加
- [x] 6.1.2 使用例を `/// # Examples` セクションに追加
- [x] 6.1.3 `# Panics`, `# Errors` セクションを必要に応じて追加

### 6.2 モジュールレベルのドキュメント
- [x] 6.2.1 `modules/actor-core/src/extension.rs` にモジュールドキュメントを追加
- [x] 6.2.2 `modules/actor-core/src/serialization/mod.rs` にモジュールドキュメントを追加

## 7. CI / Lint の対応

### 7.1 Clippy / Dylint 対応
- [x] 7.1.1 `./scripts/ci-check.sh all` を実行し、すべてのチェックをパス
- [x] 7.1.2 必要に応じて `#[allow(...)]` を追加（理由をコメントで記載）

### 7.2 フォーマット
- [x] 7.2.1 `cargo +nightly fmt` を実行

### 7.3 テスト実行
- [x] 7.3.1 `cargo test --all-features` を実行し、すべてのテストがパス

### 7.4 no_std ビルド検証
- [x] 7.4.1 `cargo build --target thumbv7em-none-eabihf --no-default-features` を実行し、`alloc` のみで serialization モジュールがリンクできることを確認
- [x] 7.4.2 同ターゲットで `SerializerImpl::deserialize` がすべて `alloc` のみで動作することを確認するための smoke テストを追加
