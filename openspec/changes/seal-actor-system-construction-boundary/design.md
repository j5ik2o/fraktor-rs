...
### Decision 3: std test helper は `new_noop_actor_system*` に置き換える
...
### Decision 4: actor-core-kernel inline tests の helper も bootstrap 経由にする
...
### Decision 5: downstream tests は system 合成ではなく目的別 setup へ移す

**選択:** downstream test の移行方針を 3 種類に分ける。

1. **System handle が欲しいだけ**
   - `fraktor_actor_adaptor_std_rs::system::new_noop_actor_system()` に置換する。
2. **Config を変えたい**
   - `new_noop_actor_system_with(|config| ...)` に置換する。
...
### Decision 6: public surface fixture で削除 API を compile-fail にする
...
### Decision 7: `ActorSystemSetup::into_actor_system_config` は単体で検証する
...
2. actor-adaptor-std helper を `new_noop_actor_system*` へ置き換える。
3. downstream tests を `new_noop_actor_system*` または lower-level state tests へ移す。
4. public surface compile-fail fixture を追加する。
5. `ActorSystemSetup` unit tests を追加する。
6. grep / cargo test / CI で削除名が残っていないことを確認する。
