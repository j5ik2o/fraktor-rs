...
### Decision 4: std test helper は `new_noop_actor_system*` に置き換える
...
3. actor-adaptor-std helper を `new_noop_actor_system*` へ置き換える。
4. downstream tests を `new_noop_actor_system*` または lower-level state tests へ移す。
