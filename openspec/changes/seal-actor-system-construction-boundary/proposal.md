...
### 3. actor-adaptor-std の test helper を bootstrap 経由へ作り直す
...
実体と合わないため、破壊的に `new_noop_actor_system` / `new_noop_actor_system_with` へ置き換える。互換 alias は
残さない。

### 4. テストは synthetic system ではなく目的別 helper へ移す

`ActorSystem::from_state(SystemStateShared::new(SystemState::new()))` で「system が 1 個欲しい」だけのテストは
`new_noop_actor_system` へ移行する。
...
### 5. `ActorSystemSetup::into_actor_system_config` の単体テストを復元する
...
- **`actor-system-construction-boundary`**
  ...
  - public `ActorSystem` constructor は guardian/config/setup を経由して bootstrap を完了する。
