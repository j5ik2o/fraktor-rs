...
- [x] 8.3 レビュー対応: CodeRabbit / Cursor Bugbot の指摘が来た場合は Pekko 互換を崩さない範囲で対応、却下する場合は理由を reply してから resolve — Bugbot 指摘 (BoundedControlAwareMessageQueue 2x 過剰確保) を commit 8b46e16f で修正、CI all green で merge
- [x] 8.4 マージ後、別 PR で change をアーカイブ + main spec を `openspec/specs/pekko-bounded-deque-control-aware-mailbox/spec.md` に sync — 本アーカイブ PR で実施
