...
- fraktor-native envelope wire format に system priority envelope 用の redelivery sequence metadata を追加する。
...
- `remote-core-wire-format`: system priority envelope が ACK/NACK 対象の sequence metadata を運べるようにする。
...
- `modules/remote-core/src/wire/` の envelope redelivery metadata。
