...
| E2E | `modules/actor-core/tests/*_e2e.rs` / user-flow scenario tests | public API だけでユーザー操作に近い流れを検証する | 高 |

Conformance / Regression は独立したピラミッド層ではなく、gap-analysis ID や過去差分を追跡する横断タグとして扱う。ID 付き regression は Unit / Contract / Integration / E2E のいずれかへ配置する。
...
E2E 層は「利用者が public API で行う一連の操作」が観測可能な結果まで到達することを検証する。Integration よりさらに数を絞り、内部型や private hook に触れない。

追加候補:

- classic user flow: system 起動 → named child spawn → tell / ask → watch → stop → terminated / dead letter 観測。
- typed user flow: typed system 起動 → spawn → message adapter → ask / pipeToSelf → stop → signal 観測。
- std adaptor boot flow: std actor system config → dispatcher / mailbox / scheduler / logging の実配線 → graceful terminate。
- path / selection flow: local / authority path の parse → selection → delivery / not-found 観測。

既存の `actor_path_e2e`, `ping_pong`, `supervisor`, `system_lifecycle`, `system_events` は E2E 候補または E2E に近い integration として棚卸し対象にする。Wave 1 では新規 E2E を無理に追加せず、既存の E2E 相当テストと不足 scenario を `docs/plan/actor-test-pyramid.md` に記録する。次 wave では少なくとも 1 件の classic user flow と 1 件の typed user flow を受け入れ条件にする。

### 6. Conformance / Regression 横断タグ
...
### Alternative 3: integration / E2E test を厚くする

却下。actor system 起動テストと E2E scenario は価値が高いが遅く壊れやすい。ピラミッドとしては Unit / Contract を厚くし、Integration は接続確認、E2E は少数の代表 user flow に絞る。
