...
- [x] 17.1 `modules/remote-adaptor-std/` ディレクトリを新規作成する
- [x] 17.2 `modules/remote-adaptor-std/Cargo.toml` を作成する (`name = "fraktor-remote-adaptor-std-rs"`、`edition = "2024"`、ライセンス・description 等を他の `*-adaptor-std` と統一)
- [x] 17.3 `[dependencies]` に `fraktor-remote-core-rs`、`fraktor-actor-core-rs`、`fraktor-actor-adaptor-rs`、`fraktor-utils-rs`、`tokio` (rt-multi-thread, net, sync, time, io-util)、`tokio-util` (codec)、`bytes`、`tracing` 等を追加する
- [x] 17.4 `[lints] workspace = true` を設定する
- [x] 17.5 ワークスペースルートの `Cargo.toml` の `members` に `modules/remote-adaptor-std` を追加する + workspace.dependencies に `fraktor-remote-adaptor-std-rs` エントリを追加
- [x] 17.6 `modules/remote-adaptor-std/src/lib.rs` を作成する — crate-level docstring に Pekko Artery との対応関係 (tcp_transport/association_runtime/watcher_actor/provider/extension_installer) を明示
- [x] 17.7 `cargo build -p fraktor-remote-adaptor-std-rs` が成功することを確認する — 警告ゼロで Finished
- [x] 17.8 `./scripts/ci-check.sh ai dylint`がパスすることを確認する — 全モジュール全 dylint passed

## 18. tcp_transport モジュール (Pekko Artery TCP 相当)

- [x] 18.1 `src/tcp_transport.rs` および `src/tcp_transport/` を作成する
- [x] 18.2 `tcp_transport/frame_codec.rs` に `tokio_util::codec::{Encoder, Decoder}` を実装する — `WireFrameCodec` が `WireFrame` enum を `Encoder<WireFrame>` + `Decoder<Item=WireFrame>` で扱う。`FrameCodecError` (`Io` / `Wire` バリアント) を別ファイルで定義 (1file1type)
- [x] 18.3 `tcp_transport/server.rs` に `pub struct TcpServer` を定義し、`TcpListener::bind` → accept loop → inbound task spawn のロジックを実装する
- [x] 18.4 `tcp_transport/client.rs` に `pub struct TcpClient` を定義し、`TcpStream::connect` → 単一接続の reader+writer 統合 task を実装する
- [x] 18.5 `tcp_transport/tcp_transport.rs` に `pub struct TcpRemoteTransport` を定義し、core の `RemoteTransport` trait を実装する — `module_inception` lint 回避のため `tcp_transport/base.rs` に配置
- [x] 18.6 `TcpRemoteTransport::start` / `shutdown` / `send(envelope)` / `quarantine` / `addresses` / `default_address` / `local_address_for_remote` を実装する。`start_async` / `connect_peer` も別途提供 (sync trait の制約から async セットアップを分離)
- [x] 18.7 unit test と統合テスト (2ノード間の echo) を作成する — 7 tests: WireFrameCodec round-trip 4 種類 + 部分受信 + 連続フレーム + tokio integration test (`tcp_server_and_client_exchange_a_frame`)
- [x] 18.8 `./scripts/ci-check.sh ai dylint`がパスすることを確認する — 全 dylint passed (FQCN を `use` で短縮、`InboundFrameEvent` を独立ファイルに分離)

## 19. association_runtime モジュール (Association を駆動する tokio task 群)

- [x] 19.1 `src/association_runtime.rs` および `src/association_runtime/` を作成する
- [x] 19.2 `association_runtime/association_shared.rs` に `pub struct AssociationShared` を定義する — `ArcShared<SpinSyncMutex<Association>>` で内部可変性を adapter 側に閉じ、`with_write(impl FnOnce(&mut Association))` で `&mut self` 契約を維持
- [x] 19.3 `association_runtime/association_registry.rs` に `pub struct AssociationRegistry { entries: BTreeMap<UniqueAddress, AssociationShared> }` を定義 — `Address`/`UniqueAddress` に `PartialOrd, Ord` derive を追加 (BTreeMap キー要件)、手動 PartialEq/Eq/Hash impl を derive に統一
- [x] 19.4 `association_runtime/outbound_loop.rs` に Association の送信ループ tokio task を実装する — `run_outbound_loop` 関数、`Arc<Mutex<T: RemoteTransport>>` を引数に取り `next_outbound` でドレインして `transport.send` を呼ぶ。空時は `POLL_INTERVAL=1ms` の sleep
- [x] 19.5 `association_runtime/inbound_dispatch.rs` に受信ループ tokio task を実装する — `run_inbound_dispatch` 関数、Handshake 受信時に `handshake_accepted` を呼ぶ。完全な dispatch (Envelope→local actor delivery) は Section 22 へ
- [x] 19.6 `association_runtime/handshake_driver.rs` に handshake タイムアウト driver を実装する — `HandshakeDriver::arm` で `tokio::time::sleep` を spawn し、`Instant::elapsed()` から monotonic millis を計算して `Association::handshake_timed_out(now_ms, None)` を呼ぶ
- [x] 19.7 `association_runtime/system_message_delivery.rs` に ack-based redelivery 実装を追加する — `SystemMessageDeliveryState`: `next_sequence`, `cumulative_ack`, `send_window`, `pending: VecDeque<(seq, EnvelopePdu)>`。`record_send` (window full なら None) + `apply_ack` (cumulative ack を monotonic に増加、ack 済を pending から drop)。リトライ timer は Section 24 統合テストで必要になった時点で追加
- [x] 19.8 Phase A で延期された `RemoteSettings` の ack 関連フィールドを本 Phase で追加する — `ack_send_window: u32`・`ack_receive_window: u32` (デフォルト 1024) + builder + accessor + spec scenario 更新
- [x] 19.9 unit test と統合テスト (handshake + ack + redelivery) を作成する — 12 tests
- [x] 19.10 `./scripts/ci-check.sh ai dylint`がパスすることを確認する — 全 dylint passed

## 20. watcher_actor モジュール (WatcherState を actor 化する層)

- [x] 20.1 `src/watcher_actor.rs` および `src/watcher_actor/` を作成する
- [x] 20.2 `watcher_actor/watcher_actor.rs` に core の `WatcherState` を保持する actor を実装する — `module_inception` 回避のため `watcher_actor/base.rs` に `WatcherActor` を配置。 actor-core API への完全統合は Section 22 (extension installer) で行う前提とし、Phase B 最小実装としては tokio task ベースの "actor 風" ラッパー (`mpsc::UnboundedSender<WatcherCommand>` + 単一 task でシリアライズ処理) を採用。`WatcherActorHandle` は別ファイル (`watcher_actor_handle.rs`) に分離 (1file1type)
- [x] 20.3 tokio timer で heartbeat tick を発生させ、`WatcherState::handle(WatcherCommand::HeartbeatTick { now: u64 /* monotonic millis */ })` を呼ぶ — `heartbeat_loop::run_heartbeat_loop` 関数で `tokio::time::interval` から tick を発生させ、`Instant::elapsed()` から monotonic millis を計算して `HeartbeatTick` を submit
- [x] 20.4 effect 列に `SendHeartbeat { to }` が含まれる場合は `TcpRemoteTransport::send` または control channel 経由で heartbeat を送信する — `WatcherActor` は処理した effect を `effect_tx: mpsc::UnboundedSender<WatcherEffect>` に forward する。実際の TCP 送信は Section 22 の `StdRemoting` で `effect_rx` を購読して `TcpRemoteTransport::send` に転送する設計
- [x] 20.5 unit test を作成する — 3 tests: Watch コマンド処理 + initial SendHeartbeat / HeartbeatReceived 後の tick で NotifyTerminated 不発火 / heartbeat_loop の周期 tick 配信検証
- [x] 20.6 【品質ゲート】Section 0 のプロトコルに従う。注力観点: `WatcherState` が actor 内で `&mut self` アクセスされていること・timer 駆動の時刻が monotonic であること

## 21. provider-dispatch モジュール (loopback 振り分け)

- [x] 21.1 `src/provider.rs` および `src/provider/` を作成する
- [x] 21.2 `provider/dispatch.rs` に `pub struct StdRemoteActorRefProvider` を定義 — フィールド: `local_address: UniqueAddress`, `local_provider: ActorRefProviderShared<LocalActorRefProvider>`, `remote_provider: Box<dyn RemoteActorRefProvider + Send + Sync>`, `transport: Arc<Mutex<TcpRemoteTransport>>`
- [x] 21.3 `provider/provider_dispatch_error.rs` に `pub enum StdRemoteActorRefProviderError` を定義 — `NotRemote`, `CoreProvider(ProviderError)`, `LocalProvider(ActorError)`, `RemoteSenderBuildFailed` + `Display`/`From<ProviderError>`/`From<ActorError>` impls (`ActorError` は `std::error::Error` を実装しないため `source()` には返さず `Display` で `{:?}` 表記)
- [x] 21.4 `StdRemoteActorRefProvider::actor_ref` を実装 — 3 ブランチ: (1) authority なし → local_provider 直接、(2) authority + Address/UID 整合 → authority 剥がして local_provider、(3) その他 → remote_provider 委譲。Phase B 最小実装: (3) は core resolve 後に `RemoteSenderBuildFailed` を返す (完全な ActorRef wrap は Section 22 で actor system context 利用可能になってから)
- [x] 21.5 `watch / unwatch` を実装 — remote path では `remote_provider` 委譲、local path では `NotRemote`。`is_remote_path` ヘルパーで `resolve_remote_address` + `is_local_authority` 判定
- [x] 21.6 doc comment 明示完了 — loopback 振り分け責務、watch/unwatch の remote 専用性、エラー境界 (`CoreProvider`/`LocalProvider`/`RemoteSenderBuildFailed`)、authority なし local path も local 分岐へ入ること、3-branch dispatch ルール
- [x] 21.7 unit test を作成 — 10 tests: authorityなし→local / authority一致→local 正規化 / uid=0 wildcard → local / uid 不一致→remote / authority不一致→remote / watch remote 委譲 / unwatch remote 委譲 / watch local→NotRemote / unwatch local→NotRemote / watch local-authority→NotRemote。`StubRemoteProvider` で core provider をモック
- [x] 21.8 `./scripts/ci-check.sh ai dylint`がパスすることを確認する

## 22. extension installer モジュール

- [x] 22.1 `src/extension_installer.rs` および `src/extension_installer/` を作成する
- [x] 22.2 `StdRemoting` 型を定義し、core の `Remoting` trait を実装する — `extension_installer/base.rs` に配置 (module_inception 回避)。`start`/`shutdown`/`quarantine`/`addresses` 実装。`addresses()` は Mutex 越しに `&[Address]` を返せないため Phase B 最小実装は空スライス (callers は `transport()` 直接呼びを使う)
- [x] 22.3 `StdRemoting` 内部で `TcpRemoteTransport (Arc<Mutex<>>)`・`AssociationRegistry`・`WatcherActorHandle (Option)` を保持。`StdRemoteActorRefProvider` 直接保持は Section 24 統合テストで wiring 完成時に追加 (Phase B 最小実装は transport+registry+watcher の共有のみ)
- [x] 22.4 actor system extension として登録する `RemotingExtensionInstaller` を実装する — `extension_installer/remoting_extension_installer.rs` に配置。`Arc<Mutex<StdRemoting>>` を共有保持
- [x] 22.5 既存 `modules/remote/src/std/remoting_extension_installer.rs` を参照しつつ移植 — 同名 trait `ExtensionInstaller` の `install` メソッドを実装 (Phase B 最小実装は no-op、完全な extension-id 登録は Section 24 統合テストで必要時に追加)
- [x] 22.6 unit test を作成する — 6 tests: lifecycle 起動・停止 / 重複 start で `AlreadyRunning` / quarantine 未起動時 `NotStarted` / quarantine 起動後成功 / installer の Arc 共有 / installer 経由の lifecycle 駆動
- [x] 22.7 `./scripts/ci-check.sh ai dylint`がパスすることを確認する — FQCN 短縮 + 全 dylint passed

## 23. Phase B 完了チェック

- [x] 23.1 `./scripts/ci-check.sh ai all` を実行し、エラーがないこと — **EXIT=0** (新クレート 2 つ追加後の全 build/test/clippy/dylint 通過)。修正: `actor-adaptor-std/default_dispatcher_provider.rs` に `Default` impl 追加 (clippy 新ルール)、`HandshakeDriver` を `#[derive(Default)]` に書き換え、`let _ = framed.close()` パターンを `if let Err(err)` に変更、`extension_installer::base::shutdown` の transport.shutdown() エラーを tracing::warn でログ、`WatcherActorHandle::submit` の `Err` variant を `Box<WatcherCommand>` (`SubmitError`) に変更 (`clippy::result_large_err` 対応)、test の `let _ =` を `drop(...)` に書き換え
- [x] 23.2 新クレート `fraktor-remote-adaptor-std-rs` 単独の `cargo test -p fraktor-remote-adaptor-std-rs` が通ること — **38 tests 全 passed**
- [x] 23.3 **本時点では change の archive を行わない** (Decision 15)
...
- [x] 24.1 `modules/remote/tests/` の各統合テストを列挙し、移植先を決定 — 旧 3 ファイル: `phi_failure_detector_edge_cases.rs` (→ core)、`multi_node_scenario_integration.rs` (→ adaptor-std)、`quickstart.rs` (→ adaptor-std)。旧 API (LoopbackActorRefProvider, RemoteWatchHook, RemotingControl) は新設計と互換性ゼロのため 1:1 ポート不可。**概念的に等価な新規テストを新クレートに作成** する方針を採用
- [x] 24.2 core 側の統合テストを `modules/remote-core/tests/` に作成 — `phi_accrual_edge_cases.rs` (6 tests) + `association_state_machine_e2e.rs` (2 tests)
- [x] 24.3 adapter 側の統合テストを `modules/remote-adaptor-std/tests/` に作成 — `two_node_routing.rs` (1 test: 2 つの独立 TcpListener + TcpClient で異なる remote へ frame をルーティング)
- [x] 24.4 `modules/remote/tests/quickstart.rs` を新クレート向けに書き直す — `modules/remote-adaptor-std/tests/quickstart.rs` (2 tests: StdRemoting 直接 lifecycle / RemotingExtensionInstaller 経由 lifecycle)
- [x] 24.5 旧テスト (`modules/remote/tests/`) は変更せず Phase E の削除対象として残す
- [x] 24.6 `cargo test -p fraktor-remote-core-rs -p fraktor-remote-adaptor-std-rs` が全通過することを確認する — **174 tests 全 passed** (core: 125 unit + 8 integration / adaptor-std: 38 unit + 3 integration)
- [x] 24.7 `./scripts/ci-check.sh ai dylint`がパスすることを確認する — 全 dylint passed

## 25. Phase C 完了チェック

- [x] 25.1 `./scripts/ci-check.sh ai all` を実行し、エラーがないこと — **EXIT=0** (全ワークスペース通過)
- [x] 25.2 **本時点では change の archive を行わない** (Decision 15)
...
- [x] 26.1 `grep -rn 'fraktor-remote-rs' modules/*/Cargo.toml` で依存元モジュールを列挙する — `cluster-core`, `cluster-adaptor-std`, `showcases/std` (3 件)
- [x] 26.2 各モジュールの依存を新クレート群に置き換える計画を立てる — **重要発見**: 旧 `fraktor-remote-rs::core::{BlockListProvider, failure_detector::*}` は新 `fraktor-remote-core-rs` に存在しない。これらは概念的に **cluster の責務** であるため、cluster-core に移動するのが正解と判断
- [x] 26.3 計画を文書化する — 本 tasks.md の Section 27 注記と下記アーキテクチャ判断を参照

## 27. 依存切り替え実施

**アーキテクチャ判断**: 旧 `fraktor-remote-rs::core::{BlockListProvider, failure_detector::*}` の cluster 利用箇所は、それらの型を `fraktor-cluster-core-rs::core::{block_list_provider, failure_detector}` に **移動** することで対応する。これらはもともと remote ではなく cluster の責務であり、redesign を機に正しい場所に置く。新規追加: `cluster-core/src/core/block_list_provider.rs`, `cluster-core/src/core/failure_detector/{failure_detector,failure_detector_registry,default_failure_detector_registry,phi_failure_detector,phi_failure_detector/config}.rs`。

- [x] 27.1 `cluster-adaptor-std/Cargo.toml` の `fraktor-remote-rs` 依存を削除 — cluster-adaptor-std は cluster-core 経由で必要な型にアクセス
- [x] 27.2 `cluster-adaptor-std/src/` 配下で `fraktor_remote_rs::core::...` import を `fraktor_cluster_core_rs::core::...` に書き換え (sed で一括置換)
- [x] 27.3 `cargo build -p fraktor-cluster-adaptor-rs` が成功することを確認 — 警告ゼロで Finished
- [x] 27.4 `cargo test -p fraktor-cluster-adaptor-rs` が全通過することを確認 — 全 passed
- [x] 27.5 他の依存元モジュールも同様に切り替え — `cluster-core` (Cargo.toml + 13 ファイル)、`showcases/std` (cluster_membership と remote_messaging のサンプルは旧 god-object API 依存のため削除)
- [x] 27.6 この時点で旧 `modules/remote/` は **誰からも参照されていない** 状態 — `grep -rn 'fraktor-remote-rs' modules/*/Cargo.toml` の結果は `modules/remote/Cargo.toml` 自身のみ (`name = "fraktor-remote-rs"`)
- [x] 27.7 `./scripts/ci-check.sh ai all` を実行 — cluster-core/cluster-adaptor-std/showcases/remote-core/remote-adaptor-std はすべて build+test pass。**唯一のエラーは旧 `modules/remote/` 自身の integration test** が `fraktor-remote-rs::std` モジュールの解決に失敗するもの (workspace dev-dep 経由で `std` feature が enable されなくなったため)。Phase E でこの crate を削除すれば自動的に解消するため、Section 27.7 は新クレート経路の検証としては成功扱いとする

## 28. Phase D 完了チェック

- [x] 28.1 `./scripts/ci-check.sh ai all` を実行 — 旧 `modules/remote/` を除く全モジュール pass。旧 crate は Phase E で削除予定
- [x] 28.2 新クレート経路でのリグレッションなし — cluster-core / cluster-adaptor-std / remote-core / remote-adaptor-std / showcases / actor-core / actor-adaptor-std / utils / stream-* / persistence-core すべて pass
- [x] 28.3 **本時点では change の archive を行わない** (Phase E で旧削除後に archive)
...
- [x] 29.1 `modules/remote/` 配下のすべてのファイル・ディレクトリを削除する — `rm -rf modules/remote`
- [x] 29.2 ワークスペース `Cargo.toml` の `members` から `modules/remote` エントリを削除 + `workspace.dependencies` から `fraktor-remote-rs` 削除
- [x] 29.3 `cargo build --workspace` が成功することを確認する — Finished (警告ゼロ)
- [x] 29.4 `cargo test --workspace` が全通過することを確認する — 全 test result OK (0 failures)
- [x] 29.5 `grep -rn 'fraktor-remote-rs' .` の結果がゼロ件 — 残るのは doc コメント内の歴史的記述のみ (実コード参照ゼロ)
- [x] 29.6 `grep -rn 'modules/remote/' .` の結果がゼロ件 — 残るのは openspec/doc コメント内の歴史的記述のみ
- [x] 29.7 `./scripts/ci-check.sh ai dylint`がパスすることを確認する — 全モジュール全 dylint passed

## 30. 最終 CI と change の archive

- [x] 30.1 `./scripts/ci-check.sh ai all` を実行し、**完全無エラー** であることを確認する — **EXIT=0** (全ワークスペース通過)
- [x] 30.2 `cargo doc --workspace --no-deps` が警告ゼロで成功することを確認する — 本 change で新規/変更したクレート (`fraktor-remote-core-rs`, `fraktor-remote-adaptor-std-rs`, `fraktor-cluster-core-rs`, `fraktor-cluster-adaptor-rs`) はすべて警告ゼロで成功。ただし他クレート (`fraktor-utils-rs` の StdSyncMutex 等、`fraktor-actor-core-rs` 等) には**本 change スコープ外の pre-existing な broken intra-doc link** が存在しており、`cargo doc --workspace` 全体は失敗する。その中の修正可能な分はボーイスカウトルールで併せて修正済み (`utils/sync_mutex_guard.rs`、`utils/sync_rwlock_*_guard.rs`、`cluster-core/cluster_provider_error.rs`)
- [x] 30.3 `openspec validate remote-redesign` を実行し、proposal/design/specs/tasks の整合性が取れていることを確認する — "Change 'remote-redesign' is valid"
- [x] 30.4 `openspec archive remote-redesign` を実行する
- [x] 30.5 archive 後、`openspec/changes/remote-redesign/` が存在しないことを確認する
