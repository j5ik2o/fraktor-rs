...
`fraktor_remote_core_rs::failure_detector::PhiAccrualFailureDetector` 型が定義され、Phi Accrual algorithm に基づく failure detector を実装する SHALL。Pekko `PhiAccrualFailureDetector` (Scala, 295行) に対応する。
