...
- [ ] 1.1 `modules/actor-core/src/core/kernel/dispatch/mailbox/base.rs:256,294` の
  `_throughput_deadline` 引数と "follow-up change" コメントを確認 (本 change で削除)
- [ ] 1.2 `Mailbox::new(policy)` の呼び出し箇所を grep で全列挙
  (`modules/actor-core/` + `modules/actor-adaptor-std/` + tests + showcases)
- [ ] 1.3 Pekko `Mailbox.scala:261-278` を開き、`processMailbox` / `shouldProcessMessage` /
  `isThroughputDeadlineTimeDefined` / `throughputDeadlineTime` の契約を行単位で再確認

## 2. `MailboxClock` 型 alias と clock 配送経路の整備 (kernel 層)

- [ ] 2.1 `modules/actor-core/src/core/kernel/dispatch/mailbox/mailbox_clock.rs` を新設
...
- [ ] 2.2 `MailboxSharedSet` (kernel `modules/actor-core/src/core/kernel/system/shared_factory/mailbox_shared_set.rs`)
...
- [ ] 2.3 `Mailbox` 構造体に `clock: Option<MailboxClock>` field を追加
...
- [ ] 2.4 **既存 8 本の factory コンストラクタは signature 維持** (`base.rs:68-187`):
...
- [ ] 3.1 `Mailbox::run()` 内で `_throughput_deadline` の `_` を削除、実名 `throughput_deadline` へ
- [ ] 3.2 `run()` 先頭で一度だけ
...
- [ ] 3.3 `process_mailbox(invoker, throughput)` のシグネチャを
  `process_mailbox(invoker, throughput, deadline_at: Option<Duration>)` に変更
- [ ] 3.4 ループ条件に deadline 判定を追加。**Pekko `Mailbox.scala:271-276` の順序を厳守**:
  ```rust
...
- [ ] 3.5 `// Deadline support is added in a follow-up change (MB-M1, Phase A3)` コメントを
...
- [ ] 4.1 `message_dispatcher_shared.rs:305` の `mbox_clone.run(throughput, throughput_deadline)`
...
- [ ] 4.2 **Instant::now() ベースの新規 closure helper** を `modules/actor-adaptor-std/` に
...
- [ ] 4.3 `MailboxSharedSet::with_clock()` 経由で std adaptor から clock を注入する factory:
  - std adaptor (`modules/actor-adaptor-std/`) の `ActorSystem` 初期化時に 4.2 の
    ...
    で clock 付き bundle を構築
  - この bundle を `*_with_shared_set` 系 factory に渡すことで production 経路は
    自動的に deadline enforcement 有効となる
  - no_std env (core 単体ビルド) では `MailboxSharedSet::builtin()` の clock は `None` のまま
    (embedded adaptor が同様の注入経路を独自に定義する)
- [ ] 4.4 `Mailbox::new(policy)` / `new_with_shared_set` / `new_from_config(_with_shared_set)` /
  ...
  **signature 維持のため書き換え不要** (破壊的書き換えなし):
  ...
  - showcases / integration tests は `MailboxSharedSet::builtin()` の default clock で十分

## 5. 契約 pinned テストの追加 (kernel mailbox `tests.rs`)

- [ ] 5.1 mock clock utility を追加: `SpinSyncMutex<Duration>` ラッパー + `Arc<dyn Fn() -> Duration + Send + Sync>`
  にキャストする helper (`MockClock::new(start)` + `MockClock::advance(delta)` + `MockClock::as_mailbox_clock()`)
- [ ] 5.2 テスト `throughput_deadline_expired_yields_before_exhausting_throughput`:
...
- [ ] 5.8 テスト `throughput_2_with_deadline_zero_and_fixed_clock` (deadline 境界動作):
...
- [ ] 5.9 テスト `clock_none_falls_back_to_throughput_only`:
...
- [ ] 5.10 テスト `deadline_zero_with_clock_progress_breaks_after_one_message` (spec R1 Scenario
...
- [ ] 6.1 `process_mailbox` の新ループ条件に Pekko `Mailbox.scala:275` への参照をコメント付与
- [ ] 6.2 `run()` 先頭の deadline 計算に Pekko `Mailbox.scala:263-266` への参照を付与
- [ ] 6.3 `MailboxClock` 定義の rustdoc に Pekko `System.nanoTime()` 互換 + monotonic 必須
  (wall-clock 禁止) を明記
- [ ] 6.4 `// Deadline support is added in a follow-up change (MB-M1, Phase A3)` の古いコメントを
  削除し、Pekko 行単位対応 rustdoc と置換

## 7. 機械的検証 (grep gate)

- [ ] 7.1 `grep -rn "_throughput_deadline" modules/actor-core/src/` が **0 件**
  (プレフィックス削除済)
- [ ] 7.2 `grep -rn "Deadline support is added in a follow-up change" modules/` が **0 件**
- [ ] 7.3 `grep -rn "MailboxClock" modules/` で **定義 1 件 + 使用 ≥ 3 箇所**
  (`Mailbox::clock` field / `MailboxSharedSet::clock` field / std adaptor の注入 factory)
- [ ] 7.4 `grep -rn "Option<MailboxClock>" modules/actor-core/src/` で
  `MailboxSharedSet` field + `Mailbox` field の 2 箇所以上
- [ ] 7.5 `grep -n "fn set_clock\b\|fn with_clock\b\|fn clock\b" modules/actor-core/src/core/kernel/`
  で `MailboxSharedSet::with_clock`, `MailboxSharedSet::clock`, `Mailbox::set_clock` がすべて存在
- [ ] 7.6 `grep -n "const fn new" modules/actor-core/src/core/kernel/system/shared_factory/mailbox_shared_set.rs`
  が **0 件** (Phase 2.2 で `const fn` を通常 fn に降格済を確認)

## 8. gap-analysis 更新

- [ ] 8.1 `docs/gap-analysis/actor-gap-analysis.md` の MB-M1 行を
  `~~medium~~ done (change `pekko-mailbox-throughput-deadline`)` に書き換え
- [ ] 8.2 第 12 版を追加、medium カウントを 12 → 11 に更新、change log に 2026-04-22 エントリ追加

## 9. Pekko 参照検証 (実装完了時の行単位突合)

- [ ] 9.1 `references/pekko/.../Mailbox.scala:261-278` の `processMailbox` と本実装の
  `process_mailbox` を行単位突合、rustdoc `// Pekko ...` コメントで対応を明示
- [ ] 9.2 Pekko `System.nanoTime + deadlineNs` の加算が
  `self.clock.as_ref().zip(throughput_deadline).map(|(c, d)| c() + d)` と
  セマンティクス的に一致することを design.md / rustdoc 双方で確認
- [ ] 9.3 本 change で Pekko 非互換を新たに作っていないことを検証:
  - 既存テスト全 passing (workspace)
  - 本 change で `#[ignore]` 新規付与なし

## 10. CI / lint の final ゲート

- [ ] 10.1 `openspec validate pekko-mailbox-throughput-deadline --strict` が valid を返す
