...
│   ├─ poll RemoteEventReceiver           ← Port (新規)
...
- **新規型・新規 trait の純増を最小化する**（純増 2 個まで: `RemoteEvent` enum と `RemoteEventReceiver` trait のみ）。
...
```rust
impl Remote {
    pub async fn run<S>(mut self, receiver: &mut S) -> Result<(), RemotingError>
    where
...
`Remote` 自体は型パラメータを持たない（Decision 4 参照）。instrument は `Box<dyn RemoteInstrument + Send>` フィールドで保持するため、`run` 側のシグネチャは `S` のみがジェネリクス。`run` は `self` を consume し、spawn した task が `Remote` を単独所有する。
...
`RemoteEventReceiver` は `Remote::run` が消費する側のみ。
...
**`RemoteEventSink` trait は core に追加しない**。adapter は内部で `tokio::sync::mpsc::channel` を作り、`Sender` clone を I/O ワーカー / handshake timer task に配り、`Receiver` を `TokioMpscRemoteEventReceiver` でラップして `Remote::run` に渡す。sender 側は adapter 内部の責務であり、core から見る必要がない。これにより new trait を 1 つ削減できる。
...
**adapter 側の閉ループ**: adapter は `schedule_handshake_timeout` 受領を契機に tokio task で sleep を起動し、満了時に `RemoteEvent::HandshakeTimerFired { authority, generation }` を adapter 内部 sender 経由で receiver に push する。古い generation の発火は `Remote::run` 側で `!=` 判定により破棄される（adapter 側でキャンセル責務を負わない、Decision 7 と整合）。
...
### Decision 10: `Remote` の所有権を run task に move する（raw shared Remote なし）

`Remote::run(self, ...)` は長期実行 task として `Remote` を単独所有する。本 change では run task 単独所有を主経路にし、installer が raw `SharedLock<Remote>` / `Arc<Mutex<Remote>>` を保持して外部から `Remote` の command/query を直接呼ぶ形にはしない。

これは project-wide な `*Shared` パターンの否定ではない。`Remote` をマルチスレッド共有する設計を採るなら、`Remote` は CQS を守り、共有 surface は `RemoteShared(SharedLock<Remote>)` のような専用 `*Shared` 型に閉じる。さらに `Remote` 内部の構成がほぼ `*Shared` 型で構成され、`Remote` 自体が軽い coordinator / handle として振る舞える場合は `Remote` を共有可能な facade として扱える。本 change ではそこまで内部構成を組み替えず、event loop の所有者を 1 task に固定する。
...
RemotingExtensionInstaller::install(actor_system) {
    ...
    let mut event_receiver = TokioMpscRemoteEventReceiver::new(mpsc_rx);
    let cached_addresses = remote.addresses().to_vec();  // 起動直後にキャッシュ（Remote::addresses() 一本）

    let run_handle = tokio::spawn(async move {
        // remote と event_receiver の所有権はここに move
        remote.run(&mut event_receiver).await
    });

    // installer は event_sender / run_handle / cached_addresses のみ保持
    Self {
        event_sender,
        run_handle,
        cached_addresses,
...
外部制御 surface は adapter 内部の次の **2 つだけ**:

- `Sender<RemoteEvent>` — `TransportShutdown` を push、I/O ワーカー / handshake timer task / RemoteActorRef が clone 共有
- `JoinHandle<Result<(), RemotingError>>` — adapter 固有の async shutdown / wait surface で await

`Remoting::addresses()` は installer の `cached_addresses` から返し、run task 起動後に raw shared handle 経由で `Remote` 本体を直接操作しない。これにより lifecycle と event loop の責務衝突を避ける。

`Remoting` trait は同期 lifecycle surface であり、`remoting.rs` の契約通り `async fn` と `Future` 戻り値を持たない。そのため、同期 `Remoting::shutdown` の内部で `event_sender.send(...).await` や `run_handle.await` を実行してはならない。run task の完了待ちは adapter 固有の async shutdown / wait surface が担う。

adapter 固有 async shutdown / wait surface の手順:
...
同期 shutdown surface を adapter 側に残す場合は、停止要求の受理可否だけを `try_send` 等で返すか、別途 supervisor task が `JoinHandle` を await して結果を log / metric に記録する。同期 API が run task の終了完了まで保証したように見せてはならない。

**棄却した代替案:**

- *raw `Arc<Mutex<Remote>>` / raw `SharedLock<Remote>` を installer field として保持*: `*Shared` 型で API を制限せずに `Remote` 本体を共有すると、CQS の command/query 境界と event loop の所有者が曖昧になる。
- *`RemoteShared(SharedLock<Remote>)` を正式 surface とする設計*: project rule には沿うが、`Remote::run(self, ...)` と同時採用するには `Remote` 内部を `*Shared` 構成へ寄せる追加設計が必要。本 change では run task 単独所有の方を採用する。
...
`Remote::run` は `self` を consume して長期実行 task が単独所有するため、本 change の主経路では `RemoteShared` を導入しない。共有設計を採るなら `RemoteShared(SharedLock<Remote>)` で CQS を保つ必要がある。

**解決策（Decision 10 で確定）:** `Remote` の所有権を spawn した tokio task に **move で渡す**。外部制御は `Sender<RemoteEvent>` と `JoinHandle<Result<(), RemotingError>>` の 2 surface のみ。同期 `Remoting::shutdown` では await せず、完了待ちは adapter 固有の async shutdown / wait surface で行う。`Remoting::addresses()` 相当の query は installer / adapter handle の起動時キャッシュから返す。
...
- `Remote::run<S: RemoteEventReceiver>(self, receiver: &mut S) -> Result<(), RemotingError>` の inherent method 実装
...
### Phase 6: adapter 側 `RemoteEventReceiver` 実装と spawn 経路
...
- 停止時の完了待ちは adapter 固有の async shutdown / wait surface で adapter 内部 sender 経由 `TransportShutdown` push → `run` task の join を行う。同期 `Remoting::shutdown` の内部では await しない
