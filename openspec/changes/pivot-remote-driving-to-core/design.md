...
- `Remoting::shutdown`: `RemoteShared::shutdown` が `with_write(|r| r.shutdown())` で `Remote` の lifecycle を terminated に遷移させる。続けて adapter 内部 sender 経由で `RemoteEvent::TransportShutdown` を `try_send` し、`RemoteShared::run` のループ内で受信させて `Ok(())` を返してループ終了する。`Remoting::shutdown` 自身は同期 method であり、completion 観測（`run_handle.await`）は adapter 固有の async wait surface 側に分離する（Decision 10 参照）。
...
impl RemoteShared {
    ...
    fn with_write<R>(&self, f: impl FnOnce(&mut Remote) -> R) -> R {
        self.inner.with_write(f)
    }

    fn with_read<R>(&self, f: impl FnOnce(&Remote) -> R) -> R {
...
// Remoting trait は RemoteShared 側に実装
...
RemotingExtensionInstaller::install(actor_system) {
    ...
    let run_target = remote_shared.clone();
    let run_handle = tokio::spawn(async move {
        run_target.run(&mut event_receiver).await
    });

    // installer は RemoteShared の clone 1 個と sender / handle のみ保持
    Self {
...
run task に対する停止経路は次の 2 つ:

- `Sender<RemoteEvent>` — `TransportShutdown` を push、I/O ワーカー / handshake timer task / RemoteActorRef が clone 共有
- `JoinHandle<Result<(), RemotingError>>` — adapter 固有の async wait surface で await し、run 終了結果を観測

#### `Remoting::shutdown` の挙動

```text
RemoteShared::shutdown(&self) {
    1. self.with_write(|remote| remote.shutdown())?;          // lifecycle terminated に遷移
    2. self.event_sender.try_send(RemoteEvent::TransportShutdown).ok();  // best-effort wake
       // try_send 失敗（Full / Closed）は wake が遅れるだけ。次の event 処理時に
       // lifecycle が terminated と観測されてループは正常終了する
    Ok(())
}
```

`Remoting::shutdown` は同期 method なので `event_sender.send(...).await` や `run_handle.await` を内部で **実行してはならない**。`event_sender` の保持先は adapter 側 installer または `RemoteShared` のフィールドとして持ち、sync 経路で `try_send` する。

#### adapter 固有 async wait surface

`run_handle.await` による完了観測は `Remoting` trait の外側に置く:

```text
adapter wait surface:
1. (任意) RemoteShared::shutdown() で停止要求を送る — 同期、await 不要
2. run_handle.await {
       Ok(Ok(()))    => Ok(()),                                  // 正常終了
...
`Remoting` trait の同期 API は run task の終了完了まで保証したように **見せてはならない**。停止完了を保証する必要があれば、上記 wait surface を呼び出し側が利用する。
...
外部制御は `Sender<RemoteEvent>` と `JoinHandle<Result<(), RemotingError>>` の 2 surface に加え、`Remoting` trait 経由（`RemoteShared::shutdown` 等）でも停止要求を出せる。同期 `Remoting::shutdown` では `await` せず（`event_sender.try_send` で best-effort wake）、完了観測は adapter 固有の async wait surface で `run_handle.await` を行う。`Remoting::addresses()` は `RemoteShared::addresses()` が `with_read` で source of truth から owned `Vec<Address>` を返すため、cached_addresses を持たない。
...
- `RemotingExtensionInstaller` のフィールドを `remote_shared: RemoteShared` / `event_sender` / `run_handle` に揃える
- `RemotingExtensionInstaller::install` で `RemoteShared::new(remote)` を構築し、clone を 1 個 spawn 用に渡す
- `installer.remote() -> RemoteShared` で `RemoteShared` の clone を返す（raw `SharedLock<Remote>` を露出しない）
- `RemoteShared::shutdown` 内部で `event_sender.try_send(TransportShutdown)` を行い、run loop を wake する（best-effort、sync API なので await しない）
- 停止完了の観測は adapter 固有の async wait surface で `run_handle.await` を行う
