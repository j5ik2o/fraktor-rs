...
│   ├─ run (&mut self, async)               ← 排他所有時の core event loop
...
│   ├─ run (&self, async)                   ← Remote::run の共有 wrapper
...
第一案では `RemoteDriver<S, K, T, I, C>` という 5 ジェネリクスの新規型を導入していたが、これは「Association を駆動するループを所有する」という責務以上のことをしておらず、既存型に統合できる。本 change では **二層構造** を採用し、core event loop は `Remote::run(&mut self, receiver)` として `Remote` に残す。共有が必要な adapter 経路だけ `RemoteShared::run(&self, receiver)` を使う。

**二層の役割分離:**

| 層 | 型 | 並行性責務 | メソッドシグネチャ |
|----|----|------------|--------------------|
| Logic | `Remote` | 知らない | CQS 純粋（`&mut self` for Command / `run`, `&self` for Query） |
...
impl RemoteShared {
        ...
        S: RemoteEventReceiver,
    {
        loop {
            ...
            // Remote::run の共有 wrapper。ロック区間は handle_remote_event 1 回分のみ。
            // RemoteShared 自身は event variant を解釈せず、Remote の CQS method へ委譲する。
            self.with_write(|remote| remote.handle_remote_event(event))?;
            if self.with_read(|remote| remote.is_terminated()) {
                return Ok(());
            }
        }
    }
...
- `Remote` 側はロックを知らずに済み、CQS（`&mut self` for Command / `run`, `&self` for Query）が型シグネチャに直接表れる
...
`async fn` を trait に直書きせず `-> impl Future` にして dyn 互換性は意図的に外す（`Remote::run` / `RemoteShared::run` 側はジェネリクス、adapter 側は具象実装で完結する）。
...
impl Remote {
    ...
    /// 同じ Command / Query を lock 境界付きで呼ぶ。
    ...
    /// 単一 event の dispatch（CQS Command）。
    /// `TransportShutdown` 受信時は内部で `lifecycle` を terminated に遷移する（状態変更）。
...
impl RemoteShared {
    pub async fn run<S: RemoteEventReceiver>(&self, receiver: &mut S) -> Result<(), RemotingError> {
        loop {
            ...
            // Remote::run の共有 wrapper。RemoteShared は event variant を解釈しない。
            self.with_write(|remote| remote.handle_remote_event(event))?;
            if self.with_read(|remote| remote.is_terminated()) {
                return Ok(());
            }
...
impl Remote {
    pub async fn run<S: RemoteEventReceiver>(&mut self, receiver: &mut S) -> Result<(), RemotingError> { ... } // 排他所有時の core event loop
...
impl RemotingExtensionInstaller {
    ...
    pub async fn shutdown_and_join(&self) -> Result<(), RemotingError> {
        ...
        //    冪等失敗（NotStarted / AlreadyShutdown）は許容、それ以外は伝播。
        let remote_shared = self.remote_shared.get().ok_or(RemotingError::NotStarted)?;
        match remote_shared.shutdown() {
            Ok(()) => {}
            Err(RemotingError::NotStarted) => {} // すでに停止済み（idempotent）
            Err(e) => return Err(e),
        }
...
- `Remote::run<S: RemoteEventReceiver>(&mut self, receiver: &mut S) -> Result<(), RemotingError>` を排他所有時の core event loop として実装（consume `self` にはしない）
- `Remote::handle_remote_event(&mut self, event: RemoteEvent) -> Result<(), RemotingError>` の inherent method 実装（CQS Command、状態変更のみ、戻り値 `()` のみ）
- `Remote::is_terminated(&self) -> bool` Query method の追加（lifecycle terminated / shutdown_requested の観測）
- `RemoteEvent::TransportShutdown` 受信時は `lifecycle.transition_to_shutdown_requested()` で状態変更（次の `is_terminated()` Query で停止を観測させる）
...
- `RemoteShared::run<S: RemoteEventReceiver>(&self, receiver: &mut S) -> Result<(), RemotingError>` の inherent method 実装（`Remote::run` の共有 wrapper。per-event `with_write` で Command 実行 → `with_read` で Query 確認、CQS 分離）
