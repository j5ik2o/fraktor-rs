...
RemotingExtensionInstaller::install(actor_system) {
    ...
    let (sender, receiver) = tokio::sync::mpsc::channel(config.event_channel_capacity);
    let mut source = TokioMpscRemoteEventSource::new(receiver);
    let cached_addresses = remote.addresses().to_vec();  // 起動時にキャッシュ
...
- 既存 capability: `openspec/specs/remote-core-extension/spec.md`、`openspec/specs/remote-core-association-state-machine/spec.md`、`openspec/specs/remote-core-instrument/spec.md`、`openspec/specs/remote-core-transport-port/spec.md`、`openspec/specs/remote-adaptor-std-runtime/spec.md`
