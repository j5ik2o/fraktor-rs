...
`Remote` 構造体に inherent method として `pub async fn run<S: RemoteEventReceiver>(self, receiver: &mut S) -> Result<(), RemotingError>` を追加する。`Remote::run` は長期実行 task の所有物になるため `Remote` を consume し、本 change の主経路では raw shared handle 経由で `Remote` 本体を直接操作しない。`Remoting` trait は同期 lifecycle 専用のままにし、async event loop は `Remote` の inherent method として trait 契約を侵食しない。
...
新規 core 型（`RemoteDriver` / `RemoteDriverHandle` / `RemoteDriverOutcome`）は **作らない**。`Remote::run` の終了結果は `Result<(), RemotingError>` で表現し、run task の停止要求と完了待ちは adapter の具体的な制御 surface（内部 `Sender<RemoteEvent>` と `JoinHandle<Result<(), RemotingError>>`）で扱う。同期 `Remoting::shutdown` の内部で `await` してはならない。
...
`RemotingExtensionInstaller` は本 change では `Remote` の所有権を spawn した task に **move** で渡す。これは `*Shared` パターンの否定ではない。`Remote` をマルチスレッド共有する設計を採る場合は、CQS を守る `Remote` を `RemoteShared(SharedLock<Remote>)` のような専用 `*Shared` 型でラップし、呼び出し側に raw `SharedLock<Remote>` を露出しない。ただし本 change の主経路では run task が `Remote` を単独所有するため、installer は raw `SharedLock<Remote>` field を持たない（後述 #9 参照）。

### 9. `Remote` の所有権を run task に move する（raw shared Remote なし）

`Remote::run(self, ...)` は `Remote` の所有権を受け取り、長期実行 task として event loop を駆動する。本 change は run task 単独所有を主経路とし、installer が raw `SharedLock<Remote>` / `Arc<Mutex<Remote>>` を保持して外部から `Remote` の command/query を直接呼ぶ形にはしない。共有が必要な別設計を採る場合は、`RemoteShared` のような専用 `*Shared` 型を定義し、`Remote` 側は CQS を維持し、内部で共有が必要な構成要素はそれぞれ `*Shared` として閉じる。

外部制御は adapter 内部の次の 2 surface のみで行う。

- `Sender<RemoteEvent>`（installer / adapter handle が clone 保持） — `TransportShutdown` を push
- `JoinHandle<Result<(), RemotingError>>` — adapter 固有の async shutdown / wait surface で await し、結果を観測する

`Remoting::addresses()` 相当の同期 query は installer / adapter handle が `transport.start()` で listening を確立した直後に `Remote::addresses()`（既存 inherent method）を呼んで取得した `Vec<Address>` を起動時にキャッシュし、cache から返す。取得経路は `Remote::addresses()` 一本に集約する（`transport.start()` の戻り値を直接キャッシュしたり、`Remote::start` 等の新規 API を追加したりしない）。run task 起動後は、raw shared handle 経由で `Remote` 本体を直接操作しない。

同期 `Remoting::shutdown` は `remoting.rs` の契約通り `async fn` / `Future` を返さないため、`event_sender.send(...).await` や `run_handle.await` を内部で実行しない。run task の完了待ちと `JoinHandle` の結果伝播は adapter 固有の async shutdown / wait surface で行う。同期 shutdown surface を adapter 側に残す場合は、非同期完了を保証したように見せず、停止要求の受理可否を `try_send` 等の観測可能な結果として返すか、別途 supervisor task が `JoinHandle` を await して log / metric に記録する。
...
- **`remote-core-extension`**
  - `Remote` に `async fn run<S: RemoteEventReceiver>(self, receiver: &mut S) -> Result<(), RemotingError>` を追加
  ...
  - run task は `Remote` を所有権 move で持ち、installer は raw `SharedLock<Remote>` / `Arc<Mutex<Remote>>` を保持しない。共有設計へ切り替える場合は専用 `RemoteShared` 型で CQS を保つ
...
- **`remote-adaptor-std-io-worker`**
  ...
  - 同期 `Remoting::shutdown` の中では `await` しない。`RemoteEvent::TransportShutdown` push と `JoinHandle::await` による完了観測は adapter 固有の async shutdown / wait surface で行う
...
- `modules/remote-core/src/core/extension/remote.rs`（`run(self, ..)` inherent method 追加、`Box<dyn RemoteInstrument + Send>` field、`with_instrument` / `set_instrument` 追加）
