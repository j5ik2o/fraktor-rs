...
- [ ] 9.1 新規追加された core 側公開型・公開 trait の数が **2 つ** （`RemoteEvent` enum + `RemoteEventSource` trait）であることを確認する。`RemoteTransport::schedule_handshake_timeout` は既存 trait への method 追加のため、新規 trait カウント外。
