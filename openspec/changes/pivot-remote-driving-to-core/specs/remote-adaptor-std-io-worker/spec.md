...
#### Scenario: 起動順序（install / start / spawn の分離）

- **WHEN** installer の install / start / spawn の経路を検査する
- **THEN** `RemotingExtensionInstaller::install` は `RemoteShared` を構築するだけで、`Remote::start` も `tokio::spawn` も内部で呼ばない
- **AND** 外部から `installer.remote().start()`（`with_write(|r| r.start())` 経由）が呼ばれて `Remote::start` が完了し、transport の listening が確立する
- **AND** その後外部から `installer.spawn_run_task()` 等の明示 API で run task が `tokio::spawn` される
- **AND** spawn 後も `installer.remote()` で `RemoteShared` clone が取得でき、`RemoteShared::addresses` で常に最新の advertised addresses が取れる
- **AND** `Remote::start` 等の別 API（例: `Remote::start_and_get_addresses`）は新設しない

### Requirement: shutdown_and_join による wake + 完了観測

run task の wake と完了観測を 1 step で行う adapter 固有の async API `RemotingExtensionInstaller::shutdown_and_join(self) -> impl Future<Output = Result<(), RemotingError>>` を提供する SHALL。`RemoteShared::shutdown` は **wake せず**、`event_sender` を持たない（薄いラッパー原則）。
...
- **THEN** installer は次の 3 ステップを順次実行する
  1. `self.remote_shared.shutdown()` を呼ぶ（`RemoteShared::shutdown` 経由で lifecycle terminated 遷移、戻り値の `Result` は best-effort で無視可能、log への記録は望ましい）
...
- **AND** ステップ 3 の `Result<Result<(), RemotingError>, JoinError>` を観測し、`Ok(Ok(()))` のみ正常終了。それ以外（`Ok(Err(e))` / `Err(join_err)`）は `RemotingError` に変換して呼出元に伝播する
- **AND** `let _ = ...` による無言握りつぶしは存在しない（observability 目的の log は許容）

#### Scenario: RemoteShared::run の異常終了の観測

- **WHEN** `RemoteShared::run` が `Err(RemotingError::TransportUnavailable)` 等を返した
- **THEN** `installer.shutdown_and_join().await` の戻り値で error が呼出元に伝播される
- **AND** adapter は必要に応じて log 記録 / actor system の error path への通知を行う

#### Scenario: shutdown_and_join 単独でも完了する

- **WHEN** 呼び出し側が事前に `Remoting::shutdown` を呼ばずに `installer.shutdown_and_join().await` だけを呼ぶ
