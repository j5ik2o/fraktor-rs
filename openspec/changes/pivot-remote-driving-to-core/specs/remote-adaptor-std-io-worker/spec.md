...
本 change の主経路では、`RemotingExtensionInstaller` は `Remote` インスタンスの所有権を spawn した tokio task に **move** しなければならない（MUST）。installer は raw `SharedLock<Remote>` / `Arc<Mutex<Remote>>` を field として保持してはならない（MUST NOT）。`Remote` 共有設計を採る場合は、別途 `RemoteShared(SharedLock<Remote>)` のような専用 `*Shared` 型を定義し、`Remote` の CQS 境界を保たなければならない。

#### Scenario: 所有権 move の経路

- **WHEN** installer が `Remote::run` を spawn する経路を検査する
- **THEN** `tokio::spawn(async move { remote.run(&mut receiver).await })` 相当で `remote` 変数が move 入りし、`Remote::run(self, ..)` に所有権を渡す
...
- **AND** `Remote` への raw 直接参照（`Arc<Remote>` / `&Remote`）を保持しない
...
run task の完了待ちは adapter 固有の async shutdown / wait surface が担当する SHALL。同期 `Remoting::shutdown` の内部で `event_sender.send(...).await` や `run_handle.await` を実行してはならない（MUST NOT）。

#### Scenario: async shutdown / wait の手順
...
- **WHEN** adapter 側に同期 `Remoting::shutdown` 相当の surface が残る
- **THEN** その内部で `await` しない
- **AND** 停止要求の受理可否を `try_send` 等で観測可能に返すか、別途 supervisor task が `JoinHandle` を await して log / metric に記録する
- **AND** 同期 API が run task の終了完了まで保証したように見せない
