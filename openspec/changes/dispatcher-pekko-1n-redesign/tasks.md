...
- [ ] 1.3 `ExecutorShared::new<E: Executor + 'static>(executor: E) -> Self` と convenience methods (`execute(&self, task)`, `shutdown(&self)`, `supports_blocking(&self) -> bool`) を実装する。`ActorRefSenderShared` と同じくロック区間を最小化する
...
- [ ] 9.1 `modules/actor-core/src/core/kernel/dispatch/mailbox/base.rs` の `Mailbox::new` を `Mailbox::new(actor_ref, queue)` の形に変更し、message queue を外部から注入可能にする
...
- [ ] 9.4 mailbox の既存 schedule_state / request_schedule は Pekko の `setAsScheduled` / `setAsIdle` / `canBeScheduledForExecution` に揃うようリネーム/シグネチャ整理する
- [ ] 9.5 mailbox 改修後の既存 tests を通す
...
- [ ] 13.4 `dispatcher-trait-family-redesign` の既存 capability spec で MODIFIED 対象となるものが整合していることを確認する

## 14. 最終検証

- [ ] 14.1 Balancing 拡張 seam 5 項目が trait / struct のシグネチャ上で満たされていることを手動確認する
