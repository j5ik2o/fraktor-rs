...
- [x] 14.5 1:N 共有 dispatcher の contention を bench もしくは diagnostics で観測し、既知のトレードオフを記録する
  > `modules/actor-adaptor-std/benches/balancing_dispatcher.rs` に Criterion ベースの contention bench (BalancingDispatcher 4 team members + 1000 envelopes vs DefaultDispatcher 1:1 baseline) を追加。`modules/actor-adaptor-std/src/std/dispatch/dispatcher/tests.rs::balancing_dispatcher_contention_distribution_observation` に同シナリオの `#[tokio::test(flavor = "multi_thread", worker_threads = 4)]` 観測テストも追加し、毎回実行される。観測結果 (5 連続 run、batch=1000、throughput=4096、worker_threads=4): per_actor=[444,288,166,102] / [244,249,240,267] / [428,165,225,182] / [295,260,229,216] / [240,259,239,262]、idle_team_members=0 (全 run)、elapsed_ms=3 typical / 21 max。**design.md §9 が予測した「receiver drain 中に idle team member が残る」トレードオフは実シナリオでは発生しなかった**: 全 team member が常に work をシェアしている。受信者 (actor[0]) にやや偏る傾向はあるが catastrophic な skew はない (max/min ≈ 1.0 〜 4.4)。テストは「全 team member が work する」という empirical invariant を assert する。bench 実行時のセットアップは `tick_driver/tests.rs` の pre-existing コンパイルエラー (`TickDriverBootstrap` private + `critical-section/std` impl) を `actor-adaptor-std/Cargo.toml` の dev-dependencies に `fraktor-actor-core-rs/test-support` と `critical-section/std` を追加することで併せて修復した (元々 main で broken だった bench harness が動くようになった)。**スコープ外 follow-up**: throughput=5 (default) で同シナリオを実行すると ~450 envelope で hang する → `Mailbox::run` が `set_idle()` の戻り値を ignore して lost wake-up を起こしている。本 change のスコープ外として記録、別 change で対処予定。
