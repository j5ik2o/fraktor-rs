...
#### Scenario: execute_task は task 実行中に dispatcher を生かし続ける
- **WHEN** `MessageDispatcher::execute_task(&mut self, task)` を呼ぶ
- **THEN** 実行前に `self.core_mut().mark_attach()` が呼ばれる
- **AND** task 完了時 cleanup で `self.core_mut().mark_detach()` が呼ばれる
- **AND** cleanup 後の inhabitants が 0 の場合のみ `self.core_mut().schedule_shutdown_if_sensible()` が呼ばれる
- **AND** task 内で panic が発生しても inhabitants の復旧が保証される
