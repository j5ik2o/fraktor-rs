...
- **AND** ロック解放後、候補配列を優先度順に走査して各 mailbox に対し `self.register_for_execution(&mbox, has_message_hint=true, has_system_hint=false)` を試みる
...
- **AND** 戻り値は `vec![receiver.mailbox()]` (V1 単一候補)
