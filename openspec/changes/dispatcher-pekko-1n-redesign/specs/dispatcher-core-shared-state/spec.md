...
- **AND** 次の command メソッドが `&mut self` で提供されている: `mark_attach(&mut self) -> i64`, `mark_detach(&mut self) -> i64`, `schedule_shutdown_if_sensible(&mut self)`, `shutdown(&mut self)`
- **AND** `mark_attach` / `mark_detach` の戻り値 `i64` は CQS 許容例外として扱われる（合成後の inhabitants を呼び出し側が直ちに判定できるようにするため）
...
`MessageDispatcherShared::register_for_execution` は Pekko `Dispatcher.registerForExecution` と同じ契約で動作しなければならない (MUST)。mailbox の `canBeScheduledForExecution` / `setAsScheduled` 相当の CAS に成功した場合のみ executor へ submit し、ロック区間を最小化しつつ結果を返す。`MessageDispatcher` trait 側の `register_for_execution(&mut self, ...)` は mailbox policy 固有の追加判定 hook を担当し、実際の closure 組み立てと executor への submit は `MessageDispatcherShared` 側で orchestrate する。
...
- **AND** CAS 成功後に `with_write` で trait hook `register_for_execution(&mut self, ...)` が呼ばれる
- **AND** hook が `false` を返した場合は `set_as_idle` で状態を戻して `false` を返す
...
- **AND** trait hook `register_for_execution` の呼び出しは 1 度の `with_write` で完結する
