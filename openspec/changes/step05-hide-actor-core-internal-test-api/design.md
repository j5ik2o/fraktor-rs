...
「外部 caller」とは **actor-core 以外の workspace crate** からの参照を指す。intra-crate (actor-core 内部) production 呼び出しは `pub(crate)` でカバー可能なので戦略 A の妨げにならない。

| # | Symbol | File | gate 種別 | 外部 caller (= actor-core 外) | intra-crate production caller | 戦略 |
|---|---|---|---|---|---|---|
| 1 | `Behavior::handle_message` | `core/typed/behavior.rs:204` | pub-promote | 0 | なし (定義サイトのみ) | A |
| 2 | `Behavior::handle_start` | `core/typed/behavior.rs:243` | pub-promote | 0 | なし (定義サイトのみ) | A |
| 3 | `Behavior::handle_signal` | `core/typed/behavior.rs:274` | pub-promote | 0 | あり (`behavior.rs:364`、`behavior_runner.rs:130`、`behaviors.rs:629`) | A |
| 4 | `TypedActorContext::from_untyped` | `core/typed/actor/actor_context.rs:50` | pub-promote | 0 | あり (`behavior.rs` 複数、`typed_actor_adapter.rs` 複数) | A |
| 5 | `state::booting_state` mod | `core/kernel/system/state.rs:21` | mod-existence toggle | 0 | なし (intra-crate も test scaffold 用途のみ) | A |
| 6 | `state::running_state` mod | `core/kernel/system/state.rs:23` | mod-existence toggle | 0 | なし (`booting_state.rs:5` から `use` されるのみ、production 経路なし) | A |
| 7 | `SystemState::register_guardian_pid` | `core/kernel/system/state/system_state.rs:500` | method-existence toggle (両分岐 pub(crate)) | 0 | あり (`booting_state.rs:19`) | A |
| 8 | `SystemStateShared::register_guardian_pid` | `core/kernel/system/state/system_state_shared.rs:430` | method-existence toggle (両分岐 pub(crate)) | 0 | なし | A |
| 9 | `ActorRef::new_with_builtin_lock` | `core/kernel/actor/actor_ref/base.rs:90` | pub-promote (`new_with_builtin_lock_impl` も同梱) | 0 | なし (定義サイトのみ。同名の `UserQueueShared::new_with_builtin_lock` は別シンボル) | A |
| 10 | `SchedulerRunner::manual` | `core/kernel/actor/scheduler/scheduler_runner.rs:39` | pub-promote | 0 | なし (定義サイトのみ) | A |
| 11 | `TickDriverBootstrap` struct + `provision` メソッド | `core/kernel/actor/scheduler/tick_driver/bootstrap.rs:17,28` | pub-promote (両分岐 pub/pub(crate) 切替) + `tick_driver.rs:25` の re-export | 0 | あり (`system_state.rs:183` import / `:258` provision 呼び出し) | A |
