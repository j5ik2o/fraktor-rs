...
**When** `use cellactor_core::actor::Actor;` でインポートする
**Then** `Actor` トレイトと関連する型（`ActorRef`, `ActorCell`, `ActorContext`等）にアクセスできる
**And** 内部実装の詳細（`actor/internal/`）は公開されていない

#### Scenario: メッセージングパッケージの使用
**Given** ユーザーがメッセージング機能を使用したい
**When** `use cellactor_core::messaging::AnyMessage;` でインポートする
...
既存のフラットなインポートパスは引き続き動作しなければならない (MUST)。`lib.rs` での再エクスポートにより、後方互換性を完全に維持する (SHALL)。

#### Scenario: 既存コードの互換性
**Given** 既存のコードが `use cellactor_core::Actor;` のようにインポートしている
**When** パッケージ構造のリファクタリングが完了した後でも
**Then** 既存のインポートパスは引き続き動作する
**And** ビルドエラーやコンパイルエラーが発生しない

### Requirement: 内部実装の隠蔽
公開APIと内部実装を明確に分離しなければならない (MUST)。内部実装は `internal/` サブパッケージに配置し、`pub(crate)` で可視性を制限する (SHALL)。

#### Scenario: 内部実装へのアクセス制限
**Given** ユーザーが`actor`パッケージを使用する
**When** 公開されていない内部実装（`actor/internal/`）にアクセスしようとする
**Then** コンパイルエラーが発生する
...
以下の11個のパッケージが存在しなければならない (MUST)：

1. `actor/` - アクタープリミティブ（`Actor`, `ActorRef`, `ActorCell`, `ActorContext`, `Pid`, `ChildRef`, `ReceiveState`）
...
**Then** テストは `actor/` パッケージ内に配置されている
**And** すべてのテストが正常にパスする
