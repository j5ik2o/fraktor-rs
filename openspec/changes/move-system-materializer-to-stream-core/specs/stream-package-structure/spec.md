...
### Requirement: std アダプタは io と materializer の責務境界で整理されなければならない
`modules/stream-adaptor-std` は、std 環境に固有の stream アダプタだけを公開しなければならない（MUST）。`FileIO`、`StreamConverters`、`StreamInputStream`、`StreamOutputStream`、std-backed source adapter は `io` package 境界に集約される。`SystemMaterializer` と `SystemMaterializerId` は `modules/stream-core-kernel/src/materialization` に属し、std アダプタ crate はこれらを定義または互換 re-export してはならない（MUST NOT）。
...
#### Scenario: std アダプタは system materializer を公開しない
