...
2026-05-08 時点の最新コードとの差分は次の通り。

- `TokioExecutor` は `modules/actor-adaptor-std/src/std/dispatch/dispatcher/tokio_executor.rs` に存在し、現在は `Handle::spawn_blocking` を使っている。
- `TokioExecutorFactory` は存在するが、blocking 専用の `TokioBlockingExecutor` / `TokioBlockingExecutorFactory` はまだない。
- `std_actor_system_config` は std monotonic mailbox clock の注入だけを担当している。Tokio default / blocking dispatcher factory と `TokioTickDriver` をまとめる helper はまだない。
- `Dispatchers::ensure_default` は default と blocking に同じ inline configurator を seed し得るが、`ActorSystemConfig::with_dispatcher_factory` により reserved id を個別 override できる。
- untyped kernel の `ActorContext::pipe_to_self` / `pipe_to`、`ContextPipeTask`、waker は存在し、cell missing と async future wakeup の tests もある。
- typed `TypedActorContext::pipe_to_self` / `pipe_to` は untyped context に委譲しており、external target delivery、adapter failure log、typed user flow の coverage がある。
- `TickDriverKind` は `Auto`、`Manual`、`Std`、`Tokio` までであり、`Embassy` variant はまだない。`AutoProfileKind::Embassy` は存在する。
- `modules/actor-adaptor-embassy` はまだ workspace に存在しない。

このため、future-to-message surface は「作る」より「公開契約として固定する」作業が中心であり、主要な未実装は Tokio executor split と Embassy adapter である。
...
`TokioExecutor` は既存名を維持し、`Handle::spawn` で async task を起動する default task executor として再定義する。`TokioTaskExecutor` は新設しない。`Executor` trait は `Box<dyn FnOnce()>` を受け取る sync submit primitive のままにするため、実装は概念上次の形になる。
...
Embassy 依存は `actor-core` に入れない。crate 名は `actor-adaptor-embassy` とする。`modules/actor-adaptor-embassy` を追加し、`actor-core` の `Executor` / `TickDriver` / mailbox clock injection へ接続する adapter だけを持つ。
...
- Embassy adapter の CI target / feature をどこまで初期 gate に含めるか。
