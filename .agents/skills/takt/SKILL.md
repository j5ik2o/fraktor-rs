...
name: takt
description: >
  TAKT ピースエンジン。codex exec でサブエージェントを起動し、ピース YAML ワークフローに従ってマルチエージェントオーケストレーションを実行する。
---
...
1. `${SKILL_ROOT}/references/engine.md` - プロンプト構築、レポート管理、ループ検出の詳細
2. `${SKILL_ROOT}/references/yaml-schema.md` - ピースYAMLの構造定義

${SKILL_ROOT}は当該ファイルが配置されているディレクトリとする。
...
2. ピース名として検索:
   - `${SKILL_ROOT}/pieces/{name}.yaml` （ユーザーカスタム、優先）
   - `${SKILL_ROOT}/pieces/{name}.yaml` （Skill同梱ビルトイン）
...
例: ピースが `${SKILL_ROOT}/pieces/default.yaml` にあり、`personas:` に `coder: ../facets/personas/coder.md` がある場合
→ 絶対パスは `${SKILL_ROOT}/takt/facets/personas/coder.md`
...
**実行ディレクトリ**: いずれかの movement に `report` フィールドがある場合、`${PROJECT_ROOT}/.takt/runs/{YYYYMMDD-HHmmss}-{slug}/` を作成し、以下を配置する。
- `reports/`（レポート出力）
...
${PROJECT_ROOT}は2階層上のプロジェクトのディレクトリとする

レポート出力先パスを `report_dir` 変数（`${PROJECT_ROOT}/.takt/runs/{slug}/reports`）として保持する。
...
詳細は `${SKILL_ROOT}/references/engine.md` の「Loop Monitors」を参照。
...
| ファイル                                      | 内容 |
|-------------------------------------------|------|
| `${SKILL_ROOT}/references/engine.md`      | プロンプト構築、レポート管理、ループ検出の詳細 |
| `${SKILL_ROOT}/references/yaml-schema.md` | ピースYAMLの構造定義とフィールド説明 |
