...
2. 全呼び出し元が既にその条件を満たしている場合、防御は不要。ただし公開API・公開trait・facade経由で外部利用されうる関数では、repo内呼び出し元だけで削除判断しない
...
| 判定 | 基準 |
|------|------|
| REJECT | 現在どこからも呼ばれていない内部関数・内部メソッド |
| REJECT | 「対称性のため」に作られたが使われていないsetter/getter |
| REJECT | 将来の拡張のために用意されたインターフェースやオプション |
| REJECT | 内部モジュールでexportされているが、grep で使用箇所が見つからない |
| OK | フレームワークが暗黙的に呼び出す（ライフサイクルフック等） |
| OK | 公開API・公開trait・facade経由の外部契約として残す必要がある |

検証アプローチ:
1. 変更・削除されたコードを参照している箇所がないか grep で確認
2. 公開モジュール（index ファイル等）のエクスポート一覧と実体が一致しているか確認
3. 公開API・公開trait・facade経由で外部利用されうるか確認。外部契約に該当する場合は、repo内使用箇所がなくても削除対象にしない
4. 新規追加されたコードに対応する古いコードが残っていないか確認
...
| パターン | 例 | 判定 |
|---------|-----|------|
| deprecated + 使用箇所なし | `@deprecated` アノテーション付きで誰も使っていない | 内部APIなら削除 |
...
| パターン | 例 | 判定 |
|---------|-----|------|
| 外部公開API | published facade のエクスポート | 公開面を確認してから判断 |
| 設定ファイル互換 | 旧形式の設定を読める | 明示要求がある場合のみ維持 |
| データ移行中 | DBスキーマ移行の途中 | 移行完了まで維持 |

判断基準:
1. 使用箇所があるか? → grep/検索で確認。なければ削除
2. 新旧両方に使用箇所があるか? → 両方が現在使われているなら、後方互換ではなく並存設計の可能性がある。呼び出し元を確認
3. 外部に公開しているか? → 内部のみなら削除可能。公開API・公開trait・facade経由の外部契約なら、削除前に公開面と移行方針を確認する
4. fraktor-rs の pre-release 方針と衝突しないか? → このプロジェクトでは後方互換性は不要なため、明示要求なしに互換レイヤーを残さない
5. 移行は完了したか? → 完了なら削除
