...
2. 全呼び出し元が既にその条件を満たしている場合、防御は不要。`public` / `export` / facade 経由というだけでは保護対象にしない
...
| 判定 | 基準 |
|------|------|
| REJECT | 現在どこからも呼ばれていない public / internal 関数・メソッド |
...
| OK | リリース済み外部契約、または明示された互換維持要求として残す必要がある |

検証アプローチ:
1. 変更・削除されたコードを参照している箇所がないか grep で確認
2. 公開モジュール（index ファイル等）のエクスポート一覧と実体が一致しているか確認
3. リリース済みの外部契約または明示された互換維持要求に該当するか確認。未リリース実装では `public` / `export` / facade 経由というだけで削除を止めない
...
| パターン | 例 | 判定 |
|---------|-----|------|
| リリース済み外部公開API | npm パッケージのエクスポート | 慎重に検討 |
...
3. 外部にリリース済みか? → 未リリースなら `public` / `export` / facade 経由でも削除可能。リリース済み外部契約なら、削除前に公開面と移行方針を確認する
