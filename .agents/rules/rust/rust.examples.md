...
```rust
let _ = mailbox.try_send(msg);  // let-underscore-must-use /
                                 // let-underscore-forbid-lint で検出
...
### `./scripts/ci-check.sh` の使い方

```bash
# 完了時の最終確認（AI 向けガード付き、所要時間長い）
./scripts/ci-check.sh ai all

# 途中工程: 対象範囲のみ
./scripts/ci-check.sh dylint module-wiring-lint
./scripts/ci-check.sh unit-test
./scripts/ci-check.sh clippy

# 並行実行は不可（内部で cargo を呼ぶ）
```
