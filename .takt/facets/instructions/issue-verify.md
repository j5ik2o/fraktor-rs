...
4. issue ごとに `./scripts/ci-check.sh all` 実行結果が PASS かを検証する
5. 仕上げとして `./scripts/ci-check.sh all` を実行し、最終 PASS を検証する
6. FAIL がある場合は不足点を具体化し、fix に戻すための修正指示をまとめる

## 判定ルール

- 全 issue の全条件が PASS または理由付き SKIPPED で、最終 `./scripts/ci-check.sh all` PASS: `issue完了条件を満たす`
