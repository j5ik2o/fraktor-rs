...
3. issue ごとに受け入れ条件が満たされているかを PASS / FAIL で判定する
4. issue ごとにコミット要件を満たすか検証する
...
- 全 issue の全条件 PASS かつ全 issue のコミット要件 PASS かつ最終 `./scripts/ci-check.sh all` PASS: `issue完了条件を満たす`
- 1件でも FAIL: `未達で追加修正が必要`
- 判定材料不足: `判断不能`
