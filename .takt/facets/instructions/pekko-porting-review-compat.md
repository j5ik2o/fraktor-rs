...
**前回指摘の追跡（必須）:**
- まず「Previous Response」と Report Directory 内の過去レビュー履歴から、前回の open findings と resolved findings を抽出する
- 各 finding に `finding_id` を付け、今回の状態を `new / persists / resolved / reopened` で判定する
- 以前 `resolved` だった finding が再発している場合は `reopened` と判定し、前回の解消根拠と今回の再発根拠を示す
- `persists` または `reopened` と判定する場合は、未解決または再発の根拠（ファイル/行）を必ず示す

## 判定手順

1. まず前回open/resolved findingsを抽出し、`new / persists / resolved / reopened` を仮判定する
