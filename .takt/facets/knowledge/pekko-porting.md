...
- `*-core*` crate: `#![no_std]` + `extern crate alloc`
- `*-adaptor-std` crate: `std` 依存OK（Tokio, ネットワーク等）
- `*-core*` crate で `std::` を直接使用禁止（`cfg-std-forbid` lint で強制）
