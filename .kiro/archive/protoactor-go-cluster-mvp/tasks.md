...
- [x] 1.1 参加・離脱・到達不能の状態遷移とハンドシェイクを実装する
...
- [x] 3.1 グレインの割当てと PID キャッシュを整備する
...
- [x] 3.2 アイドル passivation と再アクティベーションを実装する
...
- [x] 4.1 スキーマネゴシエーションとシリアライズ検証を実装する
...
- [x] 4.2 RPC 並列度・タイムアウト・リトライを制御する
...
- [x] 5.1 Quickstart 設定検証とクラスタ起動を実装する
  - authority/port/seed の必須設定を検証し、欠落時は起動を拒否して欠落項目を明示する
  - ClusterBootstrap の `with_quickstart_defaults()` を使ってデフォルト2ノードをブートし、参加成功とメンバー一覧を表示する
  - 起動ログに環境情報と設定スナップショットを残し、再現可能性を高める
  - _対応要件: 8.1, 8.4_
  - _依存タスク: 4.3_
  - _完了条件: 必須設定欠落時に起動拒否と診断を出力し、正しい設定でクラスタ形成が成功する_

- [x] 5.2 サンプルグレインのリモート往復を用意する
  - ClusterBootstrap のクイックスタート設定経由でサンプルアクター(グレイン)を生成し、ノード間 ask/reply を往復させる
