...
- **Pekko 互換リモートアドレッシング**: `ActorPathParts` と `ActorPathFormatter` が `pekko://system@host:port/...` 形式の canonical URI を生成し、`GuardianKind` で `/system` `/user` を自動注入して `cellactor` デフォルトガーディアンを守ります。
...
- **リモート互換性**: Pekko/Proto.Actor と同じ actor path 体系（`pekko` / `pekko.tcp` スキーム、guardian 自動挿入、UID suffix）と quarantine ルールを Rust/no_std 上で再現し、異種ホストと埋め込み環境間での remoting を遮断なく延長できる。
