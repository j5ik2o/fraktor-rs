...
3. 除去対象ノードと畳み込み先ノードが指定されたとき、CRDT は安全に畳み込める場合に除去ノードの寄与を畳み込み先へ移した新しい値を返し、表現範囲を超える場合は畳み込みを拒否しなければならない
...
6. 自ノードスロット加算・値集計・プルーニング畳み込みが表現範囲を超える場合、`GCounter` は wrap や saturate ではなく該当操作を拒否しなければならない
...
6. 増加成分・減少成分・差分値が表現範囲を超える場合、`PNCounter` は wrap や saturate ではなく該当操作を拒否しなければならない
...
6. キー配下の `PNCounter` 更新・値取得・プルーニング畳み込みが表現範囲を超える場合、`PNCounterMap` は wrap や saturate ではなく操作を拒否しなければならない
