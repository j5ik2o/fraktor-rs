...
- **API 形状（Canonical）**: 公開 API の正規形は Pekko と同じ `schedule_once(delay, receiver, message, dispatcher, sender)` / `scheduleAtFixedRate` / `scheduleWithFixedDelay` とし、Rust 版でも同じ順序・型制約で導入する。`SchedulerExecutionContext` はこれら canonical API を呼ぶためのヘルパ（`schedule_once_with(delay, receiver, message, &ctx)`）として提供し、dispatcher/sender のデフォルト解決や ActorContext からの暗黙取得を可能にするが、公開シグネチャ自体は Pekko 仕様に固定する。
- **取得経路**: Actor 以外の利用者（Remoting 初期化、DelayProvider 以外の subsystem）は `ActorSystem::scheduler_execution_context(name)` で Dispatcher レジストリからハンドルを受け取り、名前が `None` の場合は default dispatcher を返す。`SchedulerExecutionContext` から dispatcher/sender を取得し canonical API へ渡すため、ctx が存在しない環境でも直接 dispatcher/sender を指定すれば同じエントリポイントを利用できる。
...
`DispatcherId` は既存 Dispatcher レジストリが管理され、`SchedulerExecutionContext` を通じて暗黙に供給される。Actor 側のユーティリティ（例: `Context::schedule_once`) では `impl SchedulerExecutionContext for ActorContext` を利用し、内部で canonical API（dispatcher/sender 明示）を呼び出す。これにより Pekko 互換 API の表面仕様を明示的に固定しつつ、Rust ユーザは ctx ベースの使い勝手を得られる。

##### Canonical API と ctx ヘルパの役割分担
- Canonical: Pekko 仕様と 1:1 の関数（receiver, message, dispatcher, sender）を唯一の公開エントリポイントとし、ドキュメント/API リファレンスでもこちらを正式な形として記載する。
- Helper: `SchedulerExecutionContext` と builder (`schedule_once_fn(delay).with_context(&ctx)`) は dispatcher/sender の解決を肩代わりする薄いラッパで、内部的には canonical API を即座に呼び出す。`ctx` を採用しても戻り値/エラー型は同一であり、R3 AC5 の「Pekko と同じシグネチャ」要件が常に維持される。
- Deprecation: 旧 `DispatcherId` 直接指定のショートカットは `#[deprecated]` で残すが、次メジャーリリースで廃止する方針を設計に記録する。

`SchedulerBatchContext` は std では thread-local、no_std では `RuntimeToolbox::task_local_slot()`（critical-section + per-task スロット）上に構築した `TaskLocalBatch` で実装する。SystemMailbox は message/Runnable をデキューする直前に `SchedulerBatchContext::push(ExecutionBatch, CompletionHandle)` を呼び、Drop 時に `CompletionGuard` が `ack_complete` をトリガする。Actor は `Context::scheduler_batch()`、Runnable は `BatchAwareRunnable::run(&self, batch: ExecutionBatch)`、DelayFuture は `DelayFuture::current_batch()` でメタデータを参照でき、公開シグネチャを変えず Requirement R2 AC2/AC4/AC5 を満たす。task-local の取得/解放は RuntimeToolbox のライフタイムに束縛され、割り込み/async どちらのコンテキストでも `BorrowedBatchSlot<'a>` を介して安全に扱える。
