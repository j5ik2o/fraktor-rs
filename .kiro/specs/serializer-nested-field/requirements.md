...
2. When the Serializer サブシステムが子フィールド `B` のシリアライザバインディングを解決したとき, the Serializer サブシステム shall その結果のバイト列と必要なエンコーディングメタデータを親フィールドのエンベロープに組み込み、子フィールド単体で親と同一エンベロープを再生成しない。
