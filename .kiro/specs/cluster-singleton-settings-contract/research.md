...
## 命名修正（2026-06-11、実装完了後のユーザー決定）

- **決定**: 設定型のサフィックスは `*Config` に統一する。`*Settings` と `*Config` の混在は NG（リポジトリ全体方針）。設計ディスカバリー決定 #1（Pekko 逆輸入名 `*Settings`）はサフィックスに関して撤回し、ドメイン用語の優先は語幹のみに適用する
- **本 spec の改名**: `ClusterSingletonManagerSettings`→`ClusterSingletonManagerConfig`、`ClusterSingletonProxySettings`→`ClusterSingletonProxyConfig`、typed `ClusterSingletonSettings`→`ClusterSingletonConfig`、`LeaseUsageSettings`→`LeaseUsageConfig`、`ClusterSingletonSettingsError`→`ClusterSingletonConfigError`（ファイル名・フィールド名・アクセサ名・導出メソッド名も連動）
- **リポジトリ全体**: 既存の `DistributedPubSubSettings` / `SplitBrainResolverSettings` / `PersistenceSettings` / `StreamRefSettings` も `*Config` へ改名。互換キー文字列 `cluster.split-brain-resolver.settings` は `cluster.split-brain-resolver.config` へ（pre-release のため互換不要）
- **ルール反映**: `.agents/rules/rust/naming-conventions.md` の「参照実装の命名優先」に設定型サフィックス統一の例外を明文化
