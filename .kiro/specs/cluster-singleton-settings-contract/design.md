...
- `ClusterSingletonManagerSettings` / `ClusterSingletonProxySettings` / typed `ClusterSingletonSettings` / `LeaseUsageSettings` を Pekko 既定値互換のデフォルト付きで定義する
...
- `ClusterSingletonSettingsError` による検証エラーの語彙
...
- lease の取得・更新プロトコル（`LeaseUsageSettings` は設定 2 項目のみを所有し、SBR の lease 語彙とは結合しない）
...
graph TB
  subgraph kernel["cluster-core-kernel"]
    Singleton[singleton モジュール<br/>ManagerSettings / ProxySettings<br/>LeaseUsageSettings / Error / StuckPhase]
  ...
  subgraph typed["cluster-core-typed"]
    TypedSettings[ClusterSingletonSettings]
  end
  ExtConfig --> Singleton
  Installer --> ExtConfig
  ExtConfig --> Catalog
  Event --> Singleton
  TypedSettings --> Singleton
```
...
└── singleton/
    ├── cluster_singleton_manager_settings.rs      # manager 設定型 + validate + max_hand_over_retries
    ├── cluster_singleton_manager_settings_test.rs
    ├── cluster_singleton_proxy_settings.rs        # proxy 設定型 + validate
    ├── cluster_singleton_proxy_settings_test.rs
    ├── lease_usage_settings.rs                    # lease 設定スロット（実装名 + リトライ間隔）
    ├── lease_usage_settings_test.rs
    ├── cluster_singleton_settings_error.rs        # 検証エラー enum + Display
    ├── cluster_singleton_settings_error_test.rs
...
├── cluster_singleton_settings.rs                  # typed 統合設定 + to_manager_settings / to_proxy_settings
└── cluster_singleton_settings_test.rs
```
...
- `modules/cluster-core-kernel/src/extension/cluster_extension_config.rs`（+ `_test.rs`）— singleton 設定フィールド・setter・getter・`validate_singleton()`・`JOIN_COMPATIBILITY_CHECKS` エントリ（`singleton_settings_mismatch_detail`）の追加
...
sequenceDiagram
  ...
  participant Singleton as Singleton Settings
...
|------|------|------------|------------|-------|
| 1.1 | manager 設定の保持 | ClusterSingletonManagerSettings | フィールド + getter | — |
| 1.2 | manager 既定値 | 同上 | `new()` / `Default` | — |
| 1.3 | removal margin 未指定の区別 | 同上 | `removal_margin: Option<Duration>` | — |
| 1.4 | lease スロットの保持 | LeaseUsageSettings | `lease_settings: Option<LeaseUsageSettings>` | — |
| 2.1 | proxy 設定の保持 | ClusterSingletonProxySettings | フィールド + getter | — |
| 2.2 | proxy 既定値 | 同上 | `new()` / `Default` | — |
| 2.3 | buffer size 0 の許容 | 同上 | `validate()`（0 を有効値とする） | — |
| 3.1 | typed 一括指定 | ClusterSingletonSettings (typed) | フィールド + setter | — |
| 3.2 | manager 導出の無損失 | 同上 | `to_manager_settings(name)` | — |
| 3.3 | proxy 導出の無損失 | 同上 | `to_proxy_settings(name)` | — |
| 3.4 | typed 既定値の一致 | 同上 | `Default`（kernel 既定値委譲） | — |
| 4.1 | 同一境界での検証 | ClusterExtensionInstaller | `validate_singleton()` 呼び出し | install フロー |
| 4.2 | buffer size 範囲検証 | ClusterSingletonProxySettings | `BufferSizeOutOfRange` | install フロー |
| 4.3 | ゼロ以下間隔の拒否 | Manager / Proxy Settings | `NonPositive*Interval` | install フロー |
| 4.4 | 空 singleton 名の拒否 | Manager / Proxy Settings | `EmptySingletonName` | install フロー |
| 4.5 | lease 不成立の拒否 | LeaseUsageSettings | `EmptyLeaseImplementation` / `NonPositiveLeaseRetryInterval` | install フロー |
...
| 5.2 | 不一致項目の特定 | singleton_settings_mismatch_detail | `difference_field_names` | 既存 compat フロー |
| 5.3 | 一致時は拒否しない | 同上 | detail が `None` を返す経路 | 既存 compat フロー |
| 6.1 | install 境界での受け取り | ClusterExtensionConfig | `with_singleton_*_settings` | install フロー |
| 6.2 | 未指定でも既定値 install | 同上 | `new()` が既定値を内包 | install フロー |
| 6.3 | 既存構成の無影響 | 同上 | 追加フィールドのみ（既存 API 無変更） | — |
| 7.1 | リトライ上限の決定的導出 | ClusterSingletonManagerSettings | `max_hand_over_retries()` | — |
...
|-----------|--------------|--------|--------------|----------|-----------|
| ClusterSingletonManagerSettings | kernel/singleton | manager 設定の保持・検証・導出 | 1, 4.3, 4.4, 7.1 | LeaseUsageSettings | Service |
| ClusterSingletonProxySettings | kernel/singleton | proxy 設定の保持・検証 | 2, 4.2–4.4 | membership::DataCenter | Service |
| LeaseUsageSettings | kernel/singleton | lease スロットの 2 項目保持 | 1.4, 4.5 | なし | Service |
| ClusterSingletonSettingsError | kernel/singleton | 検証エラーの語彙 | 4.2–4.5 | なし | Service |
| SingletonStuckPhase | kernel/singleton | stuck 局面の語彙 | 7.2 | なし | State |
| ClusterSingletonSettings | typed | 統合設定と導出 | 3 | kernel singleton 型 | Service |
...
- フィールドと既定値（Pekko `reference.conf` 互換）:
  ...
  - `lease_settings: Option<LeaseUsageSettings>` — 既定 `None`
...
impl ClusterSingletonManagerSettings {
  ...
  pub fn with_lease_settings(self, lease: LeaseUsageSettings) -> Self;
  // getters: singleton_name() / role() / removal_margin() /
  //          hand_over_retry_interval() / min_hand_over_retries() / lease_settings()
...
**検証規則**: 空 `singleton_name` → `EmptySingletonName`、`hand_over_retry_interval` ゼロ → `NonPositiveHandOverRetryInterval`、lease 保持時は `LeaseUsageSettings::validate()` へ委譲。
...
**責務と制約**
- フィールド: `role` / `data_center` / `singleton_identification_interval` / `removal_margin` / `hand_over_retry_interval` / `min_hand_over_retries` / `buffer_size` / `lease_settings`（Pekko typed `ClusterSingletonSettings` と同構成）
...
**責務と制約**
- フィールド追加: `singleton_manager_settings: ClusterSingletonManagerSettings`、`singleton_proxy_settings: ClusterSingletonProxySettings`（いずれも `new()` で既定値を内包。要件 6.2）
- setter / getter 追加: `with_singleton_manager_settings` / `with_singleton_proxy_settings` / `singleton_manager_settings()` / `singleton_proxy_settings()`
- **検証の追加方針**: 既存 `validate() -> Result<(), FailureDetectorConfigError>` のシグネチャは変更しない（要件 8.1 / 8.3）。新規 `validate_singleton(&self) -> Result<(), ClusterSingletonSettingsError>` を追加し、manager → proxy の順に委譲する。installer が両者を同じ install 境界で連続実行する（要件 4.1）。検証エラー型の統一は Phase 3 以降の再設計対象として持ち越す
- **互換チェックの追加**: `JOIN_COMPATIBILITY_CHECKS` に `JoinCompatibilityCheck::new(ClusterCompatibilityKeyCatalog::SINGLETON, singleton_settings_mismatch_detail)` を追加。`singleton_settings_mismatch_detail` は manager / proxy の `difference_field_names()` を `manager.` / `proxy.` プレフィックス付きで結合し、差異がなければ `None` を返す（要件 5.2 / 5.3。failure detector 方式と同型）
...
- `Option<Duration>`（removal margin）と `Option<LeaseUsageSettings>`（lease スロット）は「未指定」を型で表現し、センチネル値を使わない
...
- 検証エラーは `ClusterSingletonSettingsError` の variant で原因項目を特定可能にし、install 境界で `ActorSystemBuildError::Configuration(format!("{error:?}"))` へ写像する（failure detector と同じ写像形式）
