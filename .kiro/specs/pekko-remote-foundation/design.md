...
- ActorSystem 側: `with_actor_ref_provider` で登録された Provider を、ActorRef 生成・Path 解決・ActorSelection 経路で必ず利用する。ローカル PID が見つからない場合は Provider がリモート authority 判定を行い、`RemoteActorRefSender` を返却する。
