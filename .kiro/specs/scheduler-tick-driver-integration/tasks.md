...
- [ ] 2. Std 自動 Tick ドライバを統合する
  - _(親タスクなので詳細は記載しない)_
  - _対応要件: R1.1, R1.2, R1.3, R1.4, R1.5, R3.4_
  - _依存タスク: 1.1, 1.2, 1.3_
- [ ] 2.1 ホストランタイム自動検出を実装する
  ...
  - _依存タスク: 1.1_
- [ ] 2.2 Tokio ベースの Tick 供給タスクを駆動する
...
- [ ] 3. 組込み向け Tick ドライバ抽象を整備する
  - _(親タスクなので詳細は記載しない)_
  - _対応要件: R2.1, R2.2, R2.3, R2.4, R2.5_
  - _依存タスク: 1.1, 1.2, 1.3_
- [ ] 3.1 ハードウェアドライバ抽象と登録経路を実装する
  ...
  - _対応要件: R2.1, R2.4_
  - _依存タスク: 1.1_
- [ ] 3.2 ISR から Tick フィードへの橋渡しを実装する
...
- [ ] 5.1 Std Quickstart テンプレートとビルダー補助を実装する
  ...
  - _依存タスク: 2.2, 4.1_
- [ ] 5.2 組込み Quickstart テンプレートを実装する
  ...
  - _対応要件: R4.2, R4.6_
  - _依存タスク: 3.2_
- [ ] 5.3 テスト用 Quickstart とドライバ選択マトリクスを提供する
