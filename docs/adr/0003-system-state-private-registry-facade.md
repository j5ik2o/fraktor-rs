...
- 既存 facade の内側に private leaf registry を置く案: 互換性を維持しつつ downstream の spec が参照できる内部境界を作れるため採用。
