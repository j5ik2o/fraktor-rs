...
一方で、残ギャップは compression table application、remote deployment、`AddressTerminated` integration に集中している。remote DeathWatch の watch / unwatch / notification delivery、ACK/NACK redelivery、shutdown / DeathWatch 前 flush lifecycle は接続済みである。
