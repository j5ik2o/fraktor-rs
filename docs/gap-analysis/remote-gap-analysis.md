...
更新日: 2026-04-28 (8th edition / 現行ワークツリー再検証 + B 方針決定反映版)
...
| hard / medium / easy / trivial gap | 10 / 3 / 0 / 0 |
...
固定スコープ概念カバレッジは 82% で、`hard` / `medium` ギャップが 13 件残っている。API / 実動作ギャップがまだ支配的なため、内部モジュール構造の詳細分析は後続フェーズとする。
