...
remote は address primitives、failure detector、association state、wire PDU、TCP transport shell、compression table application、resolve cache、remote `ActorRef` materialization、actor-core serialization registry backed payload の outbound / inbound delivery まで実装済みである。

一方で、残ギャップは remote deployment と `AddressTerminated` integration に集中している。remote DeathWatch の watch / unwatch / notification delivery、ACK/NACK redelivery、shutdown / DeathWatch 前 flush lifecycle は接続済みである。

| 指標 | 値 |
|------|-----|
| Pekko 固定スコープ対象概念 | 75 |
| fraktor-rs 固定スコープ対応概念 | 73 |
| 固定スコープ概念カバレッジ | 73/75 (97.3%) |
| raw Pekko public type declarations | 361（Scala / Java、protobuf 除外） |
| raw Pekko `def` declarations | 1594 |
| raw fraktor public type declarations | 92（`remote-core`: 78 / `remote-adaptor-std`: 14、簡易再計測） |
| raw fraktor public method declarations | 378（`remote-core`: 360 / `remote-adaptor-std`: 18、簡易再計測） |
| hard / medium / easy / trivial gap | 2 / 0 / 0 / 0 |
...
|----|----------------|-----------------|------|
| core | address、unique address、association、wire PDU、compression table、failure detector、watcher state、provider contract、typed config | `modules/remote-core/src/` に整理済み。no_std 側の状態機械、PDU、compression table は揃っている | 公開 primitive は強い。deployment と `AddressTerminated` integration は残る |
...
固定スコープ概念カバレッジは 97.3% で 80% を超えるため、公開 API の残ギャップを実装する上での構造差分も記録する。残る API gap はすべて `hard` であり、次の構造ギャップが実装順序を制約している。
...
remote は address primitives、association state machine、ACK/NACK redelivery、failure detector + registry、typed `RemoteConfig`、TCP transport shell、inbound quarantine、restart handling、compression table application、resolve cache、remote `ActorRef` materialization、remote DeathWatch、shutdown / DeathWatch 前 flush、主要 misc serialization、registered payload の二ノード配送までカバー済みで、基礎部品の parity は進んでいる。

Phase 1 / Phase 2 の低コストギャップは現時点ではない。parity を次に進めるには、Phase 3 の remote deployment または `AddressTerminated` integration を詰める必要がある。

主要ギャップは `AddressTerminated` integration と remote deployment に集中している。API カバレッジは 90% を超えているため、次のボトルネックは単なる型追加ではなく、deployment / actor-core failure notification の接続境界にある。
