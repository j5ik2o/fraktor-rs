...
更新日: 2026-05-16 (19th edition / AddressTerminated 再分類)
...
Pekko 側は `RemoteWatcher` が `AddressTerminatedTopic` へ `AddressTerminated` を publish し、actor-core の `DeathWatch.addressTerminated` が watched actor ごとの `DeathWatchNotification` へ変換する。一方、fraktor-rs 側は topic ではなく `WatcherState::on_tick` が remote node failure から `WatcherEffect::NotifyTerminated` を生成し、std watcher task が `SystemMessage::DeathWatchNotification` を local watcher へ送る。actor-core 側の `ActorCell::handle_death_watch_notification` が通常の DeathWatch termination と同じ処理を行うため、remote DeathWatch の `AddressTerminated` セマンティクスは別経路で実装済みとして再分類する。
...
| `watcher` | 3 | 4 | pure remote watcher state: watch/unwatch tracking, heartbeat response UID tracking, rewatch effect, node failure effects | `RemoteWatcher.scala` | core-owned gap なし。Pekko `AddressTerminatedTopic` 経路は `NotifyTerminated` effect 経路で代替 |
...
`remote-core` production code 上の `todo!()` / `unimplemented!()` / `panic!("not implemented")` は検出されなかった。`modules/remote-core/src/wire/primitives.rs:12` の `placeholder` は frame length を後で埋め戻すためのバッファ位置であり、未実装スタブではない。`provider_test.rs` の stub provider はテスト用 fixture なので parity gap から除外する。
...
前版で未対応とした `AddressTerminated` integration は、remote node failure を watched actor ごとの `DeathWatchNotification` に落とす契約としては実装済みだった。Pekko は `AddressTerminatedTopic` を中継点にするが、fraktor-rs は `WatcherEffect::NotifyTerminated` と std watcher task を中継点にする。
...
|----|----------------|-----------------|------|
| core | address、unique address、association、wire PDU、compression table、failure detector、watcher state、provider contract、typed config、deployment PDU | `modules/remote-core/src/` に整理済み。no_std 側の状態機械、PDU、compression table は揃っている | remote-core-owned gap は見当たらない。remote node failure は `WatcherEffect::NotifyTerminated` として表現済み |
...
| actor-core integration | serialization registry、ActorRefProvider、DeathWatch、event stream、routing/deploy | misc serializer、scheme provider lookup、remote `ActorRef` materialization、routee expansion、registered payload remote send、remote DeathWatch 通知、RemoteScope child create は接続済み | fixed-scope gap なし |
...
`AddressTerminated` については、同名 topic/API は fraktor-rs 側にないが、Pekko の実行時契約である「remote node failure を watched actor ごとの DeathWatch notification に変換する」部分は別経路で実装済みである。Pekko は `RemoteWatcher.reapUnreachable` -> `AddressTerminatedTopic.publish(AddressTerminated)` -> `DeathWatch.addressTerminated` -> `DeathWatchNotification(..., addressTerminated = true)` の経路を取る。fraktor-rs は `WatcherState::on_tick` -> `WatcherEffect::NotifyTerminated` -> std watcher `notify_local_watchers` -> `SystemMessage::DeathWatchNotification` -> `ActorCell::handle_death_watch_notification` の経路を取る。
...
固定スコープ概念カバレッジは 100% で 80% を超えるため、内部構造差分も確認した。現時点で実装を要求する構造ギャップはない。

Pekko は `AddressTerminatedTopic` を actor-core event topic として持ち、remote watcher と DeathWatch の中継点に使う。fraktor-rs は actor-core event topic を露出せず、remote watcher の effect を std watcher task が直接 `SystemMessage::DeathWatchNotification` に変換する。これは同等セマンティクスの別ルートであり、ギャップとしては扱わない。
