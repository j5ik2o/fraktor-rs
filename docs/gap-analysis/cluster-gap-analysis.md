...
| fraktor-rs 固定スコープ対応概念（実装済み） | 99 |
| 固定スコープ概念カバレッジ | 99/151 (66%) |
| 部分実装 | 13 |
| 未対応 | 39 |
...
注: `部分実装` / `未対応` / 難易度内訳は概念単位で数える。カテゴリ9の protocol / CRDT 複合行は複数概念を1行に集約しているため、表上の行数とは一致しない。
...
各カテゴリのヘッダーに **実装済み数 / 対象概念数 (カバレッジ%)** を明記する。ギャップ（未対応・部分実装）のみテーブルに列挙し、実装済みは件数カウントに含めてテーブル行には追加しない。件数は概念単位で数えるため、`LWWRegister` / `LWWMap` や `ORSet` / `ORMap` / `ORMultiMap` のような複合行は表の1行と概念数が一致しない。
