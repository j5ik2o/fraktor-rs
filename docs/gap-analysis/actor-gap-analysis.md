...
- 分析日: 2026-04-17（初版: 2026-04-15、第2版: 2026-04-16、第3版: 2026-04-17、第4版: 2026-04-17、第5版: 2026-04-17）
...
| fraktor-rs 対応実装数 | 96（うち完全実装 95 / 部分実装 1） |
| カバレッジ（型単位） | 96/102 ≈ 94% |
| ギャップ数 | 7（未対応 6 + 部分実装 1、core/kernel: 5, core/typed: 2, std: 0） |
| 部分実装ギャップ | 1（kernel 側 `ConsistentHashingRoutingLogic`。型は存在するが Pekko 互換の挙動が欠落。**本項目は「実装数 96」と「ギャップ数 7」の両方に計上される**ため `96 + 7 ≠ 102` になる点に注意。未対応分は 102 − 96 = 6） |
| n/a 除外数 | 約 61（Java DSL, IO, japi, internal, JVM 固有、`LoggingFilterWithMarker`） |

enumerated gaps (カテゴリ別ギャップから再掲):
- **core/kernel (5)**: `ConsistentHashableEnvelope`、`ConsistentHash<T>` / `MurmurHash` util、`OptimalSizeExploringResizer`、`Listeners` / `Listen` / `Deafen` / `WithListeners`、`ConsistentHashingRoutingLogic`（partial）
...
|----|-------------|------------------|-----------|
| core / untyped kernel | 40 | 36 | 36/40 = 90% |
...
実装済み型 (kernel): `RoutingLogic` trait, `RouterConfig` trait, `Pool` trait, `Group` trait, `Router`, `Routee`, `Broadcast`, `RandomRoutingLogic`, `RoundRobinRoutingLogic`, `ConsistentHashingRoutingLogic`（簡略版）, `SmallestMailboxRoutingLogic`（Pekko 互換完全実装: 2パス探索・`isSuspended`/`isProcessingMessage` 追跡・スコアリング）, `RouterCommand` (GetRoutees/AddRoutee/RemoveRoutee/AdjustPoolSize), `RouterResponse`
...
API ギャップが 94% まで詰まっており、主要カテゴリの致命的欠落がないため、内部構造ギャップも分析対象に含める。
...
- actor モジュールの parity は **約 94%**（96/102 型）に達しており、主要機能は概ねカバー済み。第5版では第4版時点で未対応扱いだった `LoggingFilter` trait、classic `Pool` / `Group` RouterConfig 基盤、`AffinityPool` executor の 3 件がすべて実装済みであることをコード直接参照で確認し再判定した。`LoggingFilterWithMarker` も `LogEvent` の marker フィールドで代替可能なため n/a に再分類。
- **完全カバー済みカテゴリ**（100%）: classic actor core, supervision, typed core surface, receptionist, scheduling/timers, ref/resolution, delivery/pubsub, serialization, extension, coordinated shutdown, pattern, dispatch/mailbox, std adaptor — **13カテゴリ**で完全 parity。
- enumerated gap 計 7 件（kernel 5, typed 2, std 0、うち partial 1）:
  - parity を低コストで前進できる未対応機能（Phase 1 = trivial/easy）:
    ...
    - `Listeners` / `Listen` / `Deafen`（core/kernel, easy）
...
- 次のボトルネックは API 不足そのものよりも、**kernel 側 `ConsistentHashingRoutingLogic` の Pekko 互換挙動差分**（virtual nodes 調整と `hashMapping` 抽象）と、**receptionist / delivery の内部責務の切り方**、および **kernel 層の `pub` 露出過多** にある。Phase 1 の easy 項目を早期に埋めつつ、Phase 2 の medium 項目および構造整理を並行することで、以後の parity 実装速度を維持できる。
