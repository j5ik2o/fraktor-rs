...
| **内部セマンティクスギャップ数 (第17版、MB-M2 完了反映後)** | **4+（high 0 / medium 4 / low 約 11）** — MB-M2 (BoundedDequeBasedMailbox / BoundedControlAwareMailbox) を完了。残存 medium は AC-M2, AC-M4b (remote 依存 / deferred), FS-M1, FS-M2 の 4 件 |
...
| MB-M2 | ✅ **完了 (change `pekko-bounded-deque-control-aware-mailbox`)** — BoundedDequeBasedMailbox / BoundedControlAwareMailbox | `Mailbox.scala:844,931` — 両型あり | `bounded_deque_mailbox_type.rs` / `bounded_deque_message_queue.rs` / `bounded_control_aware_mailbox_type.rs` / `bounded_control_aware_message_queue.rs` を新設。`mailboxes.rs::deque_mailbox_type_from_policy` と新 helper `control_aware_mailbox_type_from_policy` で bounded dispatch を配線。`MailboxConfigError::BoundedWithDeque` / `ControlAwareRequiresUnboundedPolicy` の validate 拒否を撤廃し、stash + bounded / control_aware + bounded を valid 組合せ化。overflow strategy 3 種 (Grow / DropNewest / DropOldest) 全対応、BoundedControlAware の DropOldest は normal queue を優先 evict (design Decision 3) | ~~medium~~ done |
| MB-M3 | ~~Blocking push-timeout 戦略~~ (**第16版で n/a 化**) | ~~`Mailbox.scala:556-566` — `pushTimeOut` 経由で `queue.offer(handle, timeout)` により送信側を block~~ | ~~未実装（非同期 Rust で blocking は設計上不可）~~ | Rust async モデル上、送信側 thread を blocking するのは設計思想と両立しない。Pekko の backpressure 期待値は `OverflowStrategy::DropOldest` / `Fail` 等で代替可能。**意図的に非実装** (design divergence) | ~~medium~~ n/a |
...
**残存 medium 4 件** (第17版時点): AC-M2, AC-M4b (remote 依存 / deferred), FS-M1, FS-M2
