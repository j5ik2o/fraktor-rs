...
| AC-M1 | ✅ **完了 (change `pekko-fault-dispatcher-hardening`)** — PinnedDispatcher の actor 登録排他チェック | `PinnedDispatcher.scala:48-59` — `if ((actor ne null) && actorCell != actor) throw` + unregister で owner = null | `pinned_dispatcher.rs:59-85` に `owner: Option<Pid>` + 3 分岐 match + `SpawnError::DispatcherAlreadyOwned` で実装・テスト済 (`pinned_dispatcher/tests.rs:72-127` の 5 テスト)。本 change では rustdoc を Pekko 行単位で補強 | 閉塞済み (実装・テスト完備、rustdoc で Pekko 行単位対応表を明示) | ~~medium~~ done |
...
| AC-M3 | ✅ **完了 (change `pekko-fault-dispatcher-hardening`)** — FailedFatally / isFailed ガード | `FaultHandling.scala:73-74,215-234` — `isFailed` / `isFailedFatally` + `handleInvokeFailure` 内で `setFailed(self)` + `finally clearFailed()` | `ActorCell::report_failure` 冒頭に `is_failed()` guard + `set_failed(self.pid)` を配線 (Pekko L218-234 行単位写像)。`SystemMessage::Resume` arm に `clear_failed()` を追加 (Pekko L150 相当)。既存 `finish_recreate` L1264 の `clear_failed()` と合わせて Restart / Resume 両 directive で state クリアを保証 | 閉塞済み (既知 divergence 2 件は design.md Decision 5 で受容: Fatal + Resume は production 未到達、Resume propagation の子孫 over-clear は `perpetrator()` production reader ゼロで不可視) | ~~medium~~ done |
