...
| **内部セマンティクスギャップ数 (第21版、現行コード再抽出後)** | **3+（high 0 / medium 3 / low 約 11）** — 公開 API / 内部セマンティクスの未解決件数は第19版から不変。構造面では `receptionist/` runtime 分離と delivery `internal/` 分離が完了済みであることを確認。残存 medium は AC-M4b (remote 依存 / deferred), FS-M1, FS-M2 の 3 件 |
| **内部セマンティクスギャップ数 (第22版、FS-M1 + FS-M2 完了反映後)** | **1+（high 0 / medium 1 / low 約 11）** — change `pekko-fsm-transition-extensions` で FSM `forMax` / `replying` と名前付き timer を完了。残存 medium は AC-M4b (remote 依存 / deferred) の 1 件 |
...
enumerated gaps (内部セマンティクス): 初回検出は **24+ 件**。第22版時点の残存は **medium 1 件 + low 約 11 件** — 詳細は本版末尾の「内部セマンティクスギャップ」セクションを参照。

### カバレッジの読み替え

第7版までの「101/101 = 100%」は **公開型と公開メソッドシグネチャが揃っている** ことを意味するが、**Pekko と同じ契約 (semantics) で動く** ことは保証していなかった。第8版で検出された high / medium の内部セマンティクス不一致は順次閉塞し、第22版時点では high 0、medium 1（AC-M4b）まで縮小している。現時点の実効リスクは、remote / cluster と接続する DeathWatch に絞られている。
...
第7版で API ギャップが **100%** 埋まったため、残る改善余地は **内部モジュール構造の整理** に移った。第21版の現行コード再抽出では、以前の構造ギャップ 3 件のうち `receptionist` と delivery `internal` 分離は完了済みであることを確認した。残る構造ギャップは classic kernel の public surface 縮小のみ。
...
| 完了項目 | 現行コードの根拠 | 判定 |
|----------|------------------|------|
| receptionist の facade / protocol / runtime 分離 | `core/typed/receptionist.rs:3-17` は module wiring と再公開のみ。runtime 実装は `core/typed/receptionist/runtime.rs:1-90` に分離済み | 構造ギャップから除外 |
| typed delivery の `internal` 層分離 | `core/typed/delivery.rs:44-45` で `internal` を明示し、公開 facade は `core/typed/delivery.rs:51-72` の再公開に限定。controller 実装詳細は `core/typed/delivery/internal/` 配下 | 構造ギャップから除外 |
| dispatcher / mailbox の factory API 整理 | `MessageDispatcherFactory` は `core/kernel/dispatch/dispatcher/message_dispatcher_factory.rs:11-17`、`MailboxFactory` は `core/kernel/dispatch/mailbox/mailbox_factory.rs:31-82`。`ActorSystemConfig::with_mailbox` は `MailboxFactory` を直接受け取る | HOCON dynamic loading は引き続き n/a。Rust 側は typed factory 登録で等価責務を満たす |
...
ConsistentHashingRoutingLogic 系は Batch 3 で判定クロージング済み。第21版時点では、構造整理として残っていた `receptionist` / delivery `internal` 分離も完了済み。継続対象は classic kernel の public surface 縮小のみ:
...
| ~~receptionist 実装の `receptionist/` 配下への再配置~~ | ~~core/typed~~ | ✅ 第21版で完了確認 | `core/typed/receptionist.rs` は wiring + 再公開のみ、runtime は `core/typed/receptionist/runtime.rs` に分離済み |
| ~~delivery の `internal` 分離~~ | ~~core/typed~~ | ✅ 第21版で完了確認 | `core/typed/delivery/internal/` が controller 実装詳細を保持し、`delivery.rs` は公開 facade と再公開に限定 |
| classic kernel の public surface 縮小 | core/kernel | 継続 | `core/kernel/**` の内部補助型の `pub(crate)` 化は個別確認が必要。公開 API parity には影響しないが保守性改善として残る |
...
### EventStream / Scheduler / FSM / Stash / Supervision 系（初回 3 high / 5 medium）
...
| FS-M1 | ✅ **完了 (change `pekko-fsm-transition-extensions`)** — FSM の `forMax` / `replying` | `FSM.scala` — `goto(S).forMax(5.seconds)`, `stay().replying(msg)` | `fsm_transition.rs` に `for_max` / `replying` を追加。`machine.rs::apply_for_max_timeout` で state timeout registration を変更せず transient override / cancel を適用し、`dispatch_replies` で `ctx.reply` 失敗を `record_send_error` へ記録 | `forMax(Duration::ZERO)` は cancel に正規化。複数 `replying` は順序保持 | ~~medium~~ done |
| FS-M2 | ✅ **完了 (change `pekko-fsm-transition-extensions`)** — FSM の名前付き arbitrary timer | `FSM.scala` — `setTimer(name, msg, duration, repeat)`, `cancelTimer(name)`, `isTimerActive(name)` | `Fsm::{start_single_timer,start_timer_at_fixed_rate,start_timer_with_fixed_delay,cancel_timer,is_timer_active}` を追加。`fsm_timer_fired.rs` / `fsm_named_timer.rs` と `machine.rs` の generation guard で late arrival を破棄 | 同名再登録は旧 timer を best-effort cancel して新 generation を採番。FSM stop 時は named timer を drain + cancel | ~~medium~~ done |
...
- ✅ FS-M1 (FSM `forMax` / `replying`) — 第22版 / change `pekko-fsm-transition-extensions`
- ✅ FS-M2 (FSM 名前付き arbitrary timer) — 第22版 / change `pekko-fsm-transition-extensions`

**残存 medium 1 件** (第22版時点): AC-M4b (remote 依存 / deferred)
...
- **HOCON dynamic loading 部分は `n/a` 確定**。Pekko の `Dispatchers.configuratorFrom` (`Dispatchers.scala:263-291`) は `cfg.getString("type")` の文字列ベースで `DynamicAccess` (JVM reflection) 経由 configurator を動的生成するが、fraktor-rs は: (a) HOCON パーサ非採用 (no_std-first 方針)、(b) JVM `DynamicAccess` 相当の trait object 動的 instantiation が言語機構として無い、(c) `Dispatchers::register(id, typed_factory)` の typed API が等価責務を果たす、の 3 点から scope outside
- `register_or_update` のセマンティクスは **last-writer-wins (alias を黙って wipe)** とし、builder API (`ActorSystemConfig::with_dispatcher_factory`) の infallible 合成を維持。strict `register` は既存 alias と衝突したら `AliasConflictsWithEntry` を返す
...
第20版以降の actor-core / actor-adaptor-std を再抽出した結果、公開 API カバレッジと残存内部セマンティクス件数に変化はない。更新点は、構造ギャップの closing と factory API 命名の現行化である。

### 確認結果

| 観点 | 現行コードの根拠 | 判定 |
|------|------------------|------|
| receptionist の facade / protocol / runtime 分離 | `modules/actor-core/src/core/typed/receptionist.rs:3-17` は module wiring と再公開のみ。runtime 実装は `modules/actor-core/src/core/typed/receptionist/runtime.rs:1-90` に分離済み | 完了 |
| typed delivery の `internal` 分離 | `modules/actor-core/src/core/typed/delivery.rs:44-45` で `internal` を明示し、`modules/actor-core/src/core/typed/delivery.rs:51-72` は公開型の再公開に限定 | 完了 |
| dispatcher factory API | `modules/actor-core/src/core/kernel/dispatch/dispatcher/message_dispatcher_factory.rs:11-17` に `MessageDispatcherFactory` を定義 | `Configurator` 系の旧名称ではなく、factory 登録として整理済み |
| mailbox factory API | `modules/actor-core/src/core/kernel/dispatch/mailbox/mailbox_factory.rs:31-82` に `MailboxFactory` を定義し、`ActorSystemConfig::with_mailbox` は任意の `MailboxFactory` を受け取る | builder-facing extension point として整理済み |
| delivery controller cascade 削除後の公開面 | `ConsumerController` / `ProducerController` / `WorkPullingProducerController` は公開 facade / alias として残り、内部実装は `delivery/internal/` に退避 | 公開 API parity への退行なし |

### 第22版時点の残存ギャップ

第22版で FS-M1 / FS-M2 は change `pekko-fsm-transition-extensions` により完了した。残存 medium は AC-M4b の 1 件である。

| ID | 種別 | 実装先層 | 状態 |
|----|------|----------|------|
| AC-M4b | DeathWatch の AddressTerminated 購読 | core/kernel + remote/cluster 連携 | remote/cluster transport 完成まで deferred |
| classic kernel public surface | 内部補助型の公開範囲縮小 | core/kernel | 継続的な構造改善 |

## まとめ

- actor モジュールは **主要公開契約リスト上では 101/101 型** に到達しているが、**これは parity 完了を意味しない**。この数値は「表に載せた主要公開契約が存在する」ことだけを示す。
- **第8版で内部セマンティクス観点 34 項目を比較した結果、high 11 / medium 13 / low 約 10 の不一致を検出**。第9版で SP-H1.5 (std panic → Escalate) を閉塞、**第10版で Phase A2+ の high 6 件 (AC-H2 / AC-H3 / AC-H4 / AC-H5 / AL-H1 / ES-H1) を全完了**、**第11版で SP-M1 (maxNrOfRetries 意味反転 + RestartStatistics one-shot window) を完了**、**第12版で MB-M1 (mailbox throughput deadline enforcement) を完了**、**第13版で AC-M5 (NotInfluenceReceiveTimeout marker + Identify 内部封筒化) を完了**、**第14版で AC-M1 (PinnedDispatcher 排他) / AC-M3 (isFailed guard + setFailed perpetrator + Resume clear_failed) を完了**、**第15版で AC-M4a (watchWith 重複チェック) + AL-M1 (post_restart) を完了**、**第16版で MB-M3 を n/a 化 (async Rust 設計上 blocking 非成立) + ES-M1 を low 降格 (性能差のみ)**、**第17版で MB-M2 (BoundedDequeBasedMailbox / BoundedControlAwareMailbox) を完了 (change `pekko-bounded-deque-control-aware-mailbox`) し、`MailboxConfigError::BoundedWithDeque` / `ControlAwareRequiresUnboundedPolicy` variant を削除**、**第18版で AC-M2 (Dispatchers alias chain resolution, `MAX_ALIAS_DEPTH = 20` Pekko parity) を完了 (change `pekko-dispatcher-alias-chain`) し、HOCON dynamic loading 部分は JVM reflection 依存のため `n/a` として scope 確定**、**第19版で DP-M1 (Dispatcher primary id flip) + MB-P1 (Mailbox primary id flip) を完了 (change `pekko-dispatcher-primary-id-alignment`) し、fraktor-rs 独自の legacy 短縮表記 `"default"` を完全退役**。第21版の現行コード再抽出後も残存内部セマンティクスは **high 0 / medium 3 / low 約 11** で不変だったが、**第22版で FS-M1 / FS-M2 を完了**し、残存内部セマンティクスは **high 0 / medium 1 / low 約 11** となった。Pekko互換フェイクを禁じる前提でも、**high レベルの内部セマンティクス gap は閉塞済み**。残り medium は remote/cluster 基盤完成まで deferred の `AC-M4b` 1 件である。
...
- **第19版で閉塞した divergence (DP-M1 + MB-P1)**: 第18版で残されていた primary id の非整合を change `pekko-dispatcher-primary-id-alignment` で解消。kernel の `DEFAULT_DISPATCHER_ID` と `DEFAULT_MAILBOX_ID` は Pekko 原典 (`DefaultDispatcherId` / `DefaultMailboxId`) と同値となり、fraktor-rs 独自の legacy 短縮表記 `"default"` は alias を含め完全退役。typed 層の `REGISTERED_DEFAULT_DISPATCHER_ID` も削除し、値の 2 重定義を廃止。第21版では関連 API 名も `MessageDispatcherFactory` / `MailboxFactory` ベースに整理済みであることを確認。詳細は「内部セマンティクスギャップ / 第19版 更新 (DP-M1 + MB-P1 完了)」および「現行コード再抽出 (第21版)」セクションを参照。
...
- **第22版時点の残存ギャップ**: 公開 API は **0 件**。内部セマンティクスの high は **0 件**、medium は **1 件**（AC-M4b）。low は約 11 件で、主に性能差または実装選択レベル。
- API ギャップと high 内部セマンティクス gap の両方が解消し、receptionist / delivery の構造分離と FSM medium 2 件の閉塞も完了したため、**次のボトルネックは remote/cluster 連携待ちの DeathWatch 1 件**に絞られている。
- **構造面の改善候補**: classic kernel の public surface 縮小（`pub(crate)` 化の継続的推進、medium / medium）。receptionist facade / protocol / runtime の `receptionist/` 配下への再配置と typed delivery の `internal/` 層新設は第21版で完了確認済み。
- 型面の充足と実行時契約の一致は別物であるという第8版以降の立場は維持する。第22版時点では、完全 parity 到達には remote / cluster 基盤完成後の `AC-M4b` 対応が必要である。
