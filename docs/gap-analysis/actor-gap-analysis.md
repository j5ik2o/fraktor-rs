...
- 分析日: 2026-04-19（初版: 2026-04-15、第2版: 2026-04-16、第3版: 2026-04-17、第4版: 2026-04-17、第5版: 2026-04-17、第6版: 2026-04-17、第7版: 2026-04-18、第8版: 2026-04-19、**第8.1版: 2026-04-19** — Phase A1 完了反映）
...
| **内部セマンティクスギャップ数 (第8.1版、Phase A1 完了後)** | **19+（high 6 / medium 13 / low 約 10）** — Phase A1 の 5 件 (MB-H1/H2/H3, AC-H1, SP-H1) が PR #1594 で完了 |
...
| SP-H1 | ✅ **完了 (PR #1594)** — Decider の粒度 (JVM Error → Escalate) | `SupervisorStrategy.scala:defaultDecider` — `Error` (JVM fatal) は必ず `Escalate` | `error/actor_error.rs` — `ActorError::Escalate(ActorErrorReason)` variant + `ActorError::escalate()` コンストラクタを追加。`supervision/base.rs` の 3 deciders (`with_decider::default_decider`, `Default::decider`, `backoff_decide`) で `Escalate => Escalate` arm 追加。`failure_classification.rs` / `failure_payload.rs` も `FailureClassification::Escalate` を round-trip 保持 | Phase A1 で修正。**SP-H1.5 (std adaptor 層での panic 自動 Escalate 配線) は Phase A2+ で対応必須**（skip 対象ではない）。`catch_unwind` は std 依存のため `actor-adaptor-std` 側の `Invoker` で提供する必要がある — 実装しないと std 利用者の panic → Pekko 互換の supervisor escalation が機能しない | ~~high~~ done |
...
>
> 未対応 (Phase A2+ / 別 PR 予定):
> - **SP-H1.5 (対応必須 / std adaptor 層)**: ユーザーハンドラ内 panic の自動 `ActorError::Escalate` 変換。**skip 対象ではなく、Phase A2+ で必ず実装する**。
>   - **変更先**: `modules/actor-adaptor-std/` の `Invoker` 相当 (std 依存アダプタ)。`actor-core` (no_std) には置かない。
>   - **必要性 (std 利用者側で実装しないと起きる実害)**:
>     - ユーザーの `Actor::receive` で panic → mailbox run loop が unwind
>     - `put_lock` / `SpinSyncMutex` が poisoned 状態で残る
>     - 子 actor への `Stop` / `Terminated` 通知が発行されず、supervisor が escalation を受けられない
>     - dispatcher スレッドが死に、他の actor も巻き込みで停止
>     - **Pekko では復旧する定番パターン (panic → supervisor restart) が fraktor-rs の std モードで crash する** — 致命的な互換性欠落
>   - **実装方針**: std adaptor の `Invoker` で `std::panic::catch_unwind(AssertUnwindSafe(|| handler(...)))` を噛ませ、`Err(panic)` を `ActorError::escalate(ActorErrorReason::from_panic(...))` に変換して通常の failure 経路へ合流させる。既存の SP-H1 (Escalate variant + default decider の Escalate arm) の上に配線するだけで済む。
>   - **no_std 利用者の扱い**: `panic = "abort"` コンパイル戦略 (no_std バイナリの既定) または明示的な `ActorError::Escalate` 返却で等価動作。Rust no_std には runtime での panic 捕捉 API が言語として存在しないため、no_std 側は「設計上の制約」で gap ではない。
> - AC-H2: ChildrenContainer の 4 状態 state machine (Normal / Terminating / Terminated + SuspendReason)
> - AC-H3: faultSuspend / faultResume の子への再帰伝播
> - AC-H4: restart 中の子停止完了待ち
> - AC-H5: terminatedQueued 経由の遅延 Terminated delivery
> - AL-H1: post_restart hook + preRestart デフォルト実装
> - ES-H1: EventStream のサブクラス対応 classifier
