...
| MB-M1 | ✅ **完了 (change `pekko-mailbox-throughput-deadline`)** — Throughput deadline | `Mailbox.scala:261-278` — `System.nanoTime` ベースで deadline 超過を検知して即 reschedule | `mailbox/base.rs:run()` で `MailboxClock` (`Arc<dyn Fn() -> Duration + Send + Sync>`) を `MailboxSharedSet` 経由で注入、`deadline_at = clock.zip(throughput_deadline).map(|(c, d)| c() + d)` を run 先頭で 1 回計算、`process_mailbox` ループ内で `invoke → process_all_system_messages → left -= 1 → if clock() >= da break` (Pekko `Mailbox.scala:271-276` 行単位対応)。std adaptor で `Instant::now()` capture 起点の closure を `ActorSystem` 初期化時に install | ~~medium~~ done |
