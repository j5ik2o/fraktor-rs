...
| fraktor-rs 対応実装数 | 93 |
| カバレッジ（型単位） | 93/95 (98%) |
| ギャップ数 | 2（core/kernel: 1, core/typed: 0, std: 1） |
...
| 項目 | 実装先層 | 理由 |
|------|----------|------|
| ~~`NonBlockingBoundedMailbox`~~ | ~~core/kernel~~ | **候補から除外**: `BoundedMessageQueue(DropNewest)` が意味的に等価。名前だけの parity のため追加実装は行わない。|
...
- actor モジュールの parity は **98%** に達しており、前回分析（89%）から大幅に向上した。`PoisonPill`/`Kill` 公開型、`ExtensibleBehavior` trait、`Terminated`/`ChildFailed`/`MessageAdaptionFailure` 独立 signal 型、`CircuitBreakersRegistry` Extension の計7型が実装済みであることを確認した。また `NonBlockingBoundedMailbox` は `BoundedMessageQueue(DropNewest)` が意味的に等価であるため候補から除外した。
- **完全カバー済みカテゴリ**（100%）: classic actor core, supervision, typed core surface, typed routing, receptionist, scheduling/timers, ref/resolution, delivery/pubsub, serialization, extension, coordinated shutdown, pattern, dispatch/mailbox — **13カテゴリ**で完全 parity。
- 残存 API ギャップは **2件**:
...
- Phase 1 は該当項目がなくなり、次の parity 前進は Phase 2 以降の medium / hard 項目に限定される。
