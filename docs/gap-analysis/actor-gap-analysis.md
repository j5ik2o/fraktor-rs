...
> - SP-H1.5: **std adaptor 層**での panic → `ActorError::Escalate` 自動変換配線。actor-core は no_std が基本契約のため `std::panic::catch_unwind` は **core には置けない**。配線する場合は `modules/actor-adaptor-std/` 側の `Invoker` 実装 (std 依存アダプタ) で `catch_unwind` によりユーザーハンドラの panic を捕捉し、`ActorError::escalate(...)` に変換してスーパーバイザへ渡す。no_std 利用者は `panic = "abort"` コンパイル戦略または明示的な `ActorError::Escalate` 返却で等価動作を得る（Rust の no_std では runtime での panic 捕捉 API が言語として存在しないため、これは設計上の制約であり gap ではない）。
