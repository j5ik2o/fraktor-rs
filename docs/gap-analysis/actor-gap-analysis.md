...
- 分析日: 2026-04-21（初版: 2026-04-15、第2版: 2026-04-16、第3版: 2026-04-17、第4版: 2026-04-17、第5版: 2026-04-17、第6版: 2026-04-17、第7版: 2026-04-18、第8版: 2026-04-19、**第8.1版: 2026-04-19** — Phase A1 完了反映、**第9版: 2026-04-21** — InvokeGuard / PanicInvokeGuard による SP-H1.5 完了反映）
...
- **第8版で内部セマンティクス観点 34 項目を比較した結果、high 11 / medium 13 / low 約 10 の不一致を検出**し、第9版時点でも **high 6 / medium 13 / low 約 10** が残る。Pekko互換フェイクを禁じる前提では、actor parity は **未完了** と判定する。
...
- **残存ギャップ**: 公開 API の列挙上は大きな欠落が少ない一方、**主戦場は内部セマンティクス gap**。特に `ChildrenContainer` 状態機械, `faultSuspend/faultResume`, restart 中の子停止完了待ち, `terminatedQueued`, `post_restart` 契約, EventStream classifier が未解決。
- API ギャップが少ないため、次のボトルネックは一部で内部構造にも移っているが、**構造整理だけでは parity にならない**。まずは上記の高優先度セマンティクス gap を潰す必要がある。
- **構造面の改善候補**:
...
- これら 3 項目は parity カウント対象外だが有効な整備候補。ただし、**現時点で actor モジュールを「Pekko と完全同等」とは判定しない**。型面の充足と実行時契約の一致は別物であり、後者が残っている限り parity 未達である。
