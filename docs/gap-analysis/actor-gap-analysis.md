...
| カバレッジ（型単位） | 93/102 ≈ 91% |
| ギャップ数 | 9（core/kernel: 5, core/typed: 3, std: 1） |
...
|----|-------------|------------------|-----------|
| core / untyped kernel | 39 | 34 | 34/39 ≈ 87% |
| core / typed ラッパー | 57 | 54 | 54/57 ≈ 95% |
| std / アダプタ | 6 | 5 | 5/6 ≈ 83% |
| 合計 | 102 | 93 | 93/102 ≈ 91% |
...
| `Logging.Error` / `Logging.Warning` / `Logging.Info` / `Logging.Debug` case classes | `event/Logging.scala` | 部分実装 | core/kernel | trivial | `LogEvent` 列挙で代替済み。独立 case class 相当の公開型が欲しい場合は trivial |
...
| `Router::addRoutee` / `removeRoutee` / dynamic routee management | `Router.scala:L110` | 部分実装 | core/kernel | medium | fraktor-rs の `Router` は静的ルーティーを前提。Pekko の動的 add/remove API は未対応 |
...
| `Logging.Error/Warning/Info/Debug` 相当の独立 case class | core/kernel | 既存 `LogEvent` から薄く公開する wrapper。Pekko API 形式の parity のみの trivial 作業 |
...
| `Router::addRoutee` / `removeRoutee` dynamic management | core/kernel | 動的ルーティー管理 API の追加 |
...
- actor モジュールの parity は **約 90%** に達しており、主要機能は概ねカバー済み。第3版での Pekko 両パッケージ全件再抽出により、第2版では未検出だった ergonomics 系 API（`LoggerOps`）および classic 補助パターン（`Listeners`, `OptimalSizeExploringResizer`, `ConsistentHash` ユーティリティ等）の欠落を新たに検出した。
...
- parity を低コストで前進できる未対応機能（Phase 1 = trivial/easy）:
  - `LoggerOps` 相当の N-arg / 2-arg ロガー拡張（core/typed, easy） — Scala DSL ergonomics の再現
  - `ConsistentHashableEnvelope`（core/kernel, easy） — kernel ConsistentHash 完全化と一体
  - `Logging.Error/Warning/Info/Debug` 独立 case class 相当（core/kernel, trivial）
- parity 上の主要ギャップ（Phase 2-3）:
  - `SmallestMailboxRoutingLogic` / `ConsistentHashingRoutingLogic` の Pekko 互換化（現状は簡略版、`ActorCell` 拡張と `ConsistentHash<T>` / `MurmurHash` 公開 util が必要, Phase 2 medium）
  - `LoggingFilter` / `LoggingFilterWithMarker`（core/kernel, Phase 2 medium）
  - `Listeners` / `Listen` / `Deafen`（core/kernel, Phase 2 easy〜medium）
  - `Router` 動的 addRoutee/removeRoutee（core/kernel, Phase 2 medium）
...
- 次のボトルネックは API 不足そのものよりも、**kernel 側 routing の Pekko 互換挙動差分**（特に SmallestMailbox の suspended/processing 追跡、ConsistentHash の virtual nodes 調整）と、**receptionist / delivery の内部責務の切り方**、および **kernel 層の `pub` 露出過多** にある。Phase 2 の medium 項目を順次埋めつつ、構造整理を並行することで、以後の parity 実装速度を維持できる。
