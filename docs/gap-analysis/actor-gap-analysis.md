...
**残存 medium 4 件** (第18版時点): AC-M4b (remote 依存 / deferred), FS-M1, FS-M2, **DP-M1 (primary id flip / 別 change `pekko-dispatcher-primary-id-alignment` で対応予定)**
...
**第18版で顕在化した継続 divergence (DP-M1 新規)**:

本 change の scope は alias chain resolution に限定したため、**primary entry id が `"default"` のまま** という fraktor-rs 独自の legacy 慣習が維持されている (Pekko 原典では `"pekko.actor.default-dispatcher"` が primary entry id そのもの)。本 change 後の registry 構造:
...
aliases: {
  "pekko.actor.default-dispatcher"    → "default"   (Pekko-compat alias)
  "pekko.actor.internal-dispatcher"   → "default"   (Pekko-compat alias)
}
```

**DP-M1 (Dispatcher primary id alignment)**: Pekko primary id に合わせるには `DEFAULT_DISPATCHER_ID` 定数の値を `"pekko.actor.default-dispatcher"` に flip し、`"default"` を (必要に応じて) 後方互換 alias にする。影響範囲:

- `modules/actor-core/src/core/kernel/dispatch/dispatcher/dispatchers.rs` の `DEFAULT_DISPATCHER_ID` / `DEFAULT_BLOCKING_DISPATCHER_ID` の定数値
- `modules/actor-core/src/core/kernel/actor/actor_cell.rs` の default dispatcher 解決経路
- `modules/actor-core/src/core/kernel/actor/setup/actor_system_config.rs` の seed 登録経路
- std adaptor / tests / examples で `"default"` を直接参照している箇所
- 公開 API 型名 / 定数名は不変 (`DEFAULT_DISPATCHER_ID` 自体の名前は維持、値のみ flip)

**対応予定**: 本 change (`pekko-dispatcher-alias-chain`) マージ後、別 change `pekko-dispatcher-primary-id-alignment` として open。gap-analysis 第19版で done 化する見込み。難易度 medium (破壊的変更大、ただし機械的置換が主)。
...
- **第18版で顕在化した継続 divergence (DP-M1)**: AC-M2 の完了で **Pekko id を alias chain 経由で解決できる** ようにはなったが、kernel primary entry id は依然 fraktor-rs 独自の `"default"` のまま (Pekko 原典では `"pekko.actor.default-dispatcher"` が primary entry id)。本 change の scope は alias chain resolution に限定したため、primary id flip は **別 change `pekko-dispatcher-primary-id-alignment`** として本 PR マージ後に open する。DP-M1 は残存 medium 4 件の 1 つに登録済。詳細は「内部セマンティクスギャップ / 第18版 更新 / DP-M1 新規」セクションを参照。
