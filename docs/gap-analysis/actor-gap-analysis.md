...
| fraktor-rs 固定スコープ対応概念 | 272 |
| 固定スコープ概念カバレッジ | 272/272 (100%) |
...
| raw Rust public type declarations | 644 参考値（kernel: 482, typed: 136, std: 20, embassy: 6。`*_test.rs` 除外。`pub(crate)` 含む） |
| raw Rust public fn declarations | 2,666 参考値（kernel: 2,005, typed: 611, std: 33, embassy: 17。`*_test.rs` 除外。`pub(crate)` 含む） |
...
|----|----------------|-----------------|------|
| kernel | classic actor core, FSM, supervision, DeathWatch, dispatch/mailbox, routing, event, pattern, scheduler, serialization, extension, shutdown, setup | 主要契約は到達可能。FSM 遷移購読 / pipe / CircuitBreaker backoff / BackoffSupervisor / CoordinatedShutdown task 変種 / Setup 類 / `pushTimeOut` 付き bounded mailbox 契約も実装済み | kernel surface は 100% |
| typed | typed ref/system/behavior/interceptor/context, signal, StashBuffer, router, receptionist, pubsub, delivery, ask/StatusReply, timers | `pipe_to_self` / `ctx.ask` / `with_mdc` / `monitor` / `log_messages` / `print_tree` / `ignore_ref` / `DeathPactError` / `ReceptionistSetup` まで確認。スタブ 0 | typed surface は 100% |
...
### classic actor core　✅ 実装済み 34/34 (100%)

`Actor`, `ActorContext`, `Props`, `ActorSystem`, `ExtendedActorSystem`, stash（bounded/unbounded + `StashOverflowError`）, `Timers`（`ClassicTimerScheduler`）, `PoisonPill` / `Kill` / `Identify` / `ActorIdentity` / `ReceiveTimeout` / `NotInfluenceReceiveTimeout` / `Terminated` / `UnhandledMessage`, `Status`, `PossiblyHarmful` marker, FSM 本体（`Fsm<State, Data>` の `when` / `start_with` / `on_transition` / `on_termination` / 名前付きタイマー / `LoggingFsm`）, FSM 遷移購読プロトコル（`FsmCurrentState` / `FsmStateTransition` / `FsmSubscribeTransitionCallback` / `FsmUnsubscribeTransitionCallback`）は kernel に存在する。Pekko 側でも classic `Stash` は `StashOverflowException` を投げるのみで `StashOverflowStrategy` は persistence スコープのため、actor スコープのギャップではない。

### supervision / lifecycle / DeathWatch　✅ 実装済み 13/13 (100%)

`SupervisorStrategy`（OneForOne / AllForOne / Directive）, `RestartLimit` / `RestartStatistics`, `BackoffSupervisor` + `BackoffOnFailureOptions` / `BackoffOnStopOptions`, DeathWatch（watch / unwatch / watch_with / `DeathWatchNotification`）, parent termination completion（`finish_terminate`）, `AddressTerminated` 統合, `DeadLetter`, `DeadLetterSuppression` marker + `SuppressedDeadLetter` 生成経路, `WrappedMessage`, `Dropped`（`DeadLetterReason` variant）は存在する。`AllDeadLetters` は「dead letter ストリームが reason 込みで全種を流す」別設計で対応済みとみなす。

### dispatch / mailbox　✅ 実装済み 35/35 (100%)

`Executor` / `ExecutorFactory`, `Mailbox`, `MailboxType`, `MessageQueue`, `Envelope`, `MailboxPolicy`, `MailboxOverflowStrategy`, `Dispatchers` registry, bounded / unbounded / deque / priority / stable-priority / control-aware の queue family, `MessagePriorityGenerator`, `BalancingDispatcher` は kernel に存在する。`RequiresMessageQueue` / `ProducesMessageQueue` / `MessageQueueSemantics`, `Mailboxes::lookup_by_queue_type`, `MailboxSelection`, `Mailboxes::select`, `BalancingDispatcherFactory::new_checked` / `is_mailbox_compatible` により、queue type 宣言・選択 precedence・balancing 互換チェックも公開契約として到達可能になった。`MailboxPolicy::with_push_timeout` と `MessageQueue::enqueue_with_mailbox_clock` / `DequeMessageQueue::enqueue_first_with_mailbox_clock` により、Pekko `pushTimeOut` に相当する finite/zero timeout bounded enqueue 契約も bounded / priority / stable-priority / deque / control-aware queue family へ配線済み。`NonBlockingBoundedMailbox` は現行 bounded（drop 系戦略）が同等。control-aware bounded の overflow 形状差（MBX-L1: normal queue 優先 evict による control 保護）は同目的の別設計として対応済み扱い。詳細は `docs/gap-analysis/actor-mailbox-gap-analysis.md` を併読。

dispatch / mailbox カテゴリの未対応・部分実装ギャップは残っていない。
...
### event / logging　✅ 実装済み 21/21 (100%)

`EventStream`, subscriber 群, `DeadLetter` 系, `UnhandledMessage`, `LoggingAdapter` / `DiagnosticActorLogging` / `BusLogging` / `LoggingReceive` / `LoggingFilter` / `DefaultLoggingFilter` / `NoLogging` / `LogLevel` / `ActorLogMarker`（marker 対応 LoggingAdapter）は存在する。汎用 EventBus trait 族（`EventBus` / `ActorEventBus` / `ActorClassifier` / `PredicateClassifier` / `LookupClassification` / `SubchannelClassification` / `ScanningClassification` / `ManagedActorClassification`）も `modules/actor-core-kernel/src/event/bus*.rs` に存在し、Pekko `event/EventBus.scala` と同じ分類拡張モデルを user-defined bus の公開契約として提供する。

### pattern　✅ 実装済み 16/16 (100%)

classic `ask`（timeout 込み）, `ActorSelection::ask`, `after`, `pipe_to` / `pipe_to_self`, `retry`, `graceful_stop`（メッセージ指定版含む）, `CircuitBreaker`（`on_open` / `on_close` / `on_half_open`, `with_exponential_backoff`, `with_random_factor` 含む）+ std の `CircuitBreakersRegistry`, `BackoffSupervisor` / `BackoffOpts` 相当, typed `AskPattern` / `StatusReply` / `StatusReplyError` / `ask_with_status` は存在する。`ExplicitAskSupport` は typed の reply-to ファクトリ形 ask が同等。
...
### coordinated shutdown　✅ 実装済み 9/9 (100%)

`CoordinatedShutdown`（8 フェーズ定数 + `CoordinatedShutdownPhase`）, `CoordinatedShutdownReason`（Custom 含む）, installer / id / error, `run` / `shutdown_reason`, `add_task`, `add_cancellable_task`, `add_actor_termination_task` は存在する。

### setup　✅ 実装済み 5/5 (100%)

`Setup` 基底相当, `ActorSystemSetup`, `BootstrapSetup`, `ActorRefResolverSetup`, `ReceptionistSetup` は存在する。
...
`ServiceKey`, `Receptionist` extension, `Register` / `Deregister` / `Subscribe` / `Find`, `Registered` / `Deregistered` / `Listing`（`services_were_added_or_removed` 差分フラグ込み）すべて存在。`ReceptionistSetup` は setup カテゴリに計上。
...
API カバレッジ 100%・hard/medium 未実装 0 件のため、内部モジュール構造ギャップ分析を実施した。
...
該当なし。2026-06-19 時点で easy ギャップは解消済み。

### Phase 2: medium

該当なし。`pushTimeOut` 付き bounded mailbox 契約は 2026-06-19 時点で解消済み。
...
actor モジュールの固定スコープ概念カバレッジは 272/272 (100%) である。前回（2026-05-18）の 114/114 (100%) は粗い概念粒度での判定であり、細粒度で再集計しても未実装・部分実装ギャップは残っていない。スタブ（`todo!` 等）は 4 クレートすべてで 0 件であり、存在する API の実装品質は高い。

低コストの Phase 1 ギャップ、Phase 2 の `pushTimeOut` 付き bounded mailbox 契約、hard 級ギャップはいずれも残っていない。

API ギャップが 0 件まで縮んだ現在、次のボトルネックは公開 API より内部構造に寄っている。構造面では `actor_cell.rs`（1,809 行）の dungeon facet 分離と `system_state.rs`（1,147 + 1,094 行）の分割が、今後の変更速度と保守性を左右する。
