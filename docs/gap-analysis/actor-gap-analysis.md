...
| **内部セマンティクスギャップ数 (第18版、AC-M2 完了反映後)** | **3+（high 0 / medium 3 / low 約 11）** — AC-M2 (Dispatchers alias chain resolution, `MAX_ALIAS_DEPTH = 20` Pekko parity) を完了 (**alias chain 部分のみ parity 達成**)。HOCON dynamic loading 部分 (`type = "..."` 文字列ベースの動的 instantiation) は JVM reflection 依存のため `n/a` として scope 確定 (`register(id, configurator)` の typed API が等価責務)。残存 medium は AC-M4b (remote 依存 / deferred), FS-M1, FS-M2 の 3 件 |
...
**残存 medium 3 件** (第18版時点): AC-M4b (remote 依存 / deferred), FS-M1, FS-M2
...
- 既存 `normalize_dispatcher_id` + hardcoded 2 Pekko id (`pekko.actor.default-dispatcher` / `pekko.actor.internal-dispatcher`) は `ensure_default` 内の alias 自動登録に移行。`normalize_dispatcher_id` 関数は削除、呼び出し側 (`actor_cell.rs`) は `SystemStateShared::canonical_dispatcher_id(id)` 経由に切替
