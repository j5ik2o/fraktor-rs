...
fraktor-rs 側 raw 抽出は `modules/persistence-core-kernel/src/`、`modules/persistence-core-typed/src/`、`modules/persistence-adaptor-std/src/` から `*_test.rs` / `lib_test.rs` を除外して実施した。raw public type declarations は 110 件（kernel: 73、typed: 35、std: 2）、raw public method declarations は 379 件（kernel: 252、typed: 119、std: 8）である。このうち `pub(crate)` を除いた外部到達可能な `pub` type declarations は 96 件（kernel: 69、typed: 25、std: 2）、`pub` method declarations は 335 件（kernel: 224、typed: 104、std: 7）である。
...
| 固定スコープ概念カバレッジ | 69/76 (91%) |
| raw public type declarations | 110（kernel: 73, typed: 35, std: 2） |
| raw public method declarations | 379（kernel: 252, typed: 119, std: 8） |
| externally reachable `pub` type declarations | 96（kernel: 69, typed: 25, std: 2） |
| externally reachable `pub` method declarations | 335（kernel: 224, typed: 104, std: 7） |
...
classic write-side persistence は、persistent actor、journal、snapshot、event adapter、at-least-once delivery、durable state store の基本契約が揃っている。typed write-side は Pekko typed `EventSourcedBehavior` / `DurableStateBehavior` / `Effect` の直移植ではなく、通常の typed `Behavior` を保ったまま hidden child store actor に永続化を委譲する effector-first API として実装されている。

2026-05-28 の再検証では、`origin/main` 取り込み後の現行 crate 境界が `persistence-core-kernel` / `persistence-core-typed` / `persistence-adaptor-std` であることを確認した。Pekko 側 raw extraction は 2026-05-27 版から変化していないが、fraktor-rs 側は std local snapshot store、plugin proxy extension、typed event-sourced signal、published event / rejection error、persist failure backoff hook、state-sourced effector API / runtime が追加された。前回「部分実装」として残っていた adapter manifest と serializer manifest の接続は、`PersistenceContext::to_journal_repr`、`PersistentRepr::adapter_type_id`、`MessageSerializer` の wire encoding で接続済みと判定する。
...
| core / typed | signal, typed recovery / retention | `EventSourcedEffector`, `EventSourcedEffectorConfig`, `EventSourcedEffectorSignal`, `StateSourcedEffector`, `StateSourcedEffectorConfig`, `StateSourcedEffectorSignal`, `EventSourcedSignal`, `PublishedEvent`, `EventRejectedError`, `PersistenceMode`, `SnapshotCriteria`, `RetentionCriteria`, `BackoffConfig`, typed `Recovery`, typed `EventAdapter`, `SnapshotAdapter` を提供 | Event Sourcing と State Sourcing の effector-first API が兄弟 API として揃っている。Pekko direct `EventSourcedBehavior` / `DurableStateBehavior` DSL と public `Effect` DSL は non-goal |
...
|-----------|-----------|-------------|----------|--------|------|
| `RecoveryCompleted` classic public signal type | `references/pekko/persistence/src/main/scala/org/apache/pekko/persistence/PersistentActor.scala:29` | 実装済み / non-goal | core | n/a | classic は `on_recovery_completed` callback と internal `JournalResponseAction::RecoveryCompleted` で表現済み。typed は `EventSourcedEffectorSignal::RecoveryCompleted` と `StateSourcedEffectorSignal::RecoveryCompleted` を公開済み |
...
### 8. Typed write-side effector / signal ✅ 実装済みまたは代替 7/7 (100%)

Pekko persistence の現行推奨 write-side API は typed `EventSourcedBehavior` と `Effect` 体系である。一方、fraktor-rs は専用 `EventSourcedBehavior` DSL ではなく、`EventSourcedEffector` で通常の typed `Behavior` に event-sourced side effect を注入する。`PersistenceId`、effector 経由の persist/snapshot 操作、`RetentionCriteria`、typed recovery selection、typed event/snapshot adapter contract、event-sourced signal、published event、persist failure backoff hook は実装済みまたは明確な代替とみなせる。Pekko direct `EventSourcedBehavior` DSL と public `Effect` / `EffectBuilder` / `ReplyEffect` DSL は effector-first 方針と重複し、callback が次の `Behavior<M>` を返す意味論を維持するため non-goal とする。根拠は `modules/persistence-core-typed/src/event_sourced_effector.rs:62`、`modules/persistence-core-typed/src/event_sourced_effector.rs:212`、`modules/persistence-core-typed/src/event_sourced_effector.rs:242`、`modules/persistence-core-typed/src/event_sourced_effector_config.rs:190`、`modules/persistence-core-typed/src/event_sourced_signal.rs:16`、`modules/persistence-core-typed/src/published_event.rs:13`、`modules/persistence-core-typed/src/event_rejected_error.rs:15`。

| Pekko API | Pekko参照 | fraktor対応 | 実装先層 | 難易度 | 備考 |
|-----------|-----------|-------------|----------|--------|------|
| `EventSourcedBehavior[C,E,S]` | `references/pekko/persistence-typed/src/main/scala/org/apache/pekko/persistence/typed/scaladsl/EventSourcedBehavior.scala:36`, `references/pekko/persistence-typed/src/main/scala/org/apache/pekko/persistence/typed/scaladsl/EventSourcedBehavior.scala:138` | 対象外 / non-goal | core/typed | n/a | fraktor-rs は専用 behavior DSL を導入せず、`modules/persistence-core-typed/src/event_sourced_effector.rs:62` の `EventSourcedEffector::props(config, on_ready)` で通常 `Behavior` と統合する |
| `Effect` / `EffectBuilder` / `ReplyEffect` | `references/pekko/persistence-typed/src/main/scala/org/apache/pekko/persistence/typed/scaladsl/Effect.scala:132`, `references/pekko/persistence-typed/src/main/scala/org/apache/pekko/persistence/typed/scaladsl/Effect.scala:144`, `references/pekko/persistence-typed/src/main/scala/org/apache/pekko/persistence/typed/scaladsl/Effect.scala:196` | 対象外 / non-goal | core/typed | n/a | fraktor-rs は `persist_events(..., \|persisted\| Ok(next_behavior))` のように callback が次の `Behavior<M>` を返す意味論を維持する。public Effect DSL は導入しない |

### 9. Typed state-sourced effector / signal ✅ 実装済みまたは代替 1/1 (100%)

Durable state store contract は kernel に存在し、typed 側には `StateSourcedEffector`、`StateSourcedEffectorConfig`、`StateSourcedEffectorSignal`、`StateSourcedEffectorMessageAdapter` が公開されている。runtime は internal `state_sourced_store_actor` / command / reply で durable state store の `get_object` / `upsert_object` / `delete_object` に接続する。Pekko direct `DurableStateBehavior` DSL と durable state `Effect` / `EffectBuilder` / `ReplyEffect` DSL は effector-first 方針と重複するため non-goal とする。根拠は `modules/persistence-core-typed/src/state_sourced_effector.rs:42`、`modules/persistence-core-typed/src/state_sourced_effector.rs:151`、`modules/persistence-core-typed/src/state_sourced_effector.rs:173`、`modules/persistence-core-typed/src/state_sourced_effector_config.rs:30`、`modules/persistence-core-typed/src/state_sourced_effector_signal.rs:13`、`modules/persistence-core-typed/src/internal/state_sourced_store_actor.rs:67`、`modules/persistence-core-typed/src/internal/state_sourced_store_actor.rs:84`、`modules/persistence-core-typed/src/internal/state_sourced_store_actor.rs:103`。

| Pekko API | Pekko参照 | fraktor対応 | 実装先層 | 難易度 | 備考 |
|-----------|-----------|-------------|----------|--------|------|
| `DurableStateBehavior[C,S]` | `references/pekko/persistence-typed/src/main/scala/org/apache/pekko/persistence/typed/state/scaladsl/DurableStateBehavior.scala:36`, `references/pekko/persistence-typed/src/main/scala/org/apache/pekko/persistence/typed/state/scaladsl/DurableStateBehavior.scala:127` | 対象外 / non-goal | core/typed | n/a | fraktor-rs は専用 durable state behavior DSL を導入せず、State Sourcing 用 `StateSourcedEffector` で通常 `Behavior` と統合する |
| durable state `Effect` / `EffectBuilder` / `ReplyEffect` | `references/pekko/persistence-typed/src/main/scala/org/apache/pekko/persistence/typed/state/scaladsl/Effect.scala:124`, `references/pekko/persistence-typed/src/main/scala/org/apache/pekko/persistence/typed/state/scaladsl/Effect.scala:136`, `references/pekko/persistence-typed/src/main/scala/org/apache/pekko/persistence/typed/state/scaladsl/Effect.scala:188` | 対象外 / non-goal | core/typed | n/a | fraktor-rs は `persist_state(..., \|persisted\| Ok(next_behavior))` のように callback が次の `Behavior<M>` を返す意味論を維持する。public Effect DSL は導入しない |

## 内部モジュール構造ギャップ

固定スコープ概念カバレッジは 69/76 (91%) で 80% を超え、medium gap も 0 件になったため、構造ギャップも確認する。現時点で State Sourcing Effector は `persistence-core-typed` に追加済みで、durable state store との接続は internal state-sourced store actor に集約されている。

| ギャップ名 | Pekko側の根拠 | fraktor-rs側の現状 | 問題の種類 | 推奨アクション | 緊急度 |
|------------|----------------|--------------------|------------|----------------|--------|
| なし | - | State Sourcing runtime は `persistence-core-typed` 内の `StateSourcedEffector` と internal `state_sourced_store_*` に配置済み | - | - | - |

| 構造観点 | 現状 | 次に見るべき点 |
|----------|------|----------------|
| classic と typed の境界 | `persistence-core-kernel` が classic runtime、`persistence-core-typed` が effector-first typed API を担当 | `EventSourcedEffector` と `StateSourcedEffector` は同じ typed crate に配置済み |
| journal / serializer の境界 | `Journal` は `AtomicWrite` を受け、persistence serializers は actor-core serialization registry に contributor として登録される。std local snapshot store は `SnapshotPayload` を serialization registry 経由で保存する | Event Sourcing は journal / snapshot / serializer contract を使い、State Sourcing は durable state store contract を使う |
| durable state revision model | store trait は expected revision と optional tag を受け、tagged update metadata を返す | `StateSourcedEffector` は recovery で revision を受け取り、persist / delete 時に expected revision を渡す |
...
| typed effector と Pekko typed DSL の境界 | `EventSourcedEffector` / `StateSourcedEffector` は通常 `Behavior` に統合される。Pekko direct `EventSourcedBehavior` / `DurableStateBehavior` DSL と public `EffectBuilder` DSL は non-goal | callback が次の `Behavior<M>` を返す意味論を維持した兄弟 API として整理済み |
...
現時点で固定スコープ上の hard gap はない。Pekko direct typed DSL は対象外とし、fraktor-rs 独自の `StateSourcedEffector` は typed crate 内に追加済みである。
...
| Pekko direct `EventSourcedBehavior[C,E,S]` DSL | fraktor-rs は `EventSourcedEffector` を通常の typed `Behavior` に合成する effector-first 方針を採用するため、専用 behavior DSL は重複する |
| Pekko typed `Effect` / `EffectBuilder` / `ReplyEffect` DSL | callback が次の `Behavior<M>` を返す現行 `EventSourcedEffector` API の意味論を維持するため、public Effect DSL は導入しない |
| Pekko direct `DurableStateBehavior[C,S]` DSL | fraktor-rs は State Sourcing 用 `StateSourcedEffector` を通常の typed `Behavior` に合成する方針を採用するため、専用 durable state behavior DSL は重複する |
| Pekko durable state `Effect` / `EffectBuilder` / `ReplyEffect` DSL | callback が次の `Behavior<M>` を返す `StateSourcedEffector` API の意味論を優先するため、public Effect DSL は導入しない |
...
persistence は classic write-side と std snapshot adapter の基礎部品がかなり揃っている。`PersistentActor`、journal、snapshot、event adapter、serializer、at-least-once delivery、durable state store registry、in-memory store、local snapshot store、plugin proxy extension は存在し、panic 系スタブも見つからない。今回の再検証で adapter manifest と serializer manifest の接続、std local snapshot store、plugin proxy runtime semantics、typed event-sourced signal / published event / rejection error、state-sourced effector API / runtime は実装済みに更新した。

parity を低コストで前進させる未実装機能は残っていない。Pekko direct typed DSL を non-goal としたため、固定スコープ上の hard gap も残っていない。

parity 上の主要ギャップは解消済みまたは対象外へ整理済みである。typed write-side は `EventSourcedEffector` / `StateSourcedEffector` の兄弟 API として前進しており、Pekko direct `EventSourcedBehavior` / `DurableStateBehavior` DSL と public `EffectBuilder` DSL は non-goal とする。以後の設計判断は、両 Effector API の「callback が次の `Behavior<M>` を返す」意味論を維持することに絞るのが妥当である。
