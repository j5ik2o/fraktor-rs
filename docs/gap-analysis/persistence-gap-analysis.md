...
| core / typed | signal, typed recovery / retention | `PersistenceEffector`, `PersistenceEffectorConfig`, `PersistenceEffectorSignal`, `EventSourcedSignal`, `PublishedEvent`, `EventRejectedError`, `PersistenceMode`, `SnapshotCriteria`, `RetentionCriteria`, `BackoffConfig`, typed `Recovery`, typed `EventAdapter`, `SnapshotAdapter`, `DurableStateSignal` を提供 | effector-first API は進んでいる。Pekko direct `EventSourcedBehavior` / `DurableStateBehavior` DSL と public `Effect` DSL は non-goal |
...
### 9. Typed state-sourced signal ✅ 実装済みまたは代替 1/1 (100%)

Durable state store contract は kernel に存在し、typed 側には state-sourced runtime 用の `DurableStateSignal` が先行して公開されている。Pekko direct `DurableStateBehavior` DSL と durable state `Effect` / `EffectBuilder` / `ReplyEffect` DSL は effector-first 方針と重複するため non-goal とする。fraktor-rs 側で追加する対象は、Pekko DSL ではなく `StateSourcedEffector` 系の独自 typed API である。根拠は `modules/persistence-core-typed/src/durable_state_signal.rs:13`。
...
固定スコープ概念カバレッジは 69/76 (91%) で 80% を超え、medium gap も 0 件になったため、構造ギャップも確認する。現時点の構造上の主論点は、Pekko direct typed DSL ではなく fraktor-rs 独自の State Sourcing Effector をどこに追加するかである。

| ギャップ名 | Pekko側の根拠 | fraktor-rs側の現状 | 問題の種類 | 推奨アクション | 緊急度 |
|------------|----------------|--------------------|------------|----------------|--------|
| state-sourced effector runtime の配置先未定 | `persistence-typed/.../state/scaladsl/DurableStateBehavior.scala`, `state/scaladsl/Effect.scala` は参考対象 | kernel の `DurableStateStore` と typed `DurableStateSignal` はあるが、typed `Behavior` と durable state store を接続する `StateSourcedEffector` runtime はまだない | fraktor 独自 API の未配置 | `persistence-core-typed` 内に `StateSourcedEffector` 用 submodule を追加し、kernel store trait への adapter を集約する | high |

| 構造観点 | 現状 | 次に見るべき点 |
|----------|------|----------------|
| classic と typed の境界 | `persistence-core-kernel` が classic runtime、`persistence-core-typed` が effector-first typed API を担当 | `StateSourcedEffector` を同じ typed crate に追加するか、別 change に分けるか |
| journal / serializer の境界 | `Journal` は `AtomicWrite` を受け、persistence serializers は actor-core serialization registry に contributor として登録される。std local snapshot store は `SnapshotPayload` を serialization registry 経由で保存する | `StateSourcedEffector` を追加する場合、serializer / adapter contract を event-sourced effector と共有するか |
| durable state revision model | store trait は expected revision と optional tag を受け、tagged update metadata を返す | `StateSourcedEffector` がこの contract をどう実行するか |
...
| typed effector と Pekko typed DSL の境界 | `PersistenceEffector` は通常 `Behavior` に統合される。Pekko direct `EventSourcedBehavior` / `DurableStateBehavior` DSL と public `EffectBuilder` DSL は non-goal | callback が次の `Behavior<M>` を返す意味論を維持したまま `EventSourcedEffector` / `StateSourcedEffector` を兄弟 API として整理するか |
...
現時点で固定スコープ上の hard gap はない。Pekko direct typed DSL は対象外とし、fraktor-rs 独自の `StateSourcedEffector` は別 change で追跡する。
...
| Pekko direct `EventSourcedBehavior[C,E,S]` DSL | fraktor-rs は `PersistenceEffector` を通常の typed `Behavior` に合成する effector-first 方針を採用するため、専用 behavior DSL は重複する |
| Pekko typed `Effect` / `EffectBuilder` / `ReplyEffect` DSL | callback が次の `Behavior<M>` を返す現行 `PersistenceEffector` API の意味論を維持するため、public Effect DSL は導入しない |
...
parity 上の主要ギャップは解消済みまたは対象外へ整理済みである。typed write-side は effector-first API として前進しており、Pekko direct `EventSourcedBehavior` / `DurableStateBehavior` DSL と public `EffectBuilder` DSL は non-goal とする。以後の設計判断は、現行 `PersistenceEffector` API の「callback が次の `Behavior<M>` を返す」意味論を維持したまま、`EventSourcedEffector` / `StateSourcedEffector` を兄弟 API として整理することに絞るのが妥当である。
