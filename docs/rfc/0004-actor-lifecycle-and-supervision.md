...
| OQ-SUP-4 | Pekko には「子の全滅待ち中に届く `Recreate` / `Suspend` / `Resume` / `Failed` を stash して状態復帰後に再処理する」system message stash 機構がある（RFC pekko-0004 PSUP-2）が、fraktor に相当機構は見当たらない | 待機中に届く制御メッセージの順序は現実装でどう保証されているか、stash 相当が必要か | 再帰的な失敗・再起動が重なった際の制御メッセージ順序 |
| OQ-SUP-5 | Pekko は `Failed` に uid を載せ、古い incarnation の子からの遅延 Failed を照合破棄する（RFC pekko-0004 PSUP-7）。fraktor の `FailurePayload` に相当する照合は明示されていない | restart 直後に届く旧世代の Failure が新世代の統計・戦略を汚染しないか | supervision 統計の正確性 |
| OQ-SUP-6 | Pekko の `suspendChildren` は失敗元の子を exceptFor で除外する（既に suspend 済みのため）。fraktor は全子へ送信する | 失敗元の子が二重 suspend されるか（suspend カウンタがネスト対応なら無害だが、resume 回数との対応は取れているか） | suspend/resume カウンタの均衡 |
