...
- **共有ラッパーは `XyzShared`構造体**
  ...
  - 外向き API は個別メソッドを生やさず、基本は `with_mut(|xyz| { ... })`（必要なら `with_ref`）のクロージャ実行を提供してロックを隠蔽する。
...
- **管理責務を持つ場合は `XyzHandle`構造体**
  ...
  - `Handle` も基本は `with_mut`/`with_ref` を提供し、複合操作をまとめて実行する。
...
impl<TB: RuntimeToolbox> XyzShared<TB> {
    pub fn with_mut<R>(&self, f: impl FnOnce(&mut Xyz) -> R) -> R {
        let mut guard = self.inner.lock();
        f(&mut guard)
    }

    pub fn with_ref<R>(&self, f: impl FnOnce(&Xyz) -> R) -> R {
...
// 呼び出し側例
xyz_shared.with_mut(|x| x.do_something(arg))?;
let snap = xyz_shared.with_ref(|x| x.snapshot());
```
...
2. `XyzShared` / `XyzHandle` は `with_mut` / `with_ref` だけを基本 API にし、個別メソッド乱立を避ける。複雑な処理が入っていれば `Xyz` へ移す。
