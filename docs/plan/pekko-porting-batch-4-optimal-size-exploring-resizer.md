...
| `.agents/rules/rust/cqs-principle.md` | `is_time_for_resize` は Query。`report_message_count` は Command（`()` 返却）。`resize` は delta を返しつつ探索 RNG を進めるため厳密には CQS 違反となるが、`Resizer` trait の `&self` 契約と Pekko 互換を優先した限定例外として扱う（Pekko 原典 `OptimalSizeExploringResizer.scala:268-285` と同一セマンティクス） |
