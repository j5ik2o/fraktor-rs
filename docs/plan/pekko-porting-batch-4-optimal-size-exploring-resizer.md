...
typed 側の `PoolRouter::withResizer` から `Resizer` を経由して利用する構造を取る。
fraktor-rs では `Resizer` trait が typed routing DSL (`core/typed/dsl/routing/`)
に集約されているため、本バッチは **typed 側に単独で新設** する。
...
1. **自動最適化**: ユーザーは `lowerBound` / `upperBound` と少数の探索パラメータを指定するだけ。
   実行時にスループット（単位メッセージあたりの処理時間）を観測し、プールサイズを自動調整する。
2. **3 アクション周期**: `actionInterval` を経過するごとに `downsize` / `explore` / `optimize`
   のいずれか 1 つを選択して実行する。
3. **フル稼働時のみメトリクス蓄積**: 全 routee が稼働中（mailbox 非空）の時だけ
   `performanceLog` を更新し、アイドル時の計測の歪みを避ける。
4. **長期未活用時の縮小**: `downsizeAfterUnderutilizedFor` の期間フル稼働していなかった場合、
...
これらは「契約意図」ではなく「JVM / Scala で実現するときの実装詳細」である。
Rust / no_std / `immutability-policy.md` 準拠の下では、いくつかは採用不能、または別の形で
翻訳する必要がある。
...
| `trait OptimalSizeExploringResizer extends Resizer` の 2 段構成 | **翻訳（1 段に集約）** | Scala では trait extraction に意味があるが、fraktor-rs では実装が 1 件のみのため単一の struct にまとめる。公開型は `OptimalSizeExploringResizer` のみ |
...
| デフォルト値 10 件（`lowerBound=1`, `upperBound=30`, `chanceOfScalingDownWhenFull=0.2`, `actionInterval=5s`, `optimization-range=16`, `exploreStepSize=0.1`, `downsizeRatio=0.8`, `downsizeAfterUnderutilizedFor=72h`, `explorationProbability=0.4`, `weightOfLatestMetric=0.5`）| **採用（そのまま）** | Pekko 実装が長年運用してきた閾値のセット。独自の調整はエビデンスなしに避ける |
| パラメータバリデーション（`lowerBound > 0`、`upperBound >= lowerBound`、確率 ∈ `[0,1]`、`optimization-range >= 2` など）| **翻訳** | Pekko は `IllegalArgumentException` を throw する。Rust では `assert!` で panic する（`DefaultResizer::new` の先行事例に揃える） |
| `reportMessageCount` メソッドの追加 | **翻訳（default no-op）** | Pekko はサブトレイト追加だが、Rust では `Resizer` trait 本体に `fn report_message_count(&self, _mailbox_sizes: &[usize], _counter: u64) {}` を default 実装付きで追加する。`DefaultResizer` は default を継承するため挙動は不変 |
| `isTimeForResize(messageCounter)` 署名 | **翻訳** | 現行の `is_time_for_resize(&self, u64) -> bool` を維持する。`message_counter` は `DefaultResizer` 側で必要なため署名を揃える。本 resizer は内部で `Clock` から経過時間を取って判定する |
| `resize(currentRoutees: IndexedSeq[Routee])` 署名 | **翻訳（破壊的変更）** | 現行の `resize(&self, usize) -> i32` を `resize(&self, mailbox_sizes: &[usize]) -> i32` に変更する。`DefaultResizer` は `.len()` のみ使うため挙動は不変。`OptimalSizeExploringResizer` は全要素を利用する。後方互換不要の方針で容認 |
| `LocalDateTime.now` / `System.nanoTime()` | **翻訳（Clock DI）** | `OptimalSizeExploringResizer<C: Clock>` でジェネリクス化する（`CircuitBreaker<C: Clock>` に揃える）。`Clock::now()` と `Clock::elapsed_since(Instant)` を用いて経過時間を算出する |
| `checkTime = 0L` sentinel（Pekko は nanoTime が必ず正である前提で sentinel として 0 を使う）| **翻訳** | `Clock::Instant` は抽象型で「0」に相当する値が自明でないため、`ResizeRecord` に `has_recorded: bool` を追加し `check_time` は初期化時に `clock.now()` を入れる。Pekko の `checkTime > 0` ガードは `has_recorded && under_utilization_streak.is_none()` に対応する |
| `ThreadLocalRandom` | **翻訳（seedable LCG）** | `pool_router.rs:326` / `group_router.rs:211` で既に使っている `6_364_136_223_846_793_005`/`1_442_695_040_888_963_407`（Numerical Recipes MMIX）定数の LCG を `Lcg` 型に切り出し、`OptimalSizeExploringResizer::new(.., seed: u64)` で seed を指定可能にする。決定的なテストが可能 |
| `var performanceLog` / `var record` / `var stopExploring` | **翻訳（AShared）** | `SpinSyncMutex<State<I>>` を struct に持ち、`Resizer::resize` / `report_message_count` は `&self` 契約を維持する（trait 契約を順守）。`stopExploring` は Pekko でも `private[routing]` で外部非公開かつ初期値 `false` のまま本 Batch では利用しないため **フィールド自体を持たない**（YAGNI） |
...
| `PerformanceLog = Map[PoolSize, Duration]` type alias | **翻訳（型はそのまま、alias は非公開）** | `alloc::collections::BTreeMap<usize, core::time::Duration>` を `State` の内部に保持する。公開 type alias は提供しない（`HashMap` ではなく `BTreeMap` を選んだ理由は no_std 対応と、`filter { size >= left && size <= right }` が ordered walk で書ける点） |
| `UnderUtilizationStreak` | **翻訳** | `pub(crate) struct UnderUtilizationStreak<I> { start: I, highest_utilization: usize }` として独立ファイルに配置する（`type-organization.md` 条件 c を満たさないため独立）。`pub(crate)` に留めるのは外部公開すべき API ではないため |
| `ResizeRecord` | **翻訳** | 同上、独立ファイル。Pekko の 4 フィールド + `has_recorded: bool` の 5 フィールド |
| `Lcg`（seedable RNG） | **新設（独立ファイル）** | `pool_router.rs` / `group_router.rs` の LCG ロジックを Batch 4 で本 resizer 用に切り出す。将来 resizer 以外でも共通化する余地はあるが、本バッチでは `optimal_size_exploring_resizer/` 配下の private なユーティリティに留める（YAGNI） |
| `State<I>` struct | **新設（独立ファイル）** | `SpinSyncMutex<State<I>>` で保護する mutable な bookkeeping。`performance_log` / `record` / `rng` を束ねる。`pub(crate)` で親モジュールからのみアクセスする |
| `PoolSize = Int` type alias | **非採用** | Rust では `usize` を直接使用すれば可読性は十分 |
| `@SerialVersionUID(1L)` | **非採用** | JVM のシリアライズ固有 |
| `apply(resizerCfg: Config)` HOCON ファクトリ | **非採用** | fraktor-rs は typed DSL の builder パターンで設定を受ける。HOCON は存在しない |
| `stopExploring` の public setter | **非採用（YAGNI）** | Pekko でも `private[routing]` で外部非公開、かつ実装内でも `checkTime = 0L` ブランチの内側でしか使われていない。本バッチでは `reportMessageCount` 側の「サンプリング停止」を `has_recorded` の更新停止では表現せず、**将来の拡張が必要になった時点で追加**する |
| kernel 側への `Resizer` trait 移設 | **非採用** | Pekko の classic/typed 2 段構成は Scala の歴史的な理由（classic → typed の移行期）に由来する。fraktor-rs では typed に集約済み。kernel 側に 1 件だけ trait を追加すると「Pekko に似せること自体が目的化する」ことに該当する。gap-analysis の "core/kernel" ラベルは Pekko 側のレイヤ表記であり、fraktor-rs 側の移植先を縛らない |
...
`pub(crate)` の型群は **公開型ではないため** `type-per-file-lint` の義務の対象外だが、
将来の共通化・差し替え・テスト容易性のため、すべて独立ファイルに配置した。主型ファイルは
...
- `resize` は破壊的変更（`usize` → `&[usize]`）。`DefaultResizer` は `.len()` のみ使うため挙動は不変。
- `report_message_count` は default no-op で追加する。`DefaultResizer` は override しないため影響はゼロ。
...
- Pekko `ResizablePoolCell.sendMessage` は `isTimeForResize` が真のときにだけ
  ...
  `is_time_for_resize` → `report_message_count` → `resize` を呼び、同一のスナップショットを共有する。
- この順序に従う必要がある: `report_message_count` の内部で `check_time = now` が更新されるため、
  先に呼んでしまうと直後の `is_time_for_resize` の経過時間判定が常にゼロ付近となり resize が発火しない。
  resize tick 内でのみ呼ぶ仕様も Pekko 原典と一致する。
- `is_time_for_resize` が false のときは mailbox のスナップショットを取得しないため、非リサイズ経路の
  メッセージあたりのオーバーヘッドは `is_time_for_resize` の呼び出し 1 回のみ（`DefaultResizer` は modulo 比較、
...
- `observe_routee_mailbox_sizes` は `smallest_mailbox_routing_logic::observe_actor_ref`
  のロジックを流用する。mailbox が取れない（routee が既に停止している等）場合は `0` で埋める。
  unreachable な routee には traffic を呼び込まないのが Pekko の挙動であり、Pekko-parity である。

### 内部状態と AShared パターン

`OptimalSizeExploringResizer<C>` 本体は immutable な config（10 params + `clock` + `state` mutex）を
`&self` メソッドで提供する。mutable な state は `SpinSyncMutex<State<C::Instant>>` に格納し、
`Resizer::resize` / `report_message_count` の内部でのみ `lock()` する。

これは `immutability-policy.md` の「薄い同期ラッパーは `*Shared`」規約からの軽微な逸脱に該当するが:

- `DefaultResizer`（同一 trait の既存実装）が stateless な `&self` で公開されており、
  `OptimalSizeExploringResizerShared` のような 2 段構成にするとユーザー面の整合性が崩れる
- `Resizer` trait の契約が `&self` を要求しており、`OptimalSizeExploringResizerShared` を作っても
  `Resizer for OptimalSizeExploringResizerShared` に委譲するだけで API としては等価
- `SpinSyncMutex` は struct 内の field として閉じ込められ、外部にロックを返さない
...
Pekko の `checkTime = 0L` sentinel は JVM の `System.nanoTime()` が必ず正値である前提に依存する。
...
`core::f64::ceil` / `floor` は `std` を要求する。本 crate は `no_std` を維持するため、主型ファイルの末尾に
整数キャスト経由の簡易実装を private helper として持つ。範囲は `usize * f64` でしか使われず、
64bit 整数にキャストできる領域に限定されているため、`truncated = x as i64 as f64` → 境界調整で十分である。
...
| `.agents/rules/rust/cqs-principle.md` | `is_time_for_resize` は Query。`report_message_count` は Command（`()` を返却）。`resize` は delta を返しつつ探索 RNG を進めるため厳密には CQS 違反となるが、`Resizer` trait の `&self` 契約と Pekko 互換を優先した限定的な例外として扱う（Pekko 原典 `OptimalSizeExploringResizer.scala:268-285` と同一セマンティクス） |
| `CLAUDE.md` | 後方互換不要の方針で `resize` の破壊的変更を容認。計画ドキュメントは `docs/plan/` 配下（本文書）。TAKT ムーブメント中は `final-ci` 以外で `ci-check.sh ai all` を走らせない ✅ |
...
- `modules/actor-core/src/core/typed/dsl/routing/resizer.rs`（既存編集、`resize` の署名変更 + `report_message_count` の default no-op 追加）
- `modules/actor-core/src/core/typed/dsl/routing/default_resizer.rs`（既存編集、`resize` の署名追従。挙動は不変）
- `modules/actor-core/src/core/typed/dsl/routing/pool_router.rs`（既存編集、`observe_routee_mailbox_sizes` helper + `report_message_count` の呼び出し + `resize(&[usize])` の配線）
...
- `modules/actor-core/src/core/typed/dsl/routing/pool_router/tests.rs`（write_tests ステップで smoke test を 1 件追加）
- `modules/actor-core/src/core/typed/dsl/routing/default_resizer/tests.rs`（implement ステップで `resize` の新署名への追従のみ、挙動は不変）
...
- `docs/gap-analysis/actor-gap-analysis.md` を第 7 版に更新
  ...
  - typed `OptimalSizeExploringResizer` の expose（core/typed）を「実装済み」に昇格
...
| 項目 | 理由 |
|------|------|
| kernel 側 `Resizer` trait 新設 | §「採用した判定結果」で非採用と判定済み。「Pekko に似せること自体の目的化」に該当 |
| `apply(Config)` HOCON ファクトリ | 非採用と判定済み。fraktor-rs には HOCON がない |
| `stopExploring` の公開 setter | YAGNI と判定。Pekko でも外部非公開 |
| receptionist facade / protocol / runtime の再配置（Phase 2 medium）| Batch 5 以降の構造整理フェーズで扱う |
| typed delivery の `internal/` 層新設（Phase 2 medium） | 同上 |
| classic kernel の `pub` 露出の縮小（Phase 2 medium） | 同上 |

## 未来の判定変更トリガ

以下のいずれかが発生した場合は再判定を行うこと。

1. `stopExploring` による探索の打ち切りを外部から制御する要件が発生 → public setter の追加を再検討する
2. `Resizer` trait の責務を kernel に移設する別件の要件が発生 → trait の配置を再検討する
3. `SpinSyncMutex` 保護下の state へのアクセスが性能ボトルネックとなる計測が出た → `AShared` パターンへの分離、もしくは lock-free データ構造への移行を再検討する
4. Pekko `DefaultOptimalSizeExploringResizer` の契約 1〜5 や 10 チューニングパラメータのデフォルト値が変化した → 契約への追随で実装方針を再検討する
