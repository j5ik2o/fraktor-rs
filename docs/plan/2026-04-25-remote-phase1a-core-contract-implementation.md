...
対象は `ThrowableNotSerializableException` 相当、`FailureDetectorWithAddress`、`DeadlineFailureDetector`、`RemoteLogMarker` の 4 件に限定する。
...
| `FailureDetectorWithAddress` | `remote-core` failure_detector | address を後から設定できる trait として公開する |
| `PhiAccrualFailureDetector` address 対応 | `remote-core` failure_detector | 既存の phi 計算を変えず、監視先 address の保持だけを追加する |
...
3. `remote-core` failure detector の trait、Phi address 対応、deadline detector を追加する。
