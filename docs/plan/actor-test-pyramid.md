...
| E2E | `modules/actor-core/tests/*_e2e.rs` / `modules/actor-adaptor-std/tests/*_e2e.rs` / `tests/e2e/tests/*.rs` | public API だけを使い、spawn → send / ask → watch → stop → terminate → observable event のようなユーザー操作に近い流れを検証する。crate-local で閉じるものは各クレート、複数クレートをまたぐものは `tests/e2e` に置く。 |
...
| E2E 候補 | `actor_path_e2e`, `actor_selection_e2e`, `classic_user_flow_e2e`, `typed_user_flow_e2e`, `std_adaptor_boot_e2e`, `actor_runtime_boot`。今後の Pekko 由来 E2E は crate-local / cross-crate の境界で配置を決める。 |
...
| 用途 | 配置 |
|------|------|
| actor-core runtime helper | `modules/actor-core/tests/support.rs` から明示的に module wiring |
| actor-adaptor-std helper | `modules/actor-adaptor-std/tests/support.rs` から明示的に module wiring |
