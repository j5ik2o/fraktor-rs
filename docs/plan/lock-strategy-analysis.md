...
1. **close correctness が outer lock 削減より先** → **完了** (`mailbox-close-reject-enqueue`)
   - `enqueue_envelope` / `prepend_user_messages` が lock 内で `is_closed()` を再チェック
...
5. **`lock-driver-port-adapter` は基盤完了** (`new-lock-driver-port-adapter`)
   - `LockDriver<T>` / `RwLockDriver<T>` trait 導入済み
   - `DefaultMutex<T>` compile-time 切替済み (debug-locks / std-locks / default)
   - 残: actor-core hot path の factory genericization
6. **Phase IV (outer lock reduction) の前提条件は全て充足** — 案 a2 (put_lock 限定化) が安全に採用可能

### 最終結論

本ドキュメントの現時点 (2026-04-13) の結論:

1. **fraktor-rs の Mailbox は参照実装から大きく乖離している** (lock-based vs lock-free, 2〜3 段ロック vs atomic) — この事実は変わらない
2. **outer lock 削減の前提条件 (close correctness, stash deque requirement, prepend deque-only 契約) は全て充足済み** — Phase II / III / III.5 が完了した
3. **ロックバックエンドの compile-time 切替基盤は完成** — `DefaultMutex` type alias + `std-locks` feature により、std 環境では `std::sync::Mutex` ベースに自動切替される
...
| change | Phase | 概要 |
|---|---|---|
| `mailbox-close-reject-enqueue` | II | lock 内 is_closed 再チェック、close correctness 回復 |
| `stash-requires-deque-mailbox` | III | MailboxRequirement::for_stash() 伝播 |
| `mailbox-prepend-requires-deque` | III.5 | prepend を deque-only 契約に硬化、drain_and_requeue 削除 |
| `compile-time-lock-backend-selection` | V 基盤 | DefaultMutex/DefaultRwLock type alias、debug-locks feature |
| `new-lock-driver-port-adapter` | V 設計 | LockDriver/RwLockDriver trait、LockDriverFactory 導入 |
| `utils-sync-collapse` | V 関連 | utils-core sync モジュール整理 |
| `shared-lock-consolidation` | V 関連 | SharedLock/SharedRwLock 統合 |
| `eliminate-direct-spin-sync-construction` | V 関連 | SpinSyncMutex 直接構築の撲滅 |
| `decompose-actor-shared-factories` | V 関連 | Shared 型ファクトリの分解 |
| `remove-shared-wrapper-factories` | V 関連 | 不要な Shared ラッパーファクトリ削除 |

### 進行中

| change | Phase | 概要 |
|---|---|---|
| `std-locks-feature` | VI (一部) | StdSyncMutex/StdSyncRwLock を utils-core に新設、std 環境で DefaultMutex = StdSyncMutex に切替 |

### 未着手 (次のアクション候補)

| Phase | 概要 | 優先度 |
|---|---|---|
| **IV: outer lock reduction** | 通常 enqueue/dequeue から user_queue_lock を外す。前提条件は全て充足済み | **高** |
| **V: factory genericization 完遂** | actor-core hot path 15 箇所の factory ジェネリック化 | **高** |
| VI: parking_lot driver | utils-adaptor-std に parking_lot ベースの driver を追加 | 中 |
| VII: queue lock-free 化 | MessageQueue 実装を lock-free queue ベースに差替 | 低 (将来) |
| VIII: BalancingDispatcher close | shared queue の close semantics | 低 (将来) |
| IX: stream-core 調査 | SinkQueue/SourceQueue/Hub の同型問題 | 低 (将来) |
...
- **2026-04-13**: 現状コードに合わせて全体を更新。Phase II / III / III.5 の完了を反映。`compile-time-lock-backend-selection` / `std-locks-feature` / `new-lock-driver-port-adapter` 等の完了済み change を記録。E.7.4 / E.8 の Phase ステータスを更新。次のアクション候補を Phase IV (outer lock reduction) と Phase V (factory genericization) に集約。
