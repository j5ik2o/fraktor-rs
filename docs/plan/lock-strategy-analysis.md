...
**作成日**: 2026-04-08
**最終更新**: 2026-04-13
**ステータス**: Phase I 〜 III.5 完了、Phase IV (outer lock reduction) と Phase V (factory genericization) が次の高優先度
**関連 openspec change**: `openspec/changes/archive/2026-04-13-new-lock-driver-port-adapter/`
**関連 PR**: #1530 (utils-sync-collapse), #1535, #1537, #1538, #1568 (std-locks-feature)

## 背景

`DebugSpinSyncMutex` による deadlock 検知を実用化する過程で、単純な `RuntimeMutex` の Port/Adapter 化では済まない複数の設計課題が浮上した。本ドキュメントは、浮上した課題を整理し、優先順位と依存関係を視覚化して、次のアクションを決めるための意思決定の材料を残す。

## 浮上している 3 つの課題

### 課題 1: ロック機構そのものの選定

現在の fraktor-rs は `utils-core` の `SpinSyncMutex<T>` (= `spin::Mutex<T>` の薄いラッパー) を `RuntimeMutex<T>` として全面採用している。これは **no_std 環境では妥当** だが、実際の呼び出し元の環境ごとに適切性が異なる:
...
`tokio::runtime::Builder::new_current_thread()` の single-threaded runtime では、**1 本の worker 上で自己 deadlock** する可能性もある (SpinMutex の再入不可 + async が混ざった場合)。
...
`prepend_user_messages` の実装から推測すると、外側の `user_queue_lock` は compound op (`number_of_messages() → overflow check → enqueue`) を atomic 化するために存在する:
...
| 選択肢 | 実現方法 | 問題 |
|---|---|---|
| **A** | `MessageQueue` trait に `try_enqueue_with_capacity_check` 等の compound op を追加し、各 impl 内で atomic 化する | trait API が太る、各 impl で内部 mutex を重複して扱う |
| **B** | `user_queue_lock` を `put_lock: RuntimeMutex<()>` に改名し、**prepend / bounded capacity check の時だけ** 取る。通常の enqueue では取らない | Pekko 相当、最小変更、要再設計 |
| **C** | 外側と内側を統合: Mailbox 本体で `RuntimeMutex<MessageQueue>` 相当を持ち、内部の queue 型には mutex を持たせない | 大規模なリファクタ |
| **D** | lock-free MPSC queue (crossbeam / heapless) に切り替えて両方撤廃する | no_std 制約の検証が必要、lock-free は複雑 |
...
この change 自体は (1) と (2) の **測定・検証基盤** としても機能する。driver を swap できれば、parking_lot と spin の実測比較や、lock-free queue への段階的移行の土台になる。
...
- **(3) Port/Adapter は (1) と (2) の両方の「測定・検証基盤」になる** — driver を swap できれば実測データが取れる
- **(2) 二重ロックは (3) と独立に修正可能** — `MessageQueue` trait を拡張すれば外側の `user_queue_lock: RuntimeMutex<()>` を撤廃できる可能性がある
- **(1) は本当は (2) と (3) の結果を見てから判断すべきデータ駆動の選択** — 現時点で理論だけで決めると後悔する
...
- deadlock 検知という主目的を早く達成できる
- 測定基盤が先にできる

**欠点**:
- 115 file の migration を先に被る
- 二重ロックが削れることが後で分かった場合、Port 対象の一部 (`user_queue_lock: RuntimeMutex<()>`) は無駄になる
- (1) の判断がまだなのに、呼び出し元の境界で `SpinSyncFactory` を固定する必要がある
...
**利点**:
- Port 対象の呼び出し元の数が減る (二重ロックが先に消えるため)
...
- 二重ロックも lock 選定も回避できる (Mailbox については)

**欠点**:
- no_std 制約 (crossbeam は std のみ)、代替ライブラリ (heapless, bbqueue) の調査が必要
- lock-free データ構造のデバッグは困難
- Mailbox 以外の shared state (`ActorCell`, `SystemState` 等) では依然ロックが必要
- スコープが巨大
...
2. **調査コストが極めて低い** (コードを読むだけ)。成果物は本ドキュメントの延長で、直接レビュー可能
3. **調査結果が出たら α/β/δ のどれに進むか自然に決まる**:
   - 二重ロックが削れる → β (まず削って Port 対象を減らす)
   - 二重ロックが削れない → α (Port を先にして測定基盤を整える)
...
`modules/actor-core/src/core/kernel/dispatch/mailbox/base.rs` のソースを読解した結果、`user_queue_lock: ArcShared<RuntimeMutex<()>>` は以下の 7 箇所で取得されている:
...
**観察**: 7 箇所のうち、compound op は 3 箇所 (#2, #3, #5) のみ。残る 4 箇所 (#1, #4, #6, #7) は **単純な put/pop/read** であり、外側ロックは本来不要。

#### A.2 実際に走るロックの段数 (BoundedMessageQueue の場合)

`enqueue_envelope` で `BoundedMessageQueue` を使う呼び出し元が enqueue するとき、実際に取られる mutex をトレースした結果、**3 段のネストしたロック** になっていた:
...
**事実**: `BoundedMessageQueue` 経路の 1 回の enqueue で **3 段の SpinSyncMutex を順次取得している**。うち少なくとも LOCK 1 と LOCK 3 は代替手段なしに同時に取得される。LOCK 2 も `QueueState` 内の `AtomicUsize size` と `WaitQueue` をまとめて扱うために保持されている。
...
**2 段** (LOCK 1 + LOCK 2)。`VecDeque::push_back` が 1 回走るだけの単純な op に 2 段の spin lock が重なっている。

#### A.4 compound op のためのロック必要性の検討

3 つの compound op (prepend、drain_and_requeue、close_and_clean_up) について、外側ロックを削除可能かを個別に検証する:
...
**必要な atomicity**: `number_of_messages` の読み取り → capacity check → `enqueue_first` を N 回。
途中で他の producer が enqueue すると overflow check が壊れる。
...
- **案 a2**: 外側の `put_lock: RuntimeMutex<()>` を **prepend / drain_and_requeue / close 時のみ** 取る形に再設計し、通常の enqueue/dequeue では取らない (Pekko の `putLock` 相当) → **最小変更で実現可能**
...
**必要な atomicity**: drain → rebuild → error recovery の全体を atomic に行う。
途中で他の producer が enqueue すると、順序が崩れる。

**解決案**:
- 案 a1 と同じ trait 拡張 (`prepend_batch_with_capacity`) で内部 atomic 化 → **各 impl で lock を維持すれば可能**
...
**必要な atomicity**: `state.close()` 後の drain を他の enqueue と混じらせない。
ただし `state.close()` 後は他の producer の enqueue も suspended になるので、実質的に producer はいない。
...
- 残すとしても、close 経路は 1 回しか走らないので hot path への影響はない

#### A.5 単純 op の不要ロック削減検討

4 つの単純な op (#1, #4, #6, #7) について、**外側ロックを外しても安全か** を検証する:

##### op (1): `enqueue_envelope` の単一 enqueue

- 内側の `user.enqueue` は既に atomic (各 impl が内部 mutex を持つか、QueueStateHandle 経由で atomic)
- 外側の `user_queue_lock` を取る理由は **compound op (2/3) と同時に走ることを防ぐため**
- 外側ロックを外すと、(2/3) の途中で enqueue が割り込んで順序が壊れる
- → **(2/3) との排他が必要なだけで、純粋な enqueue には不要**
- → 案 a2 (put_lock を compound op 専用にする) なら、通常の enqueue は **lock 不要**
...
- 単純な read op
- 内側の mutex で取得可能な値 (bounded の場合は `QueueState.size: AtomicUsize`)
- 外側ロックを外すと、compound op の **途中の中間値** が見えるリスクがある
- しかし metric 目的なら中間値でも致命的ではない (多少不正確でよい)
- → **外側ロック不要**
...
2. **compound op のうち close は実は不要** (state 側で suspend が強制される)
3. **通常の enqueue/dequeue の外側ロックは compound op との排他のためだけ** に存在する
4. 以下の 2 案のいずれかで削減可能:
   - **案 a1**: `MessageQueue` trait に `prepend_batch_with_capacity` / `drain_and_rebuild_atomic` を追加する。各 impl が内部 mutex で atomic 化する。外側の `user_queue_lock` を撤廃する
   - **案 a2**: 外側の `user_queue_lock` を `put_lock: RuntimeMutex<()>` に改名し、**prepend と drain_and_requeue の時だけ取る**。通常の enqueue/dequeue/read では取らない (Pekko の `putLock` 方式)
...
さらに、`QueueStateHandle` 側の LOCK 2 と `SyncQueueShared` 側の LOCK 3 の **内側の二重ロック** は別問題として残る。これは:

- LOCK 2: `QueueState` の整合性 (`queue` と `size: AtomicUsize` と `WaitQueue`) のため
- LOCK 3: backend の整合性のため
...
**観察**:
- **Pekko の default (UnboundedMailbox) は 100% lock-free**。`ConcurrentLinkedQueue` を直接継承しており、Mailbox 層での追加のロックはない
- **`ReentrantLock putLock` が登場するのは `BoundedControlAwareMailbox` のみ**。その用途は:
  ...
  2. consumer の dequeue 後の `notFull` 通知 (Condition variable signal)
  3. control queue の優先順位保証
- **通常の Bounded (`BoundedMailbox`) は `LinkedBlockingQueue` に任せきり**。putLock/takeLock は JDK の BlockingQueue 実装の内部に隠蔽され、Mailbox 層では見えない
...
- **AtomicReference for head**: producer が CAS で head を更新する
...
- **`sync.Mutex` の使用はゼロ** (hot path)
- `sync.Mutex` は `actor/future.go` の `sync.Cond` 用と、test mock にしか登場しない
- 通常の enqueue は **2 つの atomic op (Push 内の atomic + size counter の AddInt32)** だけで完了する
...
- **2 つの atomic op** (SwapPointer + StorePointer) で push が完了する
- **1 つの atomic op** (LoadPointer) で pop が完了する
...
**結論**: fraktor-rs は参照実装と **本質的に異なるロック戦略** を採用している。参照実装は lock-free queue ベースなのに対し、fraktor-rs は lock-based queue ベースである。それぞれの trade-off:
...
fraktor-rs の `user_queue_lock` を Pekko の `ReentrantLock putLock` と比較すると、**責務は似ているが適用範囲が異なる**:
...
**fraktor-rs は Pekko の特殊ケース (`BoundedControlAwareMailbox`) の設計を default に一般化してしまっている** とも読める。これは参照実装と照らすと過剰であり、改善の余地がある。

#### C.7 結論: 参照実装準拠の進路

参照実装と整合させるなら、以下の 2 段階が理想:

1. **第 1 段階 (案 a2 相当)**: `user_queue_lock` を `put_lock` に改名し、**prepend/drain/close 時のみ取る**。通常の enqueue/dequeue/read では外側ロックを取らない。Pekko の `BoundedControlAwareMailbox putLock` に準拠する
2. **第 2 段階 (lock-free 化)**: `UnboundedDequeMessageQueue` / `BoundedMessageQueue` の内部実装を Vyukov MPSC queue ベースに差し替える。`SpinSyncMutex` の使用を撲滅する

第 1 段階は trait 変更なしで実現可能で、影響範囲も狭い。第 2 段階は lock-free queue の自作 (または既存 crate の採用) を要し、大きな別 change になる。

### B. ロック使用箇所の contention 特性分類

`modules/` 配下 (`utils-core/src/core/sync/` と tests 配下を除く) で `RuntimeMutex` / `RuntimeRwLock` / `SpinSyncMutex` / `SpinSyncRwLock` を field の型として保持するすべての型を列挙し、contention 特性に基づいて分類した。合計 **50 箇所**。
...
**合計 10 個** (actor-core 6, stream-core 4)。 **この 10 個が最重要の改善対象**。
...
**合計 5 個** (actor-core 2, stream-core 3)。RwLock の採用で read 優位の contention は緩和されている。
...
1. **重要: 当初想定の「153 ファイル」は過大評価**
   - `RuntimeMutex` / `RuntimeRwLock` を使う **ファイル** は ~115 だが、**field の型としての保持箇所は 50 個**
   - 残りは type alias の parameter 経由やドキュメント内での言及、test での新規作成、など
2. **stream-core も hot path を持つ**
   - SinkQueue, SourceQueue, Hub 系がすべて per-actor hot path
   - actor-core の Mailbox と同じ懸念 (spin の多段) が stream 側にもある
3. **RwLock の read 優位の場所が多い**
   - EventStream, Scheduler, SystemState, SerializationRegistry はすべて read >> write
   - spin::RwLock は read の同時取得を許すが、contention 下では writer が starve する可能性がある
4. **Cluster 系はほぼ lifecycle のみ**
   - cluster-core の 13 箇所すべてが init/coordinate フェーズ
   - hot path は ClusterExtension 内からでも actor-core の Mailbox 経由
5. **SpinSyncMutex を直接参照している箇所** (stream-core 6 個, utils-core 内部 1 個)
   - これらは `RuntimeMutex` 経由ではなく **直接 `SpinSyncMutex` を名指ししている**
...
6. **三重ロック問題 (調査 A) の延長が stream-core にも存在する可能性**
   ...
   - 本ドキュメントでは詳細に調査していないが、別 change で検証する価値がある
...
**hot path 対象は 15 個** (当初想定の 30 個から半減)。Phase 3 (actor-core/kernel のジェネリック化) のスコープをこれに合わせれば、本 change の工数は相当縮む。

### D. driver 候補と no_std lock-free queue の可否

Web 検索と docs.rs を併用して、fraktor-rs の MessageQueue バリアントを lock-free / low-contention な MPSC queue に置き換える候補を調査した。
...
1. **`heapless::mpmc::Queue` は deprecated**
   - ドキュメントに「preemption/park で queue が使用不能になり得るため truly lock-free でない」と明記されている
   - 容量上限 128 (feature で 256) も Mailbox には不足
   - 既に workspace に `heapless 0.9` があるので「既存の依存を活用する」という誘惑には警戒が必要
2. **`thingbuf::mpsc` が MPSC 専用の最適化を備えた no_std 対応の唯一の完成形**
   - `Sender`/`Receiver` の MPSC 専用 API
   - `alloc` feature または `static` feature で両対応
   - `BoundedMessageQueue` の置き換えに最適
   - ただし **bounded のみ** (unbounded バリアントには使えない)
   - slot-reference API のため `Envelope: Default` 制約の可能性あり (実装前に要確認)
3. **`crossbeam::SegQueue` は短期的に最小コストで移行可能**
   - `default-features = false, features = ["alloc"]` で no_std + alloc に対応
   - ただし内部に spinlock あり (完全な lock-free ではない、[Issue #675](https://github.com/crossbeam-rs/crossbeam/issues/675))
   - 実用上は `VecDeque + SpinMutex` より明確に良好
4. **自作の Vyukov MPSC が長期的にはベスト**
   - protoactor-go の `mpsc.go` (67 行) と Pekko の `AbstractNodeQueue.java` (約 150 行) がそのまま参照実装になる
   - 外部依存ゼロ (既存の `portable-atomic` + `alloc` のみ)
...
- workspace は既に `spin 0.10 (portable_atomic)`, `heapless 0.9`, `critical-section 1.2`, `embassy-sync 0.8`, `portable-atomic 1.11` を保持している
- **追加で検討すべき依存**: `crossbeam-queue` または `thingbuf` (どちらを選ぶかは Phase 1 で決定する)
...
**2 つの問題を別の change として整理する**:

1. **Queue 実装の置き換え** (Phase V 相当、別 change)
   - `UnboundedMessageQueue` → `crossbeam::SegQueue` → (将来) 自作の Vyukov MPSC
   - `BoundedMessageQueue` → `thingbuf::mpsc`
   - `UnboundedDequeMessageQueue` は据え置き
2. **Mutex driver の差し替え** (現在の `lock-driver-port-adapter` change + Phase IV)
   - `LockDriver<T>` port の導入
   - `utils-adaptor-std` で `parking_lot::Mutex` driver を提供
   - `critical-section` / `embassy_sync` は将来の `*-adaptor-embedded` で提供

この 2 つを**混ぜない**のが重要。queue の置き換えは Mailbox 層だけの変更、driver の差し替えは全 RuntimeMutex 使用箇所に及ぶ。
...
1. **Pekko の `putLock`** と fraktor-rs の `user_queue_lock` の責務のマッピング
   - どちらも compound op の atomicity を保証する
   - Pekko は通常の enqueue には使わない (atomic only)
   - fraktor-rs は両方に使っている → 改善の余地あり
2. **protoactor-go の lock-free 戦略** が fraktor-rs に適用可能か
   - `mpsc.Queue` (Go) の実装調査
   - Rust 版の lock-free MPSC (`crossbeam::queue::SegQueue`, `flume` 等) の no_std 可否
...
- 内側 2 段 (`QueueStateHandle` と `SyncQueueShared`) の統合は別 change の候補

#### E.2 default driver の推奨: 環境別に切り替える

調査 A/C で判明した事実から、「spin 一択」ではなく環境ごとに default を分けるのが正しい:
...
ただし **driver の切り替えの前に lock 段数の削減 (案 a2) が優先**。現状の spin のまま lock 段数を減らす方が効果が大きい。
...
Phase II (二重ロック削減) は **本 change より先に別の小さい change として行うべきか**、**本 change の後に続ける別 change か** を判断する必要がある。調査 B (contention 特性) で `user_queue_lock` 以外の lock 箇所で同じパターンがどの程度広がっているかを見てから決める。
...
3. **新規 openspec change の検討**
   ...
   - 既存の `lock-driver-port-adapter` との前後関係を決定する (先行か後続か)

#### E.6 リスクと trade-off サマリ

| リスク | 緩和策 |
|---|---|
| 現在の openspec change を進めると、後から二重ロック削減で対象の caller が変わる | change の scope を「Port/Adapter 導入」に限定し、「全 caller を factory ジェネリック化する」とは明言しない。必要最小限のみ actor-core/kernel を対象にする |
| 二重ロック削減を先行すると、deadlock 検知基盤の整備が遅れる | Phase II は小さい PR で 1 週間以内に完了させる。DebugSpinSync は Phase III で導入する |
| lock-free 化 (Phase V) で no_std 対応が間に合わない | Phase V は別 change で後日、no_std 対応の Vyukov MPSC の自作または外部 crate (調査 D の結果次第) |
| parking_lot driver の導入 (Phase IV) で std 依存が増える | parking_lot は `utils-adaptor-std` 側に置く (utils-core は no_std の純度を維持) |
...
当初「actor-core/kernel の shared 型 30 個を factory でジェネリック化」と想定していたが、調査 B で **field の保持箇所の total は 50 個** と確定した。さらに以下の内訳:
...
**Phase 3 の factory ジェネリック化の対象を hot path 15 個に限定する**。non-hot path 35 個は Phase 4 の per-crate alias で済ませる。これで:

- actor-core/kernel の変更ファイル数が ~30 → ~10 に削減される
- stream-core にも同様のファクトリ化が必要 (Phase 3b として追加) 、~4 ファイル
- actor-core + stream-core の hot path のみ factory 化する

#### E.7.2 Queue の置き換え (新 Phase V) は独立した change で扱う (調査 D より)

調査 D の結論「queue の置き換えと Mutex driver の置き換えは独立した 2 つの問題」を採用し、**本 change のスコープから明確に除外** する:
...
2. **stream-core の SinkQueue/SourceQueue/Hub 系の同型の二重ロック調査** (別 change の候補、本 change の対象外)

stream-core の調査は Phase II とは別に、ボーイスカウトルール的に別 change で進める。

#### E.7.4 最終版の進行順序

> **注意**: この節は E.8 の再検証で改訂された。最新の進行順序は E.8 の「改訂後の作業順序」を参照。以下は E.7 時点の旧版として残す。

```
Phase I:  調査 (本ドキュメント)                              [COMPLETED]
          └─ 二重ロック判定、参照実装対比、contention 分類、queue 候補調査

Phase II: (新規) 二重ロック削減の小 change                  [COMPLETED → E.8 で再設計]
...
Phase III: (既存 openspec) lock-driver-port-adapter         [基盤完了、factory 化は未完]
          ├─ LockDriver<T> / RwLockDriver<T> trait 導入済み
          ├─ DefaultMutex<T> / DefaultRwLock<T> compile-time 切替済み
          ├─ 残: actor-core hot path 15 個の factory genericization
          ├─ 残: stream-core の per-crate alias + factory 化
          └─ 残: test instrumentation example (deadlock 検知)

Phase IV: driver 選定 (新規 change)                         [一部完了]
          ├─ std-locks-feature: std::sync::Mutex/RwLock backend 追加済み
          ├─ debug-locks: CheckedSpinSyncMutex 再入検知は利用可能
          ├─ 残: parking_lot driver 追加 (workspace 依存は存在)
          └─ 残: tokio 環境向け推奨を文書化
...
**proposal.md**:
- Why セクション末尾に「本 change は調査 A/B/C/D (`docs/plan/lock-strategy-analysis.md`) の Phase III に該当する」を追加する
- Non-Goals に以下を明示する:
  - Mailbox の queue 実装の lock-free 化 (別 change `mailbox-lock-free-queue` で扱う)
...
- Impact セクションの「影響を受ける caller」を調査 B の数字 (50 個中 15 個が factory 化の対象) に更新する

**design.md**:
- Context セクションに調査結果の要約を追加する
...
- Decision 7 の caller migration 戦略を調査 B の分類に基づいて精緻化する
- Open Questions に `docs/plan/lock-strategy-analysis.md` へのポインタを追加する
- Risks に「Phase V (queue の置き換え) で本 change の成果物の一部が不要になる可能性」を追加する

**tasks.md**:
- Phase 3 のタスクを hot path 15 個に限定する (inventory の数字を調整)
- Phase 3b (stream-core の factory 化) を追加する
- Phase 4 の per-crate alias の対象を 35 個に明示する

**specs/utils-lock-driver-port/spec.md**:
- 変更は最小限 (既に strict valid なので、文言の微調整のみ)

#### E.7.6 短期アクション優先順位

> **2026-04-13 更新**: 優先度 1〜2 は完了。現在の優先順位は E.8 の「改訂後の作業順序」を参照。

```
優先度 1 (完了):
  ├─ 本ドキュメントを commit (調査記録)                          [DONE]
  └─ lock-driver-port-adapter の proposal/design/tasks を更新     [DONE → archive]

優先度 2 (完了):
  ├─ Phase II: mailbox-close-reject-enqueue                      [DONE → archive]
  ├─ Phase III: stash-requires-deque-mailbox                     [DONE → archive]
  └─ Phase III.5: mailbox-prepend-requires-deque                 [DONE → archive]

優先度 3 (次の作業):
  ├─ Phase IV: outer lock reduction (前提条件は全て充足済み)
  └─ Phase V: factory genericization 完遂 (hot path 15 箇所)

優先度 4 (将来):
  ├─ Phase VI: parking_lot driver 追加
  ├─ Phase VII: queue lock-free 化
  └─ Phase IX: stream-core 調査
```

### E.8 2026-04-08 再検証による訂正と計画改訂

撤回された `remove-mailbox-outer-lock` proposal と、その後の `mailbox-close-reject-enqueue` proposal のレビューを通じて、上記の一部の結論には**前提の誤認**が含まれていたことが判明した。特に以下の 3 点は、**本節が旧記述を supersede する**:
...
3. **案 a2 (`put_lock` 限定化) は前提条件なしには unsafe**
   - close correctness を先に直さずに通常の enqueue/dequeue から outer lock を外すと、cleanup と in-flight producer の race を閉じられない
   - したがって、旧 E.3 / E.7.3 / E.7.4 の「Phase II = `put_lock` 限定化」はそのままでは採用できない

#### 改訂後の作業順序

上記の再検証を踏まえると、全体計画は次の順序に改めるのが妥当:

```text
Phase I:  調査 (本ドキュメント)                                  [COMPLETED]
          └─ 三重ロック / 参照実装乖離 / contention 分類 / queue 候補

Phase II: mailbox-close-reject-enqueue                          [COMPLETED]
...
Phase III: stash-requires-deque-mailbox                         [COMPLETED]
...
Phase III.5: mailbox-prepend-requires-deque                     [COMPLETED]
...
Phase IV:  outer lock reduction の再提案                        [NEXT — 前提条件は全て充足]
           ├─ Phase II/III/III.5 完了済み。案 a2 (put_lock 限定化) が安全に採用可能
           ├─ close correctness を壊さない形で lock 段数削減を設計
           └─ 必要なら `remove-mailbox-outer-lock` を別 proposal として再作成

Phase V:   lock-driver-port-adapter                             [基盤完了 / factory 化は未完]
           ├─ LockDriver<T> / RwLockDriver<T> trait は utils-core に導入済み
           ├─ DefaultMutex<T> / DefaultRwLock<T> compile-time 切替は完了
           ├─ 残: actor-core hot path 15 箇所の factory genericization
           └─ deadlock 検知 / driver 差し替えの土台を整備

Phase VI:  driver 選定                                          [一部完了]
           ├─ compile-time-lock-backend-selection: DefaultMutex type alias 導入済み
           ├─ std-locks-feature: std::sync::Mutex/RwLock backend 追加済み
           ├─ debug-locks: CheckedSpinSyncMutex による再入検知は利用可能
           ├─ parking_lot: workspace 依存に存在 (0.12.5) だが driver 未作成
           └─ 残: parking_lot driver の追加、std 環境向け default 最終決定
...
1. **close correctness が outer lock 削減より先** → **完了** (`mailbox-close-reject-enqueue`)
   - `enqueue_envelope` / `prepend_user_messages` が lock 内で `is_closed()` を再チェックする
   - `become_closed_and_clean_up` が `user_queue_lock` で直列化済み
2. **stash requirement の伝播が次の前提条件** → **完了** (`stash-requires-deque-mailbox`)
   - `MailboxRequirement::for_stash()` が伝播済み
3. **prepend deque-only 契約** → **完了** (`mailbox-prepend-requires-deque`)
   - `prepend_via_drain_and_requeue` は削除済み
4. **BalancingDispatcher は別トラック** — 変更なし
5. **`lock-driver-port-adapter` は基盤が完了** (`new-lock-driver-port-adapter`)
   - `LockDriver<T>` / `RwLockDriver<T>` trait 導入済み
   - `DefaultMutex<T>` の compile-time 切替済み (debug-locks / std-locks / default)
   - 残: actor-core hot path の factory genericization
6. **Phase IV (outer lock reduction) の前提条件はすべて充足** — 案 a2 (put_lock 限定化) が安全に採用可能

### 最終結論

本ドキュメントの現時点 (2026-04-13) の結論:

1. **fraktor-rs の Mailbox は参照実装から大きく乖離している** (lock-based か lock-free か、2〜3 段ロックか atomic か) — この事実は変わらない
2. **outer lock 削減の前提条件 (close correctness, stash deque requirement, prepend deque-only 契約) はすべて充足済み** — Phase II / III / III.5 が完了した
3. **ロックバックエンドの compile-time 切替の基盤は完成** — `DefaultMutex` type alias + `std-locks` feature により、std 環境では `std::sync::Mutex` ベースに自動で切り替わる
4. **次の高優先度は Phase IV (outer lock reduction) と Phase V (factory genericization の完遂)**
...
| change | Phase | 概要 |
|---|---|---|
| `mailbox-close-reject-enqueue` | II | lock 内での is_closed 再チェック、close correctness の回復 |
| `stash-requires-deque-mailbox` | III | MailboxRequirement::for_stash() の伝播 |
| `mailbox-prepend-requires-deque` | III.5 | prepend を deque-only 契約に硬化、drain_and_requeue の削除 |
| `compile-time-lock-backend-selection` | V 基盤 | DefaultMutex/DefaultRwLock type alias、debug-locks feature |
| `new-lock-driver-port-adapter` | V 設計 | LockDriver/RwLockDriver trait、LockDriverFactory の導入 |
| `utils-sync-collapse` | V 関連 | utils-core の sync モジュール整理 |
| `shared-lock-consolidation` | V 関連 | SharedLock/SharedRwLock の統合 |
| `eliminate-direct-spin-sync-construction` | V 関連 | SpinSyncMutex の直接構築の撲滅 |
| `decompose-actor-shared-factories` | V 関連 | Shared 型ファクトリの分解 |
| `remove-shared-wrapper-factories` | V 関連 | 不要な Shared ラッパーファクトリの削除 |

### 進行中

| change | Phase | 概要 |
|---|---|---|
| `std-locks-feature` | VI (一部) | StdSyncMutex/StdSyncRwLock を utils-core に新設、std 環境で DefaultMutex = StdSyncMutex に切替える |

### 未着手 (次のアクション候補)

| Phase | 概要 | 優先度 |
|---|---|---|
| **IV: outer lock reduction** | 通常の enqueue/dequeue から user_queue_lock を外す。前提条件はすべて充足済み | **高** |
| **V: factory genericization 完遂** | actor-core hot path 15 箇所の factory ジェネリック化 | **高** |
| VI: parking_lot driver | utils-adaptor-std に parking_lot ベースの driver を追加する | 中 |
| VII: queue lock-free 化 | MessageQueue 実装を lock-free queue ベースに差し替える | 低 (将来) |
| VIII: BalancingDispatcher close | shared queue の close semantics | 低 (将来) |
| IX: stream-core 調査 | SinkQueue/SourceQueue/Hub の同型の問題 | 低 (将来) |

## 関連ドキュメント

- `openspec/changes/archive/2026-04-13-new-lock-driver-port-adapter/` (lock-driver-port-adapter 設計、archive 済み)
...
- `modules/utils-core/src/core/sync/std_sync_mutex.rs` (std-locks backend)

## 変更履歴

- **2026-04-08**: 初版作成。3 つの課題、依存関係、4 案の比較、案 γ 推奨、調査項目の整理まで。
- **2026-04-08**: `remove-mailbox-outer-lock` / `mailbox-close-reject-enqueue` のレビュー結果を反映。`prepend_via_drain_and_requeue` の production reachability、close semantics の race、案 a2 を先行させる危険性を追記し、Phase の順序を改訂。
- **2026-04-13**: 現状のコードに合わせて全体を更新。Phase II / III / III.5 の完了を反映。`compile-time-lock-backend-selection` / `std-locks-feature` / `new-lock-driver-port-adapter` 等の完了済み change を記録。E.7.4 / E.8 の Phase ステータスを更新。次のアクション候補を Phase IV (outer lock reduction) と Phase V (factory genericization) に集約。
